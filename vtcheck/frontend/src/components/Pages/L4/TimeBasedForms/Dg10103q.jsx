...
const DG10103tb = () => {
  ...
  const dropdownLabels = [  'Use LOTO before start start of the work at incomer supply (Acceptance Criteria - Lock out with life tag)',
    ...
    'Check & tight all electrical connections(Acceptance Criteria - Good in condition & Tighten)',
    'Check & tight all nut-bolts  (Acceptance Criteria - Good in condition & Tighten)',
    'Check for oil level (Acceptance Criteria - Maximum)',
    ...
    'Check for any  leakage  (Acceptance Criteria - No Leakage)',
  ...
  const inputLabels = [
    'Check the coolent level  (Maximum.)', 
    'Check Neutral and earthing of DG electrical panel (As per std.)', 
    'Check for battery terminal voltage with multimeter (Should not excess than 13.5 V) (13.5 V)',
    'Check the RPM of Engine(Rated= 1500)  ', 
  ];

  useEffect(() => {
    // Fetch submission status for the current quarter
    axios.get('https://scheq.in/api/dg_1010_3_tb/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/dg_1010_3_tb', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      <h1 className='text-3xl font-semibold mb-8 text-center'>Time-Based Checksheet</h1>
      <h1 className='text-4xl font-semibold mb-6 text-center'> D.G.-4- 1010 KVA ESN NO.- 25366157</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : D.G.-4- 1010 KVA ESN NO.- 25366157</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(EQUIPMENT NO:  VT/FM/UTM/DG-4/1010KVA/6191)</h1>
      <hr /><hr /><br /><br />
