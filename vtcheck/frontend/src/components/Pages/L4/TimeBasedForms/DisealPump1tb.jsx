...
const DisealPump1tb = () => {
  ...
  const dropdownLabels = [  'Use LOTO before start start of the work at incomer supply (Acceptance Criteria - Lock out with life tag)',
    'Check battery water level. If require top up to be done. (Acceptance Criteria - Check)',
    'Check battery voltage (Multimeter) (Acceptance Criteria - Below 2V)',
    'Check battery terminal condition (Acceptance Criteria - Good & Tighten)',
    'Check earthen (Multimeter) (Acceptance Criteria - Below 2V)',
    'Check & clean oil filter (Acceptance Criteria - Clean )',
    'Check & clean fuel filter (Acceptance Criteria - Clean)',
    'Check & clean air filter (Acceptance Criteria -Clean)',
    'Check hose pipe condition & tightenness  (Acceptance Criteria - Good in condition & tighten )',
    'Check all pressure gauges condition & tightness (Acceptance Criteria - Good & Tighten)',
    'Check all pipe line for any leakage (Acceptance Criteria - Good in condition & No leakages )',
    'Check engine panel for all connections  (Acceptance Criteria - Good in condition & tighten)',
    'Check all panel switches condition  (Acceptance Criteria - Ok)',
    'Check diseal tank level & condition (Acceptance Criteria - Good in condition & maximum level)',
    'Clean the unit  (Acceptance Criteria -Clean)',
    'Take the trial of unit(Unit Trial) (Acceptance Criteria - Running Ok )',
    'Check for any abnormal vibration(Feel)  (Acceptance Criteria - No vibration)',
    'Check noise level which is to be below harmful level (Acceptance Criteria - Below 85db)',
    'Check the Diseal/ oil leakage (Acceptance Criteria - No leakage)',
  ...
  useEffect(() => {
    ...
    axios.get('https://scheq.in/api/diesel_pump_1_tb/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/diesel_pump_1_tb', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> Fire House- Diseal Pump</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : Fire House- Diseal Pump</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(EQUIPMENT NO: VT/FM/UTM/FH/DP/3005)</h1>
      <hr /><hr /><br /><br />
