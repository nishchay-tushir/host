...
const VCB1m = () => {
  ...
  const dropdownLabels = [  'Use LOTO before start start of the work at incomer supply (Acceptance Criteria - Lock out with life tag)',
    ...
    'Check the condition of SIC contact  (Acceptance Criteria - Ok )',
    'Checking proper closing of all poles together (Acceptance Criteria - proper )',
    ...
    'Checking motor operation in case of EDO (Acceptance Criteria - Tighten)',
    'Checking of auxiliary contacts continuity proper change over (Acceptance Criteria - Available )',
    'Checking presence of all circlips (Acceptance Criteria - Ok )',
  ...
  useEffect(() => {
    ...
    axios.get('https://scheq.in/api/vaccum_circuit_breaker_vcb_1_m/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/vaccum_circuit_breaker_vcb_1_m', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> Vacuum circuit breaker</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME: vacuum circuit breaker</h1>
