...
const FireWaterLine4m = () => {
  ...
  const dropdownLabels = [  'Check water  pipe line (Acceptance Criteria - No damage)',
    'Check for any leakage (Acceptance Criteria - No Leakage)',
    'Ckeck for all joints (Acceptance Criteria - Tighten & No damage)',
    'Check all valves (Acceptance Criteria - Tighten & No damage)',
    'Check all pipe line support & clamping (Acceptance Criteria - Tighten & No damage)',
  ];  

  const inputLabels = [
    'Check pressure gauges (Acceptance Criteria - 10-12 kg/cm^2)',
  ];

  useEffect(() => {
    // Fetch submission status for today
    axios.get('https://scheq.in/api/fire_water_line_4_m/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/fire_water_line_4_m', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'>Fire Water Line - 4</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : Fire water line (Inhouse-Admin Building Area)</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(EQUIPMENT NO: VT/FM/UTM/ADMINA/FIRELINE/6221)</h1>
      <hr /><hr /><br /><br />
...
export default FireWaterLine4m;