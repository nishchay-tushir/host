...
const RetractableGate1m = () => {
  ...
  const dropdownLabels = [  'Use LOTO before start start of the work at incomer supply (Acceptance Criteria - Lock out with life tag)',
    'Check the wheel  (Acceptance Criteria - OK)',
    'Check Structure (Acceptance Criteria - OK & Tighten)',
    'Check motor  (Acceptance Criteria - OK & Noiseless)',
    'Check operating panel  (Acceptance Criteria - OK & Tighten & No damage)',
    'Check electrical cable  (Acceptance Criteria - OK )',
  ...
  const inputLabels = [
    'Check electrical connections (Acceptance Criteria -  As per Std.)'
  ];

  useEffect(() => {
    // Fetch submission status for today
    axios.get('https://scheq.in/api/retractable_gate_1_m/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/retractable_gate_1_m', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'>Retractable gate</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : Retractable gate at Main gate</h1>
...
export default RetractableGate1m;