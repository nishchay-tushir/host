...
const StreetLight4m = () => {
  ...
  const dropdownLabels = [  'Use LOTO before start start of the work at incomer supply (Acceptance Criteria - Lock out with life tag)',
    'Check for any damage or broken (Acceptance Criteria - No damage/broken)',
    'Check & clean switch control panel (Acceptance Criteria - Clean)',
    'Check pole condition (Acceptance Criteria - Ok)',
    'Check for any burn wire or curcuit  (Acceptance Criteria - No burning smell)',
    'Check the fire extinguisher place near chiller unit or not. (Acceptance Criteria - Nearest to unit)',
    'Check pole mtg. condition (Acceptance Criteria - Tighten)',
  ];
  const inputLabels = [
    'Check lux level (Lux meter) (Acceptance Criteria - 300-400)'
  ];

  useEffect(() => {
    // Fetch submission status for today
    axios.get('https://scheq.in/api/street_lights_4_m/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/street_lights_4_m', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> Street Light Pole-Play ground area-Double light</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : Street Light Pole-Main gate-Front side gargen area-Single light</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(EQUIPMENT NO: VT/FM/UTM/MGGAR/SL/SIL/3244 TO 3259)</h1>
      <hr /><hr /><br /><br />
