...
const DG10103m = () => {
  ...
  const inputLabels = [
    'Check the Battery charging terminal in the synchornization panel(24V- DC)', 'Check for battery terminal voltage with multimeter (Should not excess than 13.5 V)', 'Check the RPM of Engine(Rated= 1500)  ', 'Exhuast level of DG must be in acceptable government norms','Check emission of CO2 '
  ];

  useEffect(() => {
    // Fetch submission status for today
    axios.get('https://scheq.in/api/dg_1010_3_m/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/dg_1010_3_m', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> DG - 1010</h1>
