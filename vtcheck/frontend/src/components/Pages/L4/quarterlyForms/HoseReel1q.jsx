...
const HoseReel1q = () => {
  ...
  const dropdownLabels = [  'Check for any damage (Acceptance Criteria - No damage)',
    ...
    'Check hose pipe condition (Acceptance Criteria - Good in condition)',
    'Check nozzle of hose pipe (Acceptance Criteria - Good in condition)',
  ...
  useEffect(() => {
    ...
    axios.get('https://scheq.in/api/hose_reel_1_q/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/hose_reel_1_q', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> Hose Reel</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : Hose Reel</h1>
