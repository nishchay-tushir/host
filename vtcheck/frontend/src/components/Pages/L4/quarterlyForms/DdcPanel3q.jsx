...
const DdcPanel3q = () => {
  ...
  const dropdownLabels = [  'Use LOTO before start start of the work at incomer supply (Acceptance Criteria - Lock out with life tag)',
    ...
    'check for any burn wire or circuit (Multimeter) (Acceptance Criteria -No burning smell)',
  ...
  useEffect(() => {
    ...
    axios.get('https://scheq.in/api/ddc_panel_3_q/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/ddc_panel_3_q', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> DDC Panel-3 </h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : DDC Panel- Near Air Cooled Chiller</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(EQUIPMENT NO: VT/FM/UTM/NRACC/DDCP/308 TO 309)</h1>
      <hr /><hr /><br /><br />
