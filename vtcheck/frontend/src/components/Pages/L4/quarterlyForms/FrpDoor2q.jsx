...
const FrpDoor2q = () => {
  ...
  const dropdownLabels = [  'Check door for any damage (Acceptance Criteria - No damage)',
    ...
    'Check \'In use\' indicator  (Acceptance Criteria - Good in condition)',
    ...
    'Check cusion gasket (Acceptance Criteria - Good in condition )',
  ...
  useEffect(() => {
    ...
    axios.get('https://scheq.in/api/frp_door_2_q/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/frp_door_2_q', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> FRP Door</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : FRP Door-Electronic production area-wash room-L</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(EQUIPMENT NO: VT/FM/UTM/ELECT/PRODA/WR-L/FRPD/5495 TO 5498)</h1>
      <hr /><hr /><br /><br />
