...
const NitrogenYard1q = () => {
  ...
  const visualInspectionLabels = [
    'Check level  (Acceptance Criteria - Above 1000)',
    'Check pressure (Acceptance Criteria - 10-12 Bar)',
    'Check pipe ice formation on pipe (Acceptance Criteria - Ice free)',
    'Check water point  (Acceptance Criteria - ON)',
  ];

  const argonSectionLabels = [
    'Check level  (Acceptance Criteria - Above 1000)',
    'Check pressure (Acceptance Criteria - 10-12 Bar)',
    'Check pipe ice formation on pipe (Acceptance Criteria - Ice free)',
    'Check water point  (Acceptance Criteria - ON)',
  ...
  useEffect(() => {
    axios.get('https://scheq.in/api/nitrogen_yard_1_q/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/nitrogen_yard_1_q', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      <h1 className='text-3xl font-semibold mb-8 text-center'>Quarterly Checksheet</h1>
        ...
        <>
          <form onSubmit={handleSubmit}>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              {visualInspectionLabels.map((label, i) => (
                <div key={i} className='flex flex-col'>
                  <label className='text-md font-medium mb-1'>{label}</label>
                  <select 
                    name={`dropdown${i + 1}`} 
                    value={formData[`dropdown${i + 1}`]} 
            ...
            <h1 className='text-2xl font-semibold mb-6 text-center'>Argon Section</h1><br />
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              {argonSectionLabels.map((label, i) => (
                <div key={i + visualInspectionLabels.length} className='flex flex-col'>
                  <label className='text-md font-medium mb-1'>{label}</label>
                  <select 
                    name={`dropdown${i + 1 + visualInspectionLabels.length}`} 
                    value={formData[`dropdown${i + 1 + visualInspectionLabels.length}`]} 
                    onChange={handleChange} 
                    className='p-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500'
                  >
                    <option value="" disabled>Select</option>
            ...
            <div className='flex justify-center'>
              <button 
                type="submit" 
                className='mt-6 px-6 py-3 bg-blue-500 text-white font-semibold rounded-md shadow-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500'
              >
                Submit
              </button>
            </div>
