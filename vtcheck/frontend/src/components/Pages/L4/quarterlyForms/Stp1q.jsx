...
const Stp1q = () => {
  ...
  const dropdownLabels = [  'Use LOTO before start of work at incomer supply (Acceptance Criteria - Lock out with life tag)',
    ...
    'Check for pressure gauges for tightening (Acceptance Criteria - Tighten)',
    'Check & tight all nut bolts of joints (Acceptance Criteria -  Tighten)',
    'check motor foundation  (Acceptance Criteria - Clean Manually)',
    'Check the filter if any cloggage found (keep it clean) (Acceptance Criteria -  Clean Manually)',
    'Check the air at the outlet of air blower (Acceptance Criteria -  Air outlet (ok))',
    'Sewage effluent test: The effluent tests must meet the standards. (Acceptance Criteria - Result (PH/TDS))',
    'Bar screen cleaning should be done twice in a month  (Acceptance Criteria -  Clean Manually)',
    'Check the working of the dosing pump  (Acceptance Criteria -  OK)',
    'Check any leakage of water which has abnormal aspets to environment  (Acceptance Criteria -  No LeakageOK)',
  ...
  useEffect(() => {
    ...
    axios.get('https://scheq.in/api/stp_1_q/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/stp_1_q', formData)
      .then(response => {
