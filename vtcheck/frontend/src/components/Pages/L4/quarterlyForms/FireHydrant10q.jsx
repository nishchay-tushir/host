...
const FireHydrant10q = () => {
  ...
  const dropdownLabels = [  'Check for any damage (Acceptance Criteria - No damage)',
    'Check for proper tightening of bolts (Acceptance Criteria - Tighten)',
    'Check valve condition (Acceptance Criteria - Good in condition)',
    'Check wheel condition (Acceptance Criteria - Good in condition)',
    'Check hose pipe clamp (Acceptance Criteria - Good in condition & Tighten)',
    'Check nozzle condition (Acceptance Criteria - Tighten )',
    'Check for unit cleanliness (Acceptance Criteria - Clean)',
  ]; 
  
  useEffect(() => {
    // Fetch submission status for the current quarter
    axios.get('https://scheq.in/api/fire_hydrant_10_q/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/fire_hydrant_10_q', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> Fire hydrant-10</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : Fire Hydrant Hose Reel-Pent House L2-(1,2)</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(EQUIPMENT NO: VT/FM/UTM/FH/PHL2/HR/3215 TO 3216 )</h1>
      <hr /><hr /><br /><br />
