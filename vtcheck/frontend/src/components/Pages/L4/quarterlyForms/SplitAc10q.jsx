...
const SplitAC10q = () => {
  ...
  const dropdownLabels = [  'Use LOTO before start start of the work at incomer supply (Acceptance Criteria - Lock out with life tag)',
    ...
    'Check & clean  the filter condition (Acceptance Criteria - Clean By Blower)',
    'Inspect the condenser coil (Acceptance Criteria - OK/NOT OK )',
    'Check the drain line for any leakage  (Acceptance Criteria - No leakage)',
    ...
    'Check the remote control (Acceptance Criteria - Trial)',
  ]; 
  
  useEffect(() => {
    // Fetch submission status for the current quarter
    axios.get('https://scheq.in/api/split_ac_10_q/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/split_ac_10_q', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> SPLIT AC-10</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : Split AC- Admin office- Swagon compressor room AC-1</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(EQUIPMENT NO:VT/FM/UTM/ADOFF/SWAGCOMR/SPTAC/6070)</h1>
      <hr /><hr /><br /><br />
