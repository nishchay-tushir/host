...
const RapidSlidingGate1q = () => {
  ...
  const dropdownLabels = [  'Use LOTO before start start of the work at incomer supply (Acceptance Criteria - Lock out with life tag)',
    'Check condion of shutter (Acceptance Criteria - OK)',
    'Check sensor  (Acceptance Criteria - OK)',
    'Check & clean belt & pulley  (Acceptance Criteria - Tighten )',
    'Check gasket   (Acceptance Criteria - OK)',
  ...
  useEffect(() => {
    ...
    axios.get('https://scheq.in/api/rapid_sliding_gate_1_q/status')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/rapid_sliding_gate_1_q', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'> Rapid sliding gate</h1>
      <h1 className='text-xl font-semibold text-center'>EQUIPMENT NAME : Rapid sliding gate-PLNA line room</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(EQUIPMENT NO: VT/FM/UTM/PLNAL/RSG/5975)</h1>
      <hr /><hr /><br /><br />
...
export default RapidSlidingGate1q;