...
const ACB2d = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://scheq.in/api/acb_2_d/status')
      .then(response => {
    ...
    const timer = setTimeout(() => {
      if (submittedToday !== 'Filled for today') {
        ...
        axios.post('https://scheq.in/api/acb_2_d', autoFormData)
          .then(response => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://scheq.in/api/acb_2_d', formData)
      .then(response => {
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg my-6'>
      ...
      <h1 className='text-4xl font-semibold mb-6 text-center'>ACB - 2</h1>
      <h1 className='text-xl font-semibold text-center'>Equipment Name: Air  circuit breaker (L&T) (Incoming supply from main mcc panel)</h1>
      <h1 className='text-xl font-semibold mb-6 text-center'>(Equipment no: VT/FM/UTM/ACB/018)</h1>
      <hr /><hr /><br /><br />
