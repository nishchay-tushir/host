...
const InhouseLight90qSchema = new mongoose.Schema({
...
const InhouseLight90q = mongoose.model('InhouseLight90q', InhouseLight90qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight90q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight90q({
        ...req.body,
...
try {
    const records = await InhouseLight90q.find();
...
try {
    const updatedRecord = await InhouseLight90q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight90q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
