...
const Ups13qSchema = new mongoose.Schema({
...
const Ups13q = mongoose.model('Ups13q', Ups13qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups13q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups13q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups13q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups13q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
