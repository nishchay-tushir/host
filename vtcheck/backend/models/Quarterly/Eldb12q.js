...
const Eldb12qSchema = new mongoose.Schema({
...
const Eldb12q = mongoose.model('Eldb12q', Eldb12qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb12q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb12q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb12q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb12q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
