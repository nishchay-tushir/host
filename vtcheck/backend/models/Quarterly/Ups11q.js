...
const Ups11qSchema = new mongoose.Schema({
...
const Ups11q = mongoose.model('Ups11q', Ups11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
