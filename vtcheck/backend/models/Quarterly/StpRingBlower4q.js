...
const StpRingBlower4qSchema = new mongoose.Schema({
...
const StpRingBlower4q = mongoose.model('StpRingBlower4q', StpRingBlower4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StpRingBlower4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StpRingBlower4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpRingBlower4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StpRingBlower4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StpRingBlower4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
