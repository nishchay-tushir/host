...
const Sprinkler4qSchema = new mongoose.Schema({
...
const Sprinkler4q = mongoose.model('Sprinkler4q', Sprinkler4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Sprinkler4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new Sprinkler4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Sprinkler4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Sprinkler4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Sprinkler4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
