...
const CassetteAc12qSchema = new mongoose.Schema({
...
const CassetteAc12q = mongoose.model('CassetteAc12q', CassetteAc12qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc12q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc12q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc12q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc12q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc12q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
