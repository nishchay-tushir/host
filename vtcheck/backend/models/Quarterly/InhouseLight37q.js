...
const InhouseLight37qSchema = new mongoose.Schema({
...
const InhouseLight37q = mongoose.model('InhouseLight37q', InhouseLight37qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight37q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight37q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight37q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight37q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight37q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
