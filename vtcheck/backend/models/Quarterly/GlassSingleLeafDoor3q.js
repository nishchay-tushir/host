...
const GlassSingleLeafDoor3qSchema = new mongoose.Schema({
...
const GlassSingleLeafDoor3q = mongoose.model('GlassSingleLeafDoor3q', GlassSingleLeafDoor3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassSingleLeafDoor3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassSingleLeafDoor3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassSingleLeafDoor3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassSingleLeafDoor3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassSingleLeafDoor3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
