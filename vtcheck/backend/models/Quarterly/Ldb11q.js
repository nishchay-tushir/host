...
const Ldb11qSchema = new mongoose.Schema({
...
const Ldb11q = mongoose.model('Ldb11q', Ldb11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
