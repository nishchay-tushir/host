...
const Pdb7qSchema = new mongoose.Schema({
...
const Pdb7q = mongoose.model('Pdb7q', Pdb7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
