...
const BeamDetector5qSchema = new mongoose.Schema({
...
const BeamDetector5q = mongoose.model('BeamDetector5q', BeamDetector5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await BeamDetector5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new BeamDetector5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await BeamDetector5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await BeamDetector5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await BeamDetector5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
