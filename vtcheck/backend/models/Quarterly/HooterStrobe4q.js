...
const HooterStrobe4qSchema = new mongoose.Schema({
...
const HooterStrobe4q = mongoose.model('HooterStrobe4q', HooterStrobe4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await HooterStrobe4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new HooterStrobe4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HooterStrobe4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HooterStrobe4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HooterStrobe4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
