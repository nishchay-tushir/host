...
const Panel46qSchema = new mongoose.Schema({
...
const Panel46q = mongoose.model('Panel46q', Panel46qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel46q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel46q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel46q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel46q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
