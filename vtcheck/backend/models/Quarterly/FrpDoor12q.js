...
const FrpDoor12qSchema = new mongoose.Schema({
...
const FrpDoor12q = mongoose.model('FrpDoor12q', FrpDoor12qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor12q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FrpDoor12q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor12q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor12q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor12q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
