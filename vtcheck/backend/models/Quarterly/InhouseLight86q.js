...
const InhouseLight86qSchema = new mongoose.Schema({
...
const InhouseLight86q = mongoose.model('InhouseLight86q', InhouseLight86qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight86q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight86q({
        ...req.body,
...
try {
    const records = await InhouseLight86q.find();
...
try {
    const updatedRecord = await InhouseLight86q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight86q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
