...
const FrpDoor7qSchema = new mongoose.Schema({
...
const FrpDoor7q = mongoose.model('FrpDoor7q', FrpDoor7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
