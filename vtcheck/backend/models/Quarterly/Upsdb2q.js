...
const Upsdb2qSchema = new mongoose.Schema({
...
const Upsdb2q = mongoose.model('Upsdb2q', Upsdb2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Upsdb2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Upsdb2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Upsdb2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Upsdb2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
