...
const InhouseLight32qSchema = new mongoose.Schema({
...
const InhouseLight32q = mongoose.model('InhouseLight32q', InhouseLight32qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight32q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight32q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight32q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight32q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight32q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
