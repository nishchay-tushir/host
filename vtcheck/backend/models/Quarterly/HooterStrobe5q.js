...
const HooterStrobe5qSchema = new mongoose.Schema({
...
const HooterStrobe5q = mongoose.model('HooterStrobe5q', HooterStrobe5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await HooterStrobe5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new HooterStrobe5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HooterStrobe5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HooterStrobe5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HooterStrobe5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
