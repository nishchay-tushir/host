...
const InhouseLight54qSchema = new mongoose.Schema({
...
const InhouseLight54q = mongoose.model('InhouseLight54q', InhouseLight54qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight54q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight54q({
        ...req.body,
...
try {
    const records = await InhouseLight54q.find();
...
try {
    const updatedRecord = await InhouseLight54q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight54q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
