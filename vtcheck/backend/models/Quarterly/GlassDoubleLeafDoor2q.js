...
const GlassDoubleLeafDoor2qSchema = new mongoose.Schema({
...
const GlassDoubleLeafDoor2q = mongoose.model('GlassDoubleLeafDoor2q', GlassDoubleLeafDoor2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassDoubleLeafDoor2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassDoubleLeafDoor2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassDoubleLeafDoor2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassDoubleLeafDoor2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassDoubleLeafDoor2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
