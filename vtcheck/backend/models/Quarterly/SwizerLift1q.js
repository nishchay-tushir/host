...
const SwizerLift1qSchema = new mongoose.Schema({
...
const SwizerLift1q = mongoose.model('SwizerLift1q', SwizerLift1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SwizerLift1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SwizerLift1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SwizerLift1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SwizerLift1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SwizerLift1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
