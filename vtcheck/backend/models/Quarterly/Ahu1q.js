...
const Ahu1qSchema = new mongoose.Schema({
...
const Ahu1q = mongoose.model('Ahu1q', Ahu1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ahu1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ahu1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ahu1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ahu1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
