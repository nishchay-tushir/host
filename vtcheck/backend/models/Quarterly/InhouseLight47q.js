...
const InhouseLight47qSchema = new mongoose.Schema({
...
const InhouseLight47q = mongoose.model('InhouseLight47q', InhouseLight47qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight47q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight47q({
        ...req.body,
...
try {
    const records = await InhouseLight47q.find();
...
try {
    const updatedRecord = await InhouseLight47q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight47q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
