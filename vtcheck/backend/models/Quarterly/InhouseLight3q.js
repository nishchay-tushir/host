...
const InhouseLight3qSchema = new mongoose.Schema({
...
const InhouseLight3q = mongoose.model('InhouseLight3q', InhouseLight3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
