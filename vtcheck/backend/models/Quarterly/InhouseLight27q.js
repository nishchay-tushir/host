...
const InhouseLight27qSchema = new mongoose.Schema({
...
const InhouseLight27q = mongoose.model('InhouseLight27q', InhouseLight27qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight27q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight27q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight27q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight27q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight27q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
