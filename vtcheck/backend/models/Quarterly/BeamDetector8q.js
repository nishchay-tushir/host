...
const BeamDetector8qSchema = new mongoose.Schema({
...
const BeamDetector8q = mongoose.model('BeamDetector8q', BeamDetector8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await BeamDetector8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new BeamDetector8q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await BeamDetector8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await BeamDetector8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await BeamDetector8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
