...
const SplitAc4qSchema = new mongoose.Schema({
...
const SplitAc4q = mongoose.model('SplitAc4q', SplitAc4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
