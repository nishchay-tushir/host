...
const RollingShutter2qSchema = new mongoose.Schema({
...
const RollingShutter2q = mongoose.model('RollingShutter2q', RollingShutter2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
