...
const InhouseLight58qSchema = new mongoose.Schema({
...
const InhouseLight58q = mongoose.model('InhouseLight58q', InhouseLight58qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight58q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight58q({
        ...req.body,
...
try {
    const records = await InhouseLight58q.find();
...
try {
    const updatedRecord = await InhouseLight58q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight58q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
