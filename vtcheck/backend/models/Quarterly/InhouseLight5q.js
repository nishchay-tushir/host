...
const InhouseLight5qSchema = new mongoose.Schema({
...
const InhouseLight5q = mongoose.model('InhouseLight5q', InhouseLight5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
