...
const CassetteAc16qSchema = new mongoose.Schema({
...
const CassetteAc16q = mongoose.model('CassetteAc16q', CassetteAc16qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc16q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc16q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc16q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc16q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc16q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
