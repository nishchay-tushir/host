...
const Eldb13qSchema = new mongoose.Schema({
...
const Eldb13q = mongoose.model('Eldb13q', Eldb13qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb13q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb13q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb13q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb13q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
