...
const FireHydrant8qSchema = new mongoose.Schema({
...
const FireHydrant8q = mongoose.model('FireHydrant8q', FireHydrant8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant8q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
