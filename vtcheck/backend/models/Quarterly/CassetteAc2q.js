...
const CassetteAc2qSchema = new mongoose.Schema({
...
const CassetteAc2q = mongoose.model('CassetteAc2q', CassetteAc2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
