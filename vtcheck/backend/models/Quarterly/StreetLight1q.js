...
const StreetLight1qSchema = new mongoose.Schema({
...
const StreetLight1q = mongoose.model('StreetLight1q', StreetLight1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StreetLight1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StreetLight1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StreetLight1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StreetLight1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StreetLight1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
