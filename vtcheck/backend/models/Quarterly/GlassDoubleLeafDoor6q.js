...
const GlassDoubleLeafDoor6qSchema = new mongoose.Schema({
...
const GlassDoubleLeafDoor6q = mongoose.model('GlassDoubleLeafDoor6q', GlassDoubleLeafDoor6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassDoubleLeafDoor6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassDoubleLeafDoor6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassDoubleLeafDoor6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassDoubleLeafDoor6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassDoubleLeafDoor6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
