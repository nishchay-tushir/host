...
const CassetteAc3qSchema = new mongoose.Schema({
...
const CassetteAc3q = mongoose.model('CassetteAc3q', CassetteAc3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
