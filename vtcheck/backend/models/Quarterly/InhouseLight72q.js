...
const InhouseLight72qSchema = new mongoose.Schema({
...
const InhouseLight72q = mongoose.model('InhouseLight72q', InhouseLight72qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight72q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight72q({
        ...req.body,
...
try {
    const records = await InhouseLight72q.find();
...
try {
    const updatedRecord = await InhouseLight72q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight72q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
