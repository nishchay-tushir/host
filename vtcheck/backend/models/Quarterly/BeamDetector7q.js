...
const BeamDetector7qSchema = new mongoose.Schema({
...
const BeamDetector7q = mongoose.model('BeamDetector7q', BeamDetector7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await BeamDetector7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new BeamDetector7q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await BeamDetector7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await BeamDetector7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await BeamDetector7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
