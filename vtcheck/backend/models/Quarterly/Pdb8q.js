...
const Pdb8qSchema = new mongoose.Schema({
...
const Pdb8q = mongoose.model('Pdb8q', Pdb8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
