...
const Panel11qSchema = new mongoose.Schema({
...
const Panel11q = mongoose.model('Panel11q', Panel11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
