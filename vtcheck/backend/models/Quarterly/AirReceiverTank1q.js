...
const AirReceiverTank1qSchema = new mongoose.Schema({
...
const AirReceiverTank1q = mongoose.model('AirReceiverTank1q', AirReceiverTank1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await AirReceiverTank1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new AirReceiverTank1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirReceiverTank1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirReceiverTank1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirReceiverTank1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
