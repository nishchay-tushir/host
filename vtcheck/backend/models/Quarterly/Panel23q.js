...
const Panel23qSchema = new mongoose.Schema({
...
const Panel23q = mongoose.model('Panel23q', Panel23qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel23q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel23q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel23q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel23q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
