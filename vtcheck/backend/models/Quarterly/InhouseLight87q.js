...
const InhouseLight87qSchema = new mongoose.Schema({
...
const InhouseLight87q = mongoose.model('InhouseLight87q', InhouseLight87qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight87q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight87q({
        ...req.body,
...
try {
    const records = await InhouseLight87q.find();
...
try {
    const updatedRecord = await InhouseLight87q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight87q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
