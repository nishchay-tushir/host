...
const Pdb9qSchema = new mongoose.Schema({
...
const Pdb9q = mongoose.model('Pdb9q', Pdb9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
