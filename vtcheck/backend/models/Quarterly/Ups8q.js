...
const Ups8qSchema = new mongoose.Schema({
...
const Ups8q = mongoose.model('Ups8q', Ups8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
