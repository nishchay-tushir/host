...
const CassetteAc8qSchema = new mongoose.Schema({
...
const CassetteAc8q = mongoose.model('CassetteAc8q', CassetteAc8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc8q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
