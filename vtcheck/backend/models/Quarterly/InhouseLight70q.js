...
const InhouseLight70qSchema = new mongoose.Schema({
...
const InhouseLight70q = mongoose.model('InhouseLight70q', InhouseLight70qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight70q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight70q({
        ...req.body,
...
try {
    const records = await InhouseLight70q.find();
...
try {
    const updatedRecord = await InhouseLight70q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight70q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
