...
const StreetLight4qSchema = new mongoose.Schema({
...
const StreetLight4q = mongoose.model('StreetLight4q', StreetLight4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StreetLight4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StreetLight4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StreetLight4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StreetLight4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StreetLight4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
