...
const Ups18qSchema = new mongoose.Schema({
...
const Ups18q = mongoose.model('Ups18q', Ups18qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups18q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups18q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups18q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups18q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
