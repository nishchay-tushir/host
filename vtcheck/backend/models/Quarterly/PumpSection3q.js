...
const PumpSection3qSchema = new mongoose.Schema({
...
const PumpSection3q = mongoose.model('PumpSection3q', PumpSection3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await PumpSection3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new PumpSection3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await PumpSection3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await PumpSection3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await PumpSection3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
