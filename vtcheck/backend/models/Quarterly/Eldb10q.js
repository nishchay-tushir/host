...
const Eldb10qSchema = new mongoose.Schema({
...
const Eldb10q = mongoose.model('Eldb10q', Eldb10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
