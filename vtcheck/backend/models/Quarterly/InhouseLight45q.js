...
const InhouseLight45qSchema = new mongoose.Schema({
...
const InhouseLight45q = mongoose.model('InhouseLight45q', InhouseLight45qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight45q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight45q({
        ...req.body,
...
try {
    const records = await InhouseLight45q.find();
...
try {
    const updatedRecord = await InhouseLight45q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight45q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
