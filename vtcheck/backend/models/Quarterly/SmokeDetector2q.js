...
const SmokeDetector2qSchema = new mongoose.Schema({
...
const SmokeDetector2q = mongoose.model('SmokeDetector2q', SmokeDetector2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SmokeDetector2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SmokeDetector2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SmokeDetector2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SmokeDetector2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SmokeDetector2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
