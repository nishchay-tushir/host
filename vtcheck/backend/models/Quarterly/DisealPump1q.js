...
const DisealPump1qSchema = new mongoose.Schema({
...
const DisealPump1q = mongoose.model('DisealPump1q', DisealPump1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DisealPump1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new DisealPump1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DisealPump1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DisealPump1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DisealPump1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
