...
const InhouseLight29qSchema = new mongoose.Schema({
...
const InhouseLight29q = mongoose.model('InhouseLight29q', InhouseLight29qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight29q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight29q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight29q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight29q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight29q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
