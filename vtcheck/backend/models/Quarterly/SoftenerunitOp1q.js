...
const SoftenerUnitOp1qSchema = new mongoose.Schema({
...
const SoftenerUnitOp1q = mongoose.model('SoftenerUnitOp1q', SoftenerUnitOp1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SoftenerUnitOp1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SoftenerUnitOp1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SoftenerUnitOp1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SoftenerUnitOp1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SoftenerUnitOp1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
