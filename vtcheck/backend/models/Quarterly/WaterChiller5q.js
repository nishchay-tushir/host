...
const WaterChiller5qSchema = new mongoose.Schema({
...
const WaterChiller5q = mongoose.model('WaterChiller5q', WaterChiller5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WaterChiller5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WaterChiller5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WaterChiller5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WaterChiller5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
