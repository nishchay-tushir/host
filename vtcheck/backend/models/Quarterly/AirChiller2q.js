...
const AirChiller2qSchema = new mongoose.Schema({
...
const AirChiller2q = mongoose.model('AirChiller2q', AirChiller2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await AirChiller2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new AirChiller2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirChiller2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirChiller2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirChiller2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
