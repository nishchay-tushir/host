...
const InhouseLight6qSchema = new mongoose.Schema({
...
const InhouseLight6q = mongoose.model('InhouseLight6q', InhouseLight6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
