...
const InhouseLight21qSchema = new mongoose.Schema({
...
const InhouseLight21q = mongoose.model('InhouseLight21q', InhouseLight21qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight21q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight21q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight21q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight21q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight21q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
