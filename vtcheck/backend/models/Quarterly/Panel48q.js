...
const Panel48qSchema = new mongoose.Schema({
...
const Panel48q = mongoose.model('Panel48q', Panel48qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel48q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel48q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel48q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel48q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
