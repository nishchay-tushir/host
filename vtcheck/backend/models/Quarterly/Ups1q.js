...
const Ups1qSchema = new mongoose.Schema({
...
const Ups1q = mongoose.model('Ups1q', Ups1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
