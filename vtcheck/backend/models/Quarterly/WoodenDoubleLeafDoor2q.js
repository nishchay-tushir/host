...
const WoodenDoubleLeafDoor2qSchema = new mongoose.Schema({
...
const WoodenDoubleLeafDoor2q = mongoose.model('WoodenDoubleLeafDoor2q', WoodenDoubleLeafDoor2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WoodenDoubleLeafDoor2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WoodenDoubleLeafDoor2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenDoubleLeafDoor2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WoodenDoubleLeafDoor2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WoodenDoubleLeafDoor2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
