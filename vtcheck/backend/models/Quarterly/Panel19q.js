...
const Panel19qSchema = new mongoose.Schema({
...
const Panel19q = mongoose.model('Panel19q', Panel19qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel19q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel19q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel19q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel19q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
