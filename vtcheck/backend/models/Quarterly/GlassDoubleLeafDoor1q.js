...
const GlassDoubleLeafDoor1qSchema = new mongoose.Schema({
...
const GlassDoubleLeafDoor1q = mongoose.model('GlassDoubleLeafDoor1q', GlassDoubleLeafDoor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassDoubleLeafDoor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassDoubleLeafDoor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassDoubleLeafDoor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassDoubleLeafDoor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassDoubleLeafDoor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
