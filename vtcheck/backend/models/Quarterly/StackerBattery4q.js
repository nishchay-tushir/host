...
const StackerBattery4qSchema = new mongoose.Schema({
...
const StackerBattery4q = mongoose.model('StackerBattery4q', StackerBattery4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StackerBattery4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StackerBattery4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StackerBattery4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StackerBattery4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StackerBattery4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
