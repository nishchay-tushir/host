...
const PumpSection1qSchema = new mongoose.Schema({
...
const PumpSection1q = mongoose.model('PumpSection1q', PumpSection1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await PumpSection1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new PumpSection1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await PumpSection1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await PumpSection1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await PumpSection1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
