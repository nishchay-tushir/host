...
const RollingShutter9qSchema = new mongoose.Schema({
...
const RollingShutter9q = mongoose.model('RollingShutter9q', RollingShutter9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter9q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
