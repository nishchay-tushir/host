...
const Ups6qSchema = new mongoose.Schema({
...
const Ups6q = mongoose.model('Ups6q', Ups6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
