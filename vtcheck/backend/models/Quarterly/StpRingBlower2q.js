...
const StpRingBlower2qSchema = new mongoose.Schema({
...
const StpRingBlower2q = mongoose.model('StpRingBlower2q', StpRingBlower2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StpRingBlower2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StpRingBlower2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpRingBlower2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StpRingBlower2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StpRingBlower2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
