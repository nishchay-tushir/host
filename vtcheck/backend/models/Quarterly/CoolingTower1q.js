...
const CoolingTower1qSchema = new mongoose.Schema({
...
const CoolingTower1q = mongoose.model('CoolingTower1q', CoolingTower1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CoolingTower1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CoolingTower1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CoolingTower1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CoolingTower1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CoolingTower1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
