...
const InhouseLight38qSchema = new mongoose.Schema({
...
const InhouseLight38q = mongoose.model('InhouseLight38q', InhouseLight38qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight38q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight38q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight38q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight38q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight38q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
