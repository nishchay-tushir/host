...
const Ups5qSchema = new mongoose.Schema({
...
const Ups5q = mongoose.model('Ups5q', Ups5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
