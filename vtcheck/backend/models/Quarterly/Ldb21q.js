...
const Ldb21qSchema = new mongoose.Schema({
...
const Ldb21q = mongoose.model('Ldb21q', Ldb21qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb21q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb21q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb21q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb21q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
