...
const MotorisedSlidingGate1qSchema = new mongoose.Schema({
...
const MotorisedSlidingGate1q = mongoose.model('MotorisedSlidingGate1q', MotorisedSlidingGate1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await MotorisedSlidingGate1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new MotorisedSlidingGate1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MotorisedSlidingGate1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await MotorisedSlidingGate1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await MotorisedSlidingGate1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
