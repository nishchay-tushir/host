...
const InhouseLight69qSchema = new mongoose.Schema({
...
const InhouseLight69q = mongoose.model('InhouseLight69q', InhouseLight69qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight69q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight69q({
        ...req.body,
...
try {
    const records = await InhouseLight69q.find();
...
try {
    const updatedRecord = await InhouseLight69q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight69q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
