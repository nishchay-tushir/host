...
const InhouseLight33qSchema = new mongoose.Schema({
...
const InhouseLight33q = mongoose.model('InhouseLight33q', InhouseLight33qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight33q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight33q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight33q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight33q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight33q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
