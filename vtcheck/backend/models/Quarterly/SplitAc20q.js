...
const SplitAc20qSchema = new mongoose.Schema({
...
const SplitAc20q = mongoose.model('SplitAc20q', SplitAc20qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc20q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc20q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc20q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc20q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc20q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
