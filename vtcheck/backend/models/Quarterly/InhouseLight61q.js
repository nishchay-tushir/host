...
const InhouseLight61qSchema = new mongoose.Schema({
...
const InhouseLight61q = mongoose.model('InhouseLight61q', InhouseLight61qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight61q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight61q({
        ...req.body,
...
try {
    const records = await InhouseLight61q.find();
...
try {
    const updatedRecord = await InhouseLight61q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight61q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
