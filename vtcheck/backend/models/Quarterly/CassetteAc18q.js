...
const CassetteAc18qSchema = new mongoose.Schema({
...
const CassetteAc18q = mongoose.model('CassetteAc18q', CassetteAc18qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc18q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc18q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc18q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc18q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc18q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
