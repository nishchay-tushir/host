...
const Pdb5qSchema = new mongoose.Schema({
...
const Pdb5q = mongoose.model('Pdb5q', Pdb5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
