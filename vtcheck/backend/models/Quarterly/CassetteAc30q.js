...
const CassetteAc30qSchema = new mongoose.Schema({
...
const CassetteAc30q = mongoose.model('CassetteAc30q', CassetteAc30qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc30q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc30q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc30q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc30q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc30q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
