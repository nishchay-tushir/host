...
const InhouseLight93qSchema = new mongoose.Schema({
...
const InhouseLight93q = mongoose.model('InhouseLight93q', InhouseLight93qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight93q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight93q({
        ...req.body,
...
try {
    const records = await InhouseLight93q.find();
...
try {
    const updatedRecord = await InhouseLight93q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight93q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
