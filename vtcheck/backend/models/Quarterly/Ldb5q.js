...
const Ldb5qSchema = new mongoose.Schema({
...
const Ldb5q = mongoose.model('Ldb5q', Ldb5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
