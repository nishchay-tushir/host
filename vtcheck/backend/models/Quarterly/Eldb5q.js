...
const Eldb5qSchema = new mongoose.Schema({
...
const Eldb5q = mongoose.model('Eldb5q', Eldb5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
