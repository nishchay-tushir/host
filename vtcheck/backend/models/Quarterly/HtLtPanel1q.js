...
const HtLtPanel1qSchema = new mongoose.Schema({
...
const HtLtPanel1q = mongoose.model('HtLtPanel1q', HtLtPanel1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await HtLtPanel1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new HtLtPanel1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HtLtPanel1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HtLtPanel1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HtLtPanel1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
