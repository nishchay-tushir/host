...
const CassetteAc19qSchema = new mongoose.Schema({
...
const CassetteAc19q = mongoose.model('CassetteAc19q', CassetteAc19qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc19q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc19q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc19q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc19q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc19q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
