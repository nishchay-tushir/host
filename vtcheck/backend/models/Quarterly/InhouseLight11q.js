...
const InhouseLight11qSchema = new mongoose.Schema({
...
const InhouseLight11q = mongoose.model('InhouseLight11q', InhouseLight11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight11q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
