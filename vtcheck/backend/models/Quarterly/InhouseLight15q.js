...
const InhouseLight15qSchema = new mongoose.Schema({
...
const InhouseLight15q = mongoose.model('InhouseLight15q', InhouseLight15qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight15q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight15q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight15q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight15q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight15q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
