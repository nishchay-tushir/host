...
const ManualCallPt2qSchema = new mongoose.Schema({
...
const ManualCallPt2q = mongoose.model('ManualCallPt2q', ManualCallPt2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await ManualCallPt2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new ManualCallPt2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await ManualCallPt2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ManualCallPt2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ManualCallPt2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
