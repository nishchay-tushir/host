...
const InhouseLight49qSchema = new mongoose.Schema({
...
const InhouseLight49q = mongoose.model('InhouseLight49q', InhouseLight49qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight49q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight49q({
        ...req.body,
...
try {
    const records = await InhouseLight49q.find();
...
try {
    const updatedRecord = await InhouseLight49q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight49q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
