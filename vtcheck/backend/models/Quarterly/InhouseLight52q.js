...
const InhouseLight52qSchema = new mongoose.Schema({
...
const InhouseLight52q = mongoose.model('InhouseLight52q', InhouseLight52qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight52q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight52q({
        ...req.body,
...
try {
    const records = await InhouseLight52q.find();
...
try {
    const updatedRecord = await InhouseLight52q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight52q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
