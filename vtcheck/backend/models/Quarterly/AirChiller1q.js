...
const AirChiller1qSchema = new mongoose.Schema({
...
const AirChiller1q = mongoose.model('AirChiller1q', AirChiller1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await AirChiller1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new AirChiller1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirChiller1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirChiller1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirChiller1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
