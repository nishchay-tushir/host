...
const RollingShutter3qSchema = new mongoose.Schema({
...
const RollingShutter3q = mongoose.model('RollingShutter3q', RollingShutter3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
