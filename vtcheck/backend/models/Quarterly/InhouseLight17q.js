...
const InhouseLight17qSchema = new mongoose.Schema({
...
const InhouseLight17q = mongoose.model('InhouseLight17q', InhouseLight17qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight17q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight17q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight17q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight17q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight17q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
