...
const SmokeDetector4qSchema = new mongoose.Schema({
...
const SmokeDetector4q = mongoose.model('SmokeDetector4q', SmokeDetector4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SmokeDetector4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SmokeDetector4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SmokeDetector4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SmokeDetector4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SmokeDetector4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
