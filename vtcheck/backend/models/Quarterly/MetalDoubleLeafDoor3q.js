...
const MetalDoubleLeafDoor3qSchema = new mongoose.Schema({
...
const MetalDoubleLeafDoor3q = mongoose.model('MetalDoubleLeafDoor3q', MetalDoubleLeafDoor3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await MetalDoubleLeafDoor3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new MetalDoubleLeafDoor3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MetalDoubleLeafDoor3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await MetalDoubleLeafDoor3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await MetalDoubleLeafDoor3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
