...
const Sprinkler1qSchema = new mongoose.Schema({
...
const Sprinkler1q = mongoose.model('Sprinkler1q', Sprinkler1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Sprinkler1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new Sprinkler1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Sprinkler1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Sprinkler1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Sprinkler1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
