...
const BeamDetector3qSchema = new mongoose.Schema({
...
const BeamDetector3q = mongoose.model('BeamDetector3q', BeamDetector3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await BeamDetector3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new BeamDetector3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await BeamDetector3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await BeamDetector3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await BeamDetector3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
