...
const PdbPanel1qSchema = new mongoose.Schema({
...
const PdbPanel1q = mongoose.model('PdbPanel1q', PdbPanel1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await PdbPanel1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new PdbPanel1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await PdbPanel1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await PdbPanel1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await PdbPanel1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
