...
const UpsdbPanel1qSchema = new mongoose.Schema({
...
const UpsdbPanel1q = mongoose.model('UpsdbPanel1q', UpsdbPanel1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await UpsdbPanel1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new UpsdbPanel1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await UpsdbPanel1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await UpsdbPanel1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await UpsdbPanel1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
