...
const Panel28qSchema = new mongoose.Schema({
...
const Panel28q = mongoose.model('Panel28q', Panel28qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel28q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel28q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel28q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel28q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
