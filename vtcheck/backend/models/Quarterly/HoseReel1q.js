...
const HoseReel1qSchema = new mongoose.Schema({
...
const HoseReel1q = mongoose.model('HoseReel1q', HoseReel1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await HoseReel1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new HoseReel1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HoseReel1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HoseReel1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HoseReel1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
