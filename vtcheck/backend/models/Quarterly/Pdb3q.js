...
const Pdb3qSchema = new mongoose.Schema({
...
const Pdb3q = mongoose.model('Pdb3q', Pdb3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
