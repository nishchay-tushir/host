...
const WoodenSingleLeafDoor5qSchema = new mongoose.Schema({
...
const WoodenSingleLeafDoor5q = mongoose.model('WoodenSingleLeafDoor5q', WoodenSingleLeafDoor5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WoodenSingleLeafDoor5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WoodenSingleLeafDoor5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenSingleLeafDoor5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WoodenSingleLeafDoor5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WoodenSingleLeafDoor5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
