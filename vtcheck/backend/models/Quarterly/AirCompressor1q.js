...
const AirCompressor1qSchema = new mongoose.Schema({
...
const AirCompressor1q = mongoose.model('AirCompressor1q', AirCompressor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await AirCompressor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new AirCompressor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirCompressor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirCompressor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirCompressor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
