...
const SplitAc16qSchema = new mongoose.Schema({
...
const SplitAc16q = mongoose.model('SplitAc16q', SplitAc16qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc16q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc16q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc16q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc16q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc16q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
