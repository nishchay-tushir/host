...
const InhouseLight96qSchema = new mongoose.Schema({
...
const InhouseLight96q = mongoose.model('InhouseLight96q', InhouseLight96qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight96q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight96q({
        ...req.body,
...
try {
    const records = await InhouseLight96q.find();
...
try {
    const updatedRecord = await InhouseLight96q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight96q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
