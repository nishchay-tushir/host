...
const StackerBattery3qSchema = new mongoose.Schema({
...
const StackerBattery3q = mongoose.model('StackerBattery3q', StackerBattery3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StackerBattery3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StackerBattery3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StackerBattery3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StackerBattery3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StackerBattery3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
