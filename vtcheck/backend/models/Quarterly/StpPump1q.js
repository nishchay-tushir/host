...
const StpPump1qSchema = new mongoose.Schema({
...
const StpPump1q = mongoose.model('StpPump1q', StpPump1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StpPump1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpPump1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StpPump1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StpPump1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
