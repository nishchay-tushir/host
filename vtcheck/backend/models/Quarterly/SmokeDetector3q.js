...
const SmokeDetector3qSchema = new mongoose.Schema({
...
const SmokeDetector3q = mongoose.model('SmokeDetector3q', SmokeDetector3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SmokeDetector3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SmokeDetector3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SmokeDetector3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SmokeDetector3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SmokeDetector3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
