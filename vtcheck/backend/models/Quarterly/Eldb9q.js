...
const Eldb9qSchema = new mongoose.Schema({
...
const Eldb9q = mongoose.model('Eldb9q', Eldb9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
