...
const FrpDoor11qSchema = new mongoose.Schema({
...
const FrpDoor11q = mongoose.model('FrpDoor11q', FrpDoor11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FrpDoor11q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
