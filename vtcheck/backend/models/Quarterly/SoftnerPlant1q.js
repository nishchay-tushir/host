...
const SoftnerPlant1qSchema = new mongoose.Schema({
...
const SoftnerPlant1q = mongoose.model('SoftnerPlant1q', SoftnerPlant1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SoftnerPlant1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SoftnerPlant1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SoftnerPlant1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SoftnerPlant1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SoftnerPlant1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
