...
const InhouseLight85qSchema = new mongoose.Schema({
...
const InhouseLight85q = mongoose.model('InhouseLight85q', InhouseLight85qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight85q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight85q({
        ...req.body,
...
try {
    const records = await InhouseLight85q.find();
...
try {
    const updatedRecord = await InhouseLight85q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight85q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
