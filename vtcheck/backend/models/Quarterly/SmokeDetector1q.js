...
const SmokeDetector1qSchema = new mongoose.Schema({
...
const SmokeDetector1q = mongoose.model('SmokeDetector1q', SmokeDetector1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SmokeDetector1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SmokeDetector1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SmokeDetector1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SmokeDetector1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SmokeDetector1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
