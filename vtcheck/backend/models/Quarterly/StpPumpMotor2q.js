...
const StpPumpMotor2qSchema = new mongoose.Schema({
...
const StpPumpMotor2q = mongoose.model('StpPumpMotor2q', StpPumpMotor2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StpPumpMotor2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StpPumpMotor2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpPumpMotor2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StpPumpMotor2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StpPumpMotor2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
