...
const Eldb6qSchema = new mongoose.Schema({
...
const Eldb6q = mongoose.model('Eldb6q', Eldb6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
