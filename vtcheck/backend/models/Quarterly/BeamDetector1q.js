...
const BeamDetector1qSchema = new mongoose.Schema({
...
const BeamDetector1q = mongoose.model('BeamDetector1q', BeamDetector1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await BeamDetector1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new BeamDetector1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await BeamDetector1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await BeamDetector1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await BeamDetector1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
