...
const ManualCallPt3qSchema = new mongoose.Schema({
...
const ManualCallPt3q = mongoose.model('ManualCallPt3q', ManualCallPt3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await ManualCallPt3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new ManualCallPt3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await ManualCallPt3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ManualCallPt3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ManualCallPt3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
