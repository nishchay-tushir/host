...
const StackerBattery1qSchema = new mongoose.Schema({
...
const StackerBattery1q = mongoose.model('StackerBattery1q', StackerBattery1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StackerBattery1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StackerBattery1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StackerBattery1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StackerBattery1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StackerBattery1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
