...
const FrpDoor3qSchema = new mongoose.Schema({
...
const FrpDoor3q = mongoose.model('FrpDoor3q', FrpDoor3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
