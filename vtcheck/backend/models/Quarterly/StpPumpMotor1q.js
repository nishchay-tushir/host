...
const StpPumpMotor1qSchema = new mongoose.Schema({
...
const StpPumpMotor1q = mongoose.model('StpPumpMotor1q', StpPumpMotor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StpPumpMotor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StpPumpMotor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpPumpMotor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StpPumpMotor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StpPumpMotor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
