...
const Ups16qSchema = new mongoose.Schema({
...
const Ups16q = mongoose.model('Ups16q', Ups16qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups16q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups16q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups16q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups16q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
