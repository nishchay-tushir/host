...
const WaterChiller1qSchema = new mongoose.Schema({
...
const WaterChiller1q = mongoose.model('WaterChiller1q', WaterChiller1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WaterChiller1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WaterChiller1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WaterChiller1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WaterChiller1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
