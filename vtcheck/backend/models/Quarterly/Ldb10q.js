...
const Ldb10qSchema = new mongoose.Schema({
...
const Ldb10q = mongoose.model('Ldb10q', Ldb10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
