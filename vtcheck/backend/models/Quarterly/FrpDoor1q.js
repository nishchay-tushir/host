...
const FrpDoor1qSchema = new mongoose.Schema({
...
const FrpDoor1q = mongoose.model('FrpDoor1q', FrpDoor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
