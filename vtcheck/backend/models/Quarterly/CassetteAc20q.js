...
const CassetteAc20qSchema = new mongoose.Schema({
...
const CassetteAc20q = mongoose.model('CassetteAc20q', CassetteAc20qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc20q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc20q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc20q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc20q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc20q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
