...
const DdcPanel6qSchema = new mongoose.Schema({
...
const DdcPanel6q = mongoose.model('DdcPanel6q', DdcPanel6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DdcPanel6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new DdcPanel6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DdcPanel6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DdcPanel6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DdcPanel6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
