...
const SplitAc11qSchema = new mongoose.Schema({
...
const SplitAc11q = mongoose.model('SplitAc11q', SplitAc11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc11q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
