...
const StackerBattery2qSchema = new mongoose.Schema({
...
const StackerBattery2q = mongoose.model('StackerBattery2q', StackerBattery2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StackerBattery2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StackerBattery2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StackerBattery2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StackerBattery2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StackerBattery2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
