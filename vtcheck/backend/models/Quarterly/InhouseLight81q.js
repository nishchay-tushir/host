...
const InhouseLight81qSchema = new mongoose.Schema({
...
const InhouseLight81q = mongoose.model('InhouseLight81q', InhouseLight81qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight81q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight81q({
        ...req.body,
...
try {
    const records = await InhouseLight81q.find();
...
try {
    const updatedRecord = await InhouseLight81q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight81q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
