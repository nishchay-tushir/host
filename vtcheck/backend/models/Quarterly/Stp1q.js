...
const Stp1qSchema = new mongoose.Schema({
...
const Stp1q = mongoose.model('Stp1q', Stp1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Stp1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Stp1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Stp1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Stp1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
