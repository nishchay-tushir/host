...
const InhouseLight41qSchema = new mongoose.Schema({
...
const InhouseLight41q = mongoose.model('InhouseLight41q', InhouseLight41qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight41q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight41q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight41q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight41q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight41q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
