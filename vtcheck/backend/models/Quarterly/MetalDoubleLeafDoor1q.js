...
const MetalDoubleLeafDoor1qSchema = new mongoose.Schema({
...
const MetalDoubleLeafDoor1q = mongoose.model('MetalDoubleLeafDoor1q', MetalDoubleLeafDoor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await MetalDoubleLeafDoor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new MetalDoubleLeafDoor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MetalDoubleLeafDoor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await MetalDoubleLeafDoor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await MetalDoubleLeafDoor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
