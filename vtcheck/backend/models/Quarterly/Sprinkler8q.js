...
const Sprinkler8qSchema = new mongoose.Schema({
...
const Sprinkler8q = mongoose.model('Sprinkler8q', Sprinkler8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Sprinkler8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new Sprinkler8q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Sprinkler8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Sprinkler8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Sprinkler8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
