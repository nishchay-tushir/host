...
const InhouseLight64qSchema = new mongoose.Schema({
...
const InhouseLight64q = mongoose.model('InhouseLight64q', InhouseLight64qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight64q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight64q({
        ...req.body,
...
try {
    const records = await InhouseLight64q.find();
...
try {
    const updatedRecord = await InhouseLight64q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight64q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
