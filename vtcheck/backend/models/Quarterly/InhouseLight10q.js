...
const InhouseLight10qSchema = new mongoose.Schema({
...
const InhouseLight10q = mongoose.model('InhouseLight10q', InhouseLight10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight10q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
