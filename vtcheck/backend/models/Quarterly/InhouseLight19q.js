...
const InhouseLight19qSchema = new mongoose.Schema({
...
const InhouseLight19q = mongoose.model('InhouseLight19q', InhouseLight19qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight19q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight19q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight19q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight19q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight19q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
