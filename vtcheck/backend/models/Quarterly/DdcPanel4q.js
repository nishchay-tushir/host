...
const DdcPanel4qSchema = new mongoose.Schema({
...
const DdcPanel4q = mongoose.model('DdcPanel4q', DdcPanel4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DdcPanel4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new DdcPanel4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DdcPanel4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DdcPanel4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DdcPanel4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
