...
const RapidRollingShutter1qSchema = new mongoose.Schema({
...
const RapidRollingShutter1q = mongoose.model('RapidRollingShutter1q', RapidRollingShutter1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RapidRollingShutter1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RapidRollingShutter1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidRollingShutter1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RapidRollingShutter1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RapidRollingShutter1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
