...
const FrpDoor13qSchema = new mongoose.Schema({
...
const FrpDoor13q = mongoose.model('FrpDoor13q', FrpDoor13qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor13q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FrpDoor13q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor13q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor13q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor13q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
