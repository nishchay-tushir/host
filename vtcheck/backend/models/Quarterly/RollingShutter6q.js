...
const RollingShutter6qSchema = new mongoose.Schema({
...
const RollingShutter6q = mongoose.model('RollingShutter6q', RollingShutter6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
