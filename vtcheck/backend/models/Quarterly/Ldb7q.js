...
const Ldb7qSchema = new mongoose.Schema({
...
const Ldb7q = mongoose.model('Ldb7q', Ldb7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
