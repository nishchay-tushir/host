...
const CassetteAc28qSchema = new mongoose.Schema({
...
const CassetteAc28q = mongoose.model('CassetteAc28q', CassetteAc28qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc28q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc28q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc28q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc28q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc28q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
