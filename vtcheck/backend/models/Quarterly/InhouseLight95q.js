...
const InhouseLight95qSchema = new mongoose.Schema({
...
const InhouseLight95q = mongoose.model('InhouseLight95q', InhouseLight95qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight95q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight95q({
        ...req.body,
...
try {
    const records = await InhouseLight95q.find();
...
try {
    const updatedRecord = await InhouseLight95q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight95q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
