...
const Ldb22qSchema = new mongoose.Schema({
...
const Ldb22q = mongoose.model('Ldb22q', Ldb22qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb22q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb22q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb22q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb22q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
