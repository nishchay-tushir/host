...
const MetalDoubleLeafDoor2qSchema = new mongoose.Schema({
...
const MetalDoubleLeafDoor2q = mongoose.model('MetalDoubleLeafDoor2q', MetalDoubleLeafDoor2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await MetalDoubleLeafDoor2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new MetalDoubleLeafDoor2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MetalDoubleLeafDoor2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await MetalDoubleLeafDoor2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await MetalDoubleLeafDoor2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
