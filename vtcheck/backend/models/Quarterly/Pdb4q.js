...
const Pdb4qSchema = new mongoose.Schema({
...
const Pdb4q = mongoose.model('Pdb4q', Pdb4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
