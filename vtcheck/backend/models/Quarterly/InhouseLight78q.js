...
const InhouseLight78qSchema = new mongoose.Schema({
...
const InhouseLight78q = mongoose.model('InhouseLight78q', InhouseLight78qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight78q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight78q({
        ...req.body,
...
try {
    const records = await InhouseLight78q.find();
...
try {
    const updatedRecord = await InhouseLight78q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight78q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
