...
const DdcPanel2qSchema = new mongoose.Schema({
...
const DdcPanel2q = mongoose.model('DdcPanel2q', DdcPanel2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DdcPanel2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new DdcPanel2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DdcPanel2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DdcPanel2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DdcPanel2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
