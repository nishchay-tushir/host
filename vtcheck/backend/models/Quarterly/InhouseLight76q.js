...
const InhouseLight76qSchema = new mongoose.Schema({
...
const InhouseLight76q = mongoose.model('InhouseLight76q', InhouseLight76qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight76q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight76q({
        ...req.body,
...
try {
    const records = await InhouseLight76q.find();
...
try {
    const updatedRecord = await InhouseLight76q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight76q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
