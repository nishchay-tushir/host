...
const InhouseLight91qSchema = new mongoose.Schema({
...
const InhouseLight91q = mongoose.model('InhouseLight91q', InhouseLight91qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight91q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight91q({
        ...req.body,
...
try {
    const records = await InhouseLight91q.find();
...
try {
    const updatedRecord = await InhouseLight91q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight91q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
