...
const InhouseLight28qSchema = new mongoose.Schema({
...
const InhouseLight28q = mongoose.model('InhouseLight28q', InhouseLight28qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight28q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight28q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight28q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight28q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight28q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
