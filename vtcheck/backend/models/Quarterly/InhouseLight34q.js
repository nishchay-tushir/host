...
const InhouseLight34qSchema = new mongoose.Schema({
...
const InhouseLight34q = mongoose.model('InhouseLight34q', InhouseLight34qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight34q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight34q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight34q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight34q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight34q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
