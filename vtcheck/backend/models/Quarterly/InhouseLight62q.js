...
const InhouseLight62qSchema = new mongoose.Schema({
...
const InhouseLight62q = mongoose.model('InhouseLight62q', InhouseLight62qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight62q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight62q({
        ...req.body,
...
try {
    const records = await InhouseLight62q.find();
...
try {
    const updatedRecord = await InhouseLight62q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight62q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
