...
const MetalDoubleLeafDoor4qSchema = new mongoose.Schema({
...
const MetalDoubleLeafDoor4q = mongoose.model('MetalDoubleLeafDoor4q', MetalDoubleLeafDoor4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await MetalDoubleLeafDoor4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new MetalDoubleLeafDoor4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MetalDoubleLeafDoor4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await MetalDoubleLeafDoor4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await MetalDoubleLeafDoor4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
