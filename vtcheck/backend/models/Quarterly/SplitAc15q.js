...
const SplitAc15qSchema = new mongoose.Schema({
...
const SplitAc15q = mongoose.model('SplitAc15q', SplitAc15qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc15q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc15q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc15q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc15q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc15q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
