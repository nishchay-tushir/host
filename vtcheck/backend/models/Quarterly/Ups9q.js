...
const Ups9qSchema = new mongoose.Schema({
...
const Ups9q = mongoose.model('Ups9q', Ups9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
