...
const InhouseLight2qSchema = new mongoose.Schema({
...
const InhouseLight2q = mongoose.model('InhouseLight2q', InhouseLight2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
