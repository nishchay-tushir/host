...
const SplitAc10qSchema = new mongoose.Schema({
...
const SplitAc10q = mongoose.model('SplitAc10q', SplitAc10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc10q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
