...
const FireHydrant6qSchema = new mongoose.Schema({
...
const FireHydrant6q = mongoose.model('FireHydrant6q', FireHydrant6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
