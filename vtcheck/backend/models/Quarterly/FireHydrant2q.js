...
const FireHydrant2qSchema = new mongoose.Schema({
...
const FireHydrant2q = mongoose.model('FireHydrant2q', FireHydrant2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
