...
const BeamDetector6qSchema = new mongoose.Schema({
...
const BeamDetector6q = mongoose.model('BeamDetector6q', BeamDetector6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await BeamDetector6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new BeamDetector6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await BeamDetector6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await BeamDetector6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await BeamDetector6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
