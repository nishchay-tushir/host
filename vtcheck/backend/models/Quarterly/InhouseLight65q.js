...
const InhouseLight65qSchema = new mongoose.Schema({
...
const InhouseLight65q = mongoose.model('InhouseLight65q', InhouseLight65qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight65q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight65q({
        ...req.body,
...
try {
    const records = await InhouseLight65q.find();
...
try {
    const updatedRecord = await InhouseLight65q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight65q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
