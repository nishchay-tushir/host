...
const StpRingBlower1qSchema = new mongoose.Schema({
...
const StpRingBlower1q = mongoose.model('StpRingBlower1q', StpRingBlower1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StpRingBlower1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StpRingBlower1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpRingBlower1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StpRingBlower1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StpRingBlower1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
