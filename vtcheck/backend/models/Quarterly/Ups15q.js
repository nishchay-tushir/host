...
const Ups15qSchema = new mongoose.Schema({
...
const Ups15q = mongoose.model('Ups15q', Ups15qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups15q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups15q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups15q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups15q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
