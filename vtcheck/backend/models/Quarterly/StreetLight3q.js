...
const StreetLight3qSchema = new mongoose.Schema({
...
const StreetLight3q = mongoose.model('StreetLight3q', StreetLight3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StreetLight3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StreetLight3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StreetLight3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StreetLight3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StreetLight3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
