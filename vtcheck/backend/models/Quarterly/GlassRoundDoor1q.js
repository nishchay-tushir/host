...
const GlassRoundDoor1qSchema = new mongoose.Schema({
...
const GlassRoundDoor1q = mongoose.model('GlassRoundDoor1q', GlassRoundDoor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassRoundDoor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassRoundDoor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassRoundDoor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassRoundDoor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassRoundDoor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
