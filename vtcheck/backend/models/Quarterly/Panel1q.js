...
const Panel1qSchema = new mongoose.Schema({
...
const Panel1q = mongoose.model('Panel1q', Panel1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
