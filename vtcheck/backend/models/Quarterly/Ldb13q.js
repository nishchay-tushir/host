...
const Ldb13qSchema = new mongoose.Schema({
...
const Ldb13q = mongoose.model('Ldb13q', Ldb13qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb13q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb13q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb13q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb13q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
