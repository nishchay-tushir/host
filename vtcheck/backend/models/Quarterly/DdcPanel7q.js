...
const DdcPanel7qSchema = new mongoose.Schema({
...
const DdcPanel7q = mongoose.model('DdcPanel7q', DdcPanel7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DdcPanel7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new DdcPanel7q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DdcPanel7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DdcPanel7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DdcPanel7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
