...
const InhouseLight43qSchema = new mongoose.Schema({
...
const InhouseLight43q = mongoose.model('InhouseLight43q', InhouseLight43qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight43q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight43q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight43q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight43q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight43q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
