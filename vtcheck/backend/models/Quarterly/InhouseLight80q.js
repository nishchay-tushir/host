...
const InhouseLight80qSchema = new mongoose.Schema({
...
const InhouseLight80q = mongoose.model('InhouseLight80q', InhouseLight80qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight80q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight80q({
        ...req.body,
...
try {
    const records = await InhouseLight80q.find();
...
try {
    const updatedRecord = await InhouseLight80q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight80q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
