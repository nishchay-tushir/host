...
const InhouseLight23qSchema = new mongoose.Schema({
...
const InhouseLight23q = mongoose.model('InhouseLight23q', InhouseLight23qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight23q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight23q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight23q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight23q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight23q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
