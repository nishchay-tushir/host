...
const FrpDoor5qSchema = new mongoose.Schema({
...
const FrpDoor5q = mongoose.model('FrpDoor5q', FrpDoor5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
