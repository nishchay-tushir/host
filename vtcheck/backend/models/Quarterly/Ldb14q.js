...
const Ldb14qSchema = new mongoose.Schema({
...
const Ldb14q = mongoose.model('Ldb14q', Ldb14qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb14q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb14q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb14q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb14q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
