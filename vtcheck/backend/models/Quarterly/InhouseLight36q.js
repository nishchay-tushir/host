...
const InhouseLight36qSchema = new mongoose.Schema({
...
const InhouseLight36q = mongoose.model('InhouseLight36q', InhouseLight36qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight36q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight36q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight36q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight36q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight36q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
