...
const CassetteAc25qSchema = new mongoose.Schema({
...
const CassetteAc25q = mongoose.model('CassetteAc25q', CassetteAc25qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc25q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc25q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc25q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc25q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc25q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
