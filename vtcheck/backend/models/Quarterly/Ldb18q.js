...
const Ldb18qSchema = new mongoose.Schema({
...
const Ldb18q = mongoose.model('Ldb18q', Ldb18qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb18q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb18q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb18q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb18q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
