...
const SmokeDetector5qSchema = new mongoose.Schema({
...
const SmokeDetector5q = mongoose.model('SmokeDetector5q', SmokeDetector5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SmokeDetector5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SmokeDetector5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SmokeDetector5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SmokeDetector5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SmokeDetector5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
