...
const DdcPanel1qSchema = new mongoose.Schema({
...
const DdcPanel1q = mongoose.model('DdcPanel1q', DdcPanel1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DdcPanel1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new DdcPanel1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DdcPanel1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DdcPanel1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DdcPanel1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
