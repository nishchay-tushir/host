...
const HooterStrobe2qSchema = new mongoose.Schema({
...
const HooterStrobe2q = mongoose.model('HooterStrobe2q', HooterStrobe2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await HooterStrobe2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new HooterStrobe2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HooterStrobe2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HooterStrobe2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HooterStrobe2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
