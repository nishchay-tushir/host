...
const Eldb11qSchema = new mongoose.Schema({
...
const Eldb11q = mongoose.model('Eldb11q', Eldb11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
