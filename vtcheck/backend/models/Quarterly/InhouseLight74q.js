...
const InhouseLight74qSchema = new mongoose.Schema({
...
const InhouseLight74q = mongoose.model('InhouseLight74q', InhouseLight74qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight74q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight74q({
        ...req.body,
...
try {
    const records = await InhouseLight74q.find();
...
try {
    const updatedRecord = await InhouseLight74q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight74q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
