...
const Pdb6qSchema = new mongoose.Schema({
...
const Pdb6q = mongoose.model('Pdb6q', Pdb6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
