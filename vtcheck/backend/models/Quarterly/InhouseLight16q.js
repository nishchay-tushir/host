...
const InhouseLight16qSchema = new mongoose.Schema({
...
const InhouseLight16q = mongoose.model('InhouseLight16q', InhouseLight16qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight16q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight16q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight16q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight16q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight16q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
