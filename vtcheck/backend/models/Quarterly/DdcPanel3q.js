...
const DdcPanel3qSchema = new mongoose.Schema({
...
const DdcPanel3q = mongoose.model('DdcPanel3q', DdcPanel3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DdcPanel3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new DdcPanel3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DdcPanel3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DdcPanel3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DdcPanel3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
