...
const GlassSingleLeafDoor1qSchema = new mongoose.Schema({
...
const GlassSingleLeafDoor1q = mongoose.model('GlassSingleLeafDoor1q', GlassSingleLeafDoor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassSingleLeafDoor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassSingleLeafDoor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassSingleLeafDoor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassSingleLeafDoor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassSingleLeafDoor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
