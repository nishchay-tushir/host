...
const Eldb3qSchema = new mongoose.Schema({
...
const Eldb3q = mongoose.model('Eldb3q', Eldb3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
