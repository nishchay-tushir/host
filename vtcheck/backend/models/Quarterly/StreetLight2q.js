...
const StreetLight2qSchema = new mongoose.Schema({
...
const StreetLight2q = mongoose.model('StreetLight2q', StreetLight2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StreetLight2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StreetLight2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StreetLight2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StreetLight2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StreetLight2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
