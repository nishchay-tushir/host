...
const FireHydrantSystem1qSchema = new mongoose.Schema({
...
const FireHydrantSystem1q = mongoose.model('FireHydrantSystem1q', FireHydrantSystem1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrantSystem1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrantSystem1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrantSystem1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrantSystem1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrantSystem1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
