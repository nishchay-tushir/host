...
const InhouseLight98qSchema = new mongoose.Schema({
...
const InhouseLight98q = mongoose.model('InhouseLight98q', InhouseLight98qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight98q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight98q({
        ...req.body,
...
try {
    const records = await InhouseLight98q.find();
...
try {
    const updatedRecord = await InhouseLight98q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight98q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
