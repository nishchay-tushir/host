    ...
    const InhouseLight44qSchema = new mongoose.Schema({
    ...
    const InhouseLight44q = mongoose.model('InhouseLight44q', InhouseLight44qSchema);
    ...
    try {
        ...
        const existingRecord = await InhouseLight44q.findOne({
            submissionDate: {
                $gte: startOfQuarter,
        ...
        const newRecord = new InhouseLight44q({
            ...req.body,
    ...
    try {
        const records = await InhouseLight44q.find();
    ...
    try {
        const updatedRecord = await InhouseLight44q.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (updatedRecord) {
    ...
    try {
        const deletedRecord = await InhouseLight44q.findByIdAndDelete(req.params.id);
        if (deletedRecord) {
