...
const FireHydrant3qSchema = new mongoose.Schema({
...
const FireHydrant3q = mongoose.model('FireHydrant3q', FireHydrant3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
