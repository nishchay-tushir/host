...
const WaterChiller4qSchema = new mongoose.Schema({
...
const WaterChiller4q = mongoose.model('WaterChiller4q', WaterChiller4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WaterChiller4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WaterChiller4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WaterChiller4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WaterChiller4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
