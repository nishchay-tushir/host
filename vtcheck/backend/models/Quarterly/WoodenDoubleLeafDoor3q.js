...
const WoodenDoubleLeafDoor3qSchema = new mongoose.Schema({
...
const WoodenDoubleLeafDoor3q = mongoose.model('WoodenDoubleLeafDoor3q', WoodenDoubleLeafDoor3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WoodenDoubleLeafDoor3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WoodenDoubleLeafDoor3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenDoubleLeafDoor3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WoodenDoubleLeafDoor3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WoodenDoubleLeafDoor3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
