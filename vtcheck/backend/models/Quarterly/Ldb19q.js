...
const Ldb19qSchema = new mongoose.Schema({
...
const Ldb19q = mongoose.model('Ldb19q', Ldb19qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb19q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb19q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb19q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb19q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
