...
const FrpDoor10qSchema = new mongoose.Schema({
...
const FrpDoor10q = mongoose.model('FrpDoor10q', FrpDoor10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FrpDoor10q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
