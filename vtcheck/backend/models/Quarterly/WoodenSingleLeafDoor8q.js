...
const WoodenSingleLeafDoor8qSchema = new mongoose.Schema({
...
const WoodenSingleLeafDoor8q = mongoose.model('WoodenSingleLeafDoor8q', WoodenSingleLeafDoor8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WoodenSingleLeafDoor8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WoodenSingleLeafDoor8q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenSingleLeafDoor8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WoodenSingleLeafDoor8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WoodenSingleLeafDoor8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
