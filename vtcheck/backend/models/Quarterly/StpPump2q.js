...
const StpPump2qSchema = new mongoose.Schema({
...
const StpPump2q = mongoose.model('StpPump2q', StpPump2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StpPump2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpPump2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StpPump2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StpPump2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
