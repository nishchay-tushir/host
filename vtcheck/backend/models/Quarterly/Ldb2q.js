...
const Ldb2qSchema = new mongoose.Schema({
...
const Ldb2q = mongoose.model('Ldb2q', Ldb2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
