...
const BeamDetector4qSchema = new mongoose.Schema({
...
const BeamDetector4q = mongoose.model('BeamDetector4q', BeamDetector4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await BeamDetector4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new BeamDetector4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await BeamDetector4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await BeamDetector4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await BeamDetector4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
