...
const Ldb15qSchema = new mongoose.Schema({
...
const Ldb15q = mongoose.model('Ldb15q', Ldb15qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb15q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb15q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb15q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb15q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
