...
const StackerManual1qSchema = new mongoose.Schema({
...
const StackerManual1q = mongoose.model('StackerManual1q', StackerManual1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StackerManual1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StackerManual1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StackerManual1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StackerManual1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StackerManual1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
