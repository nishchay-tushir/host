...
const InhouseLight57qSchema = new mongoose.Schema({
...
const InhouseLight57q = mongoose.model('InhouseLight57q', InhouseLight57qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight57q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight57q({
        ...req.body,
...
try {
    const records = await InhouseLight57q.find();
...
try {
    const updatedRecord = await InhouseLight57q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight57q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
