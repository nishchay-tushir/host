...
const RollingShutter4qSchema = new mongoose.Schema({
...
const RollingShutter4q = mongoose.model('RollingShutter4q', RollingShutter4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
