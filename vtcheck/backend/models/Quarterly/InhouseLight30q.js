...
const InhouseLight30qSchema = new mongoose.Schema({
...
const InhouseLight30q = mongoose.model('InhouseLight30q', InhouseLight30qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight30q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight30q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight30q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight30q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight30q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
