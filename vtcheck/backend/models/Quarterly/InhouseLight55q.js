...
const InhouseLight55qSchema = new mongoose.Schema({
...
const InhouseLight55q = mongoose.model('InhouseLight55q', InhouseLight55qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight55q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight55q({
        ...req.body,
...
try {
    const records = await InhouseLight55q.find();
...
try {
    const updatedRecord = await InhouseLight55q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight55q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
