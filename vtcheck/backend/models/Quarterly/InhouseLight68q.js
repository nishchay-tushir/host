...
const InhouseLight68qSchema = new mongoose.Schema({
...
const InhouseLight68q = mongoose.model('InhouseLight68q', InhouseLight68qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight68q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight68q({
        ...req.body,
...
try {
    const records = await InhouseLight68q.find();
...
try {
    const updatedRecord = await InhouseLight68q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight68q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
