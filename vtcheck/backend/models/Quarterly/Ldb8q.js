...
const Ldb8qSchema = new mongoose.Schema({
...
const Ldb8q = mongoose.model('Ldb8q', Ldb8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
