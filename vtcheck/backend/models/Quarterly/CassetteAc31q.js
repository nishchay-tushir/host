...
const CassetteAc31qSchema = new mongoose.Schema({
...
const CassetteAc31q = mongoose.model('CassetteAc31q', CassetteAc31qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc31q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc31q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc31q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc31q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc31q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
