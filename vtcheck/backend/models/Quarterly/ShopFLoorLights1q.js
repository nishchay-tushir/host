...
const ShopFloorLights1qSchema = new mongoose.Schema({
...
const ShopFloorLights1q = mongoose.model('ShopFloorLights1q', ShopFloorLights1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await ShopFloorLights1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new ShopFloorLights1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await ShopFloorLights1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ShopFloorLights1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ShopFloorLights1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
