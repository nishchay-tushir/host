...
const Panel43qSchema = new mongoose.Schema({
...
const Panel43q = mongoose.model('Panel43q', Panel43qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel43q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel43q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel43q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel43q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
