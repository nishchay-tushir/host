...
const InhouseLight14qSchema = new mongoose.Schema({
...
const InhouseLight14q = mongoose.model('InhouseLight14q', InhouseLight14qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight14q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight14q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight14q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight14q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight14q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
