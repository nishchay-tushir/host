...
const CassetteAc15qSchema = new mongoose.Schema({
...
const CassetteAc15q = mongoose.model('CassetteAc15q', CassetteAc15qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc15q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc15q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc15q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc15q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc15q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
