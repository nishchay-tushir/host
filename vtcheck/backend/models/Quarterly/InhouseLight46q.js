...
const InhouseLight46qSchema = new mongoose.Schema({
...
const InhouseLight46q = mongoose.model('InhouseLight46q', InhouseLight46qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight46q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight46q({
        ...req.body,
...
try {
    const records = await InhouseLight46q.find();
...
try {
    const updatedRecord = await InhouseLight46q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight46q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
