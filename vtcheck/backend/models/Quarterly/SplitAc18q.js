...
const SplitAc18qSchema = new mongoose.Schema({
...
const SplitAc18q = mongoose.model('SplitAc18q', SplitAc18qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc18q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc18q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc18q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc18q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc18q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
