...
const Cassette1qSchema = new mongoose.Schema({
...
const Cassette1q = mongoose.model('Cassette1q', Cassette1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Cassette1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new Cassette1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Cassette1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Cassette1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Cassette1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
