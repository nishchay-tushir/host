...
const FrpDoor2qSchema = new mongoose.Schema({
...
const FrpDoor2q = mongoose.model('FrpDoor2q', FrpDoor2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
