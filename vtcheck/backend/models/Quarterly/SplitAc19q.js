...
const SplitAc19qSchema = new mongoose.Schema({
...
const SplitAc19q = mongoose.model('SplitAc19q', SplitAc19qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc19q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc19q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc19q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc19q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc19q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
