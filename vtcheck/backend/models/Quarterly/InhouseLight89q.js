...
const InhouseLight89qSchema = new mongoose.Schema({
...
const InhouseLight89q = mongoose.model('InhouseLight89q', InhouseLight89qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight89q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight89q({
        ...req.body,
...
try {
    const records = await InhouseLight89q.find();
...
try {
    const updatedRecord = await InhouseLight89q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight89q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
