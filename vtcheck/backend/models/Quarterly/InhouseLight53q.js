...
const InhouseLight53qSchema = new mongoose.Schema({
...
const InhouseLight53q = mongoose.model('InhouseLight53q', InhouseLight53qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight53q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight53q({
        ...req.body,
...
try {
    const records = await InhouseLight53q.find();
...
try {
    const updatedRecord = await InhouseLight53q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight53q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
