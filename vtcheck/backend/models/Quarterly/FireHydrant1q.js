...
const FireHydrant1qSchema = new mongoose.Schema({
...
const FireHydrant1q = mongoose.model('FireHydrant1q', FireHydrant1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
