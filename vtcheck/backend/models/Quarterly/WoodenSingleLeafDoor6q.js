...
const WoodenSingleLeafDoor6qSchema = new mongoose.Schema({
...
const WoodenSingleLeafDoor6q = mongoose.model('WoodenSingleLeafDoor6q', WoodenSingleLeafDoor6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WoodenSingleLeafDoor6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WoodenSingleLeafDoor6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenSingleLeafDoor6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WoodenSingleLeafDoor6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WoodenSingleLeafDoor6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
