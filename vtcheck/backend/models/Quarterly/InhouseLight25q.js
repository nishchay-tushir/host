...
const InhouseLight25qSchema = new mongoose.Schema({
...
const InhouseLight25q = mongoose.model('InhouseLight25q', InhouseLight25qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight25q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight25q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight25q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight25q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight25q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
