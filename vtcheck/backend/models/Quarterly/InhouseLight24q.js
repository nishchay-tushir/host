...
const InhouseLight24qSchema = new mongoose.Schema({
...
const InhouseLight24q = mongoose.model('InhouseLight24q', InhouseLight24qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight24q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight24q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight24q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight24q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight24q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
