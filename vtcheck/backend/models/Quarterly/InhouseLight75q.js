...
const InhouseLight75qSchema = new mongoose.Schema({
...
const InhouseLight75q = mongoose.model('InhouseLight75q', InhouseLight75qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight75q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight75q({
        ...req.body,
...
try {
    const records = await InhouseLight75q.find();
...
try {
    const updatedRecord = await InhouseLight75q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight75q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
