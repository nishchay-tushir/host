...
const Ldb9qSchema = new mongoose.Schema({
...
const Ldb9q = mongoose.model('Ldb9q', Ldb9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
