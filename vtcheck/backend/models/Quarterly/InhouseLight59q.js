...
const InhouseLight59qSchema = new mongoose.Schema({
...
const InhouseLight59q = mongoose.model('InhouseLight59q', InhouseLight59qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight59q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight59q({
        ...req.body,
...
try {
    const records = await InhouseLight59q.find();
...
try {
    const updatedRecord = await InhouseLight59q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight59q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
