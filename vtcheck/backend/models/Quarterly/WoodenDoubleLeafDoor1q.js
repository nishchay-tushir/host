...
const WoodenDoubleLeafDoor1qSchema = new mongoose.Schema({
...
const WoodenDoubleLeafDoor1q = mongoose.model('WoodenDoubleLeafDoor1q', WoodenDoubleLeafDoor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WoodenDoubleLeafDoor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WoodenDoubleLeafDoor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenDoubleLeafDoor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WoodenDoubleLeafDoor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WoodenDoubleLeafDoor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
