...
const InhouseLight92qSchema = new mongoose.Schema({
...
const InhouseLight92q = mongoose.model('InhouseLight92q', InhouseLight92qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight92q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight92q({
        ...req.body,
...
try {
    const records = await InhouseLight92q.find();
...
try {
    const updatedRecord = await InhouseLight92q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight92q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
