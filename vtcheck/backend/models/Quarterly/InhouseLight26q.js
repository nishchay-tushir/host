...
const InhouseLight26qSchema = new mongoose.Schema({
...
const InhouseLight26q = mongoose.model('InhouseLight26q', InhouseLight26qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight26q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight26q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight26q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight26q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight26q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
