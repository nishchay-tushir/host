...
const CassetteAc17qSchema = new mongoose.Schema({
...
const CassetteAc17q = mongoose.model('CassetteAc17q', CassetteAc17qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc17q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc17q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc17q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc17q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc17q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
