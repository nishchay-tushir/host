...
const Panel42qSchema = new mongoose.Schema({
...
const Panel42q = mongoose.model('Panel42q', Panel42qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel42q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel42q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel42q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel42q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
