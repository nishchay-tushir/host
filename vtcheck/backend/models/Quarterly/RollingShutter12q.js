...
const RollingShutter12qSchema = new mongoose.Schema({
...
const RollingShutter12q = mongoose.model('RollingShutter12q', RollingShutter12qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter12q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter12q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter12q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter12q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter12q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
