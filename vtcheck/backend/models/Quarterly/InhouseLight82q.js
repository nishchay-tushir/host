...
const InhouseLight82qSchema = new mongoose.Schema({
...
const InhouseLight82q = mongoose.model('InhouseLight82q', InhouseLight82qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight82q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight82q({
        ...req.body,
...
try {
    const records = await InhouseLight82q.find();
...
try {
    const updatedRecord = await InhouseLight82q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight82q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
