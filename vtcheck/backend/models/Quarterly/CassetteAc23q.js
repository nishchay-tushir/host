...
const CassetteAc23qSchema = new mongoose.Schema({
...
const CassetteAc23q = mongoose.model('CassetteAc23q', CassetteAc23qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc23q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc23q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc23q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc23q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc23q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
