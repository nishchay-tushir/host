...
const InhouseLight63qSchema = new mongoose.Schema({
...
const InhouseLight63q = mongoose.model('InhouseLight63q', InhouseLight63qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight63q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight63q({
        ...req.body,
...
try {
    const records = await InhouseLight63q.find();
...
try {
    const updatedRecord = await InhouseLight63q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight63q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
