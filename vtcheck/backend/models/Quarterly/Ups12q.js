...
const Ups12qSchema = new mongoose.Schema({
...
const Ups12q = mongoose.model('Ups12q', Ups12qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups12q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups12q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups12q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups12q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
