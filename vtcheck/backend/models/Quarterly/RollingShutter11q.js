...
const RollingShutter11qSchema = new mongoose.Schema({
...
const RollingShutter11q = mongoose.model('RollingShutter11q', RollingShutter11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter11q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
