...
const StackerManual2qSchema = new mongoose.Schema({
...
const StackerManual2q = mongoose.model('StackerManual2q', StackerManual2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StackerManual2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StackerManual2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StackerManual2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StackerManual2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StackerManual2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
