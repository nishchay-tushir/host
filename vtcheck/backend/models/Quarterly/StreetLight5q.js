...
const StreetLight5qSchema = new mongoose.Schema({
...
const StreetLight5q = mongoose.model('StreetLight5q', StreetLight5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StreetLight5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StreetLight5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StreetLight5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StreetLight5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StreetLight5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
