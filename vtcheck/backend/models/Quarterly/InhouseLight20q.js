...
const InhouseLight20qSchema = new mongoose.Schema({
...
const InhouseLight20q = mongoose.model('InhouseLight20q', InhouseLight20qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight20q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight20q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight20q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight20q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight20q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
