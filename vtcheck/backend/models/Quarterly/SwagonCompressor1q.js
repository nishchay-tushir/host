...
const SwagonCompressor1qSchema = new mongoose.Schema({
...
const SwagonCompressor1q = mongoose.model('SwagonCompressor1q', SwagonCompressor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SwagonCompressor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SwagonCompressor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SwagonCompressor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SwagonCompressor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SwagonCompressor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
