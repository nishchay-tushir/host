...
const InhouseLight12qSchema = new mongoose.Schema({
...
const InhouseLight12q = mongoose.model('InhouseLight12q', InhouseLight12qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight12q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight12q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight12q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight12q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight12q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
