...
const FrpDoor4qSchema = new mongoose.Schema({
...
const FrpDoor4q = mongoose.model('FrpDoor4q', FrpDoor4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
