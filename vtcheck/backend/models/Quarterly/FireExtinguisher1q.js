...
const FireExtinguisher1qSchema = new mongoose.Schema({
...
const FireExtinguisher1q = mongoose.model('FireExtinguisher1q', FireExtinguisher1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireExtinguisher1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireExtinguisher1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireExtinguisher1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireExtinguisher1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
