...
const RollingShutter8qSchema = new mongoose.Schema({
...
const RollingShutter8q = mongoose.model('RollingShutter8q', RollingShutter8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter8q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
