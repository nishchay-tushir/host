...
const ManualCallPt1qSchema = new mongoose.Schema({
...
const ManualCallPt1q = mongoose.model('ManualCallPt1q', ManualCallPt1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await ManualCallPt1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new ManualCallPt1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await ManualCallPt1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ManualCallPt1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ManualCallPt1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
