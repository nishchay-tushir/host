...
const InhouseLight8qSchema = new mongoose.Schema({
...
const InhouseLight8q = mongoose.model('InhouseLight8q', InhouseLight8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight8q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
