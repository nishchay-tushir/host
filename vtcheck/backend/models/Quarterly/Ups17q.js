...
const Ups17qSchema = new mongoose.Schema({
...
const Ups17q = mongoose.model('Ups17q', Ups17qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups17q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups17q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups17q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups17q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
