...
const ManualCallPt5qSchema = new mongoose.Schema({
...
const ManualCallPt5q = mongoose.model('ManualCallPt5q', ManualCallPt5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await ManualCallPt5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new ManualCallPt5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await ManualCallPt5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ManualCallPt5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ManualCallPt5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
