...
const Ups10qSchema = new mongoose.Schema({
...
const Ups10q = mongoose.model('Ups10q', Ups10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
