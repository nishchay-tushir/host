...
const BeamDetector2qSchema = new mongoose.Schema({
...
const BeamDetector2q = mongoose.model('BeamDetector2q', BeamDetector2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await BeamDetector2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new BeamDetector2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await BeamDetector2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await BeamDetector2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await BeamDetector2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
