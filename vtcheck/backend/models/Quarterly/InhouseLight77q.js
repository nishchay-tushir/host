...
const InhouseLight77qSchema = new mongoose.Schema({
...
const InhouseLight77q = mongoose.model('InhouseLight77q', InhouseLight77qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight77q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight77q({
        ...req.body,
...
try {
    const records = await InhouseLight77q.find();
...
try {
    const updatedRecord = await InhouseLight77q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight77q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
