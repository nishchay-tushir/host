...
const CassetteAc24qSchema = new mongoose.Schema({
...
const CassetteAc24q = mongoose.model('CassetteAc24q', CassetteAc24qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc24q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc24q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc24q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc24q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc24q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
