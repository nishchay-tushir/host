...
const Ldb12qSchema = new mongoose.Schema({
...
const Ldb12q = mongoose.model('Ldb12q', Ldb12qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb12q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb12q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb12q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb12q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
