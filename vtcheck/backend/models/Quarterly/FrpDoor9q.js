...
const FrpDoor9qSchema = new mongoose.Schema({
...
const FrpDoor9q = mongoose.model('FrpDoor9q', FrpDoor9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
