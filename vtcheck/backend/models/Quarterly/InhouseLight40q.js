...
const InhouseLight40qSchema = new mongoose.Schema({
...
const InhouseLight40q = mongoose.model('InhouseLight40q', InhouseLight40qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight40q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight40q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight40q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight40q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight40q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
