...
const Eldb7qSchema = new mongoose.Schema({
...
const Eldb7q = mongoose.model('Eldb7q', Eldb7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
