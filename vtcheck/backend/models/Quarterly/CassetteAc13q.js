...
const CassetteAc13qSchema = new mongoose.Schema({
...
const CassetteAc13q = mongoose.model('CassetteAc13q', CassetteAc13qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc13q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc13q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc13q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc13q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc13q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
