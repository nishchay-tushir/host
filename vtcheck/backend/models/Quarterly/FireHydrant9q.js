...
const FireHydrant9qSchema = new mongoose.Schema({
...
const FireHydrant9q = mongoose.model('FireHydrant9q', FireHydrant9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant9q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
