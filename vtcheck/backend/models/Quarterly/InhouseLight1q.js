...
const InhouseLight1qSchema = new mongoose.Schema({
...
const InhouseLight1q = mongoose.model('InhouseLight1q', InhouseLight1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
