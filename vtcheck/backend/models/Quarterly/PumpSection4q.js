...
const PumpSection4qSchema = new mongoose.Schema({
...
const PumpSection4q = mongoose.model('PumpSection4q', PumpSection4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await PumpSection4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new PumpSection4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await PumpSection4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await PumpSection4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await PumpSection4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
