...
const CassetteAc6qSchema = new mongoose.Schema({
...
const CassetteAc6q = mongoose.model('CassetteAc6q', CassetteAc6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
