...
const InhouseLight4qSchema = new mongoose.Schema({
...
const InhouseLight4q = mongoose.model('InhouseLight4q', InhouseLight4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
