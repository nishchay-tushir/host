...
const Ldb1qSchema = new mongoose.Schema({
...
const Ldb1q = mongoose.model('Ldb1q', Ldb1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
