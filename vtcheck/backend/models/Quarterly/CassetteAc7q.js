...
const CassetteAc7qSchema = new mongoose.Schema({
...
const CassetteAc7q = mongoose.model('CassetteAc7q', CassetteAc7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc7q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
