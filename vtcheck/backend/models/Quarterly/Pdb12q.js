...
const Pdb12qSchema = new mongoose.Schema({
...
const Pdb12q = mongoose.model('Pdb12q', Pdb12qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb12q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb12q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb12q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb12q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
