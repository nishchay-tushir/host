...
const Sprinkler3qSchema = new mongoose.Schema({
...
const Sprinkler3q = mongoose.model('Sprinkler3q', Sprinkler3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Sprinkler3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new Sprinkler3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Sprinkler3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Sprinkler3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Sprinkler3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
