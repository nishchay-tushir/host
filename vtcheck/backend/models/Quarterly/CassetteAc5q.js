...
const CassetteAc5qSchema = new mongoose.Schema({
...
const CassetteAc5q = mongoose.model('CassetteAc5q', CassetteAc5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
