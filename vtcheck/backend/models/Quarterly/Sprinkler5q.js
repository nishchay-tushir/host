...
const Sprinkler5qSchema = new mongoose.Schema({
...
const Sprinkler5q = mongoose.model('Sprinkler5q', Sprinkler5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Sprinkler5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new Sprinkler5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Sprinkler5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Sprinkler5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Sprinkler5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
