...
const GlassDoubleLeafDoor5qSchema = new mongoose.Schema({
...
const GlassDoubleLeafDoor5q = mongoose.model('GlassDoubleLeafDoor5q', GlassDoubleLeafDoor5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassDoubleLeafDoor5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassDoubleLeafDoor5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassDoubleLeafDoor5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassDoubleLeafDoor5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassDoubleLeafDoor5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
