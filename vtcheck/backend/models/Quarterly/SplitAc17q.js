...
const SplitAc17qSchema = new mongoose.Schema({
...
const SplitAc17q = mongoose.model('SplitAc17q', SplitAc17qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc17q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc17q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc17q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc17q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc17q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
