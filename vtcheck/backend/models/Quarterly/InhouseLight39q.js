...
const InhouseLight39qSchema = new mongoose.Schema({
...
const InhouseLight39q = mongoose.model('InhouseLight39q', InhouseLight39qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight39q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight39q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight39q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight39q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight39q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
