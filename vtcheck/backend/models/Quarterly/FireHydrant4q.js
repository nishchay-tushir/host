...
const FireHydrant4qSchema = new mongoose.Schema({
...
const FireHydrant4q = mongoose.model('FireHydrant4q', FireHydrant4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
