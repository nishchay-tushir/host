...
const InhouseLight73qSchema = new mongoose.Schema({
...
const InhouseLight73q = mongoose.model('InhouseLight73q', InhouseLight73qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight73q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight73q({
        ...req.body,
...
try {
    const records = await InhouseLight73q.find();
...
try {
    const updatedRecord = await InhouseLight73q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight73q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
