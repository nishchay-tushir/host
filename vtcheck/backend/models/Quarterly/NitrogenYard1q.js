...
const NitrogenYard1qSchema = new mongoose.Schema({
...
const NitrogenYard1q = mongoose.model('NitrogenYard1q', NitrogenYard1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await NitrogenYard1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new NitrogenYard1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await NitrogenYard1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await NitrogenYard1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await NitrogenYard1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
