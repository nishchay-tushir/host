...
const Panel13qSchema = new mongoose.Schema({
...
const Panel13q = mongoose.model('Panel13q', Panel13qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel13q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel13q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel13q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel13q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
