...
const RollingShutter1qSchema = new mongoose.Schema({
...
const RollingShutter1q = mongoose.model('RollingShutter1q', RollingShutter1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
