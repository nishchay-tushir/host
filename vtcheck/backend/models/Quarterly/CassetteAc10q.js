...
const CassetteAc10qSchema = new mongoose.Schema({
...
const CassetteAc10q = mongoose.model('CassetteAc10q', CassetteAc10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc10q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
