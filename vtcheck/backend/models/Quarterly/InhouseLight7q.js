...
const InhouseLight7qSchema = new mongoose.Schema({
...
const InhouseLight7q = mongoose.model('InhouseLight7q', InhouseLight7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight7q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
