...
const Ldb4qSchema = new mongoose.Schema({
...
const Ldb4q = mongoose.model('Ldb4q', Ldb4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
