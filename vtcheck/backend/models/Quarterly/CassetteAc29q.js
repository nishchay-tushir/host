...
const CassetteAc29qSchema = new mongoose.Schema({
...
const CassetteAc29q = mongoose.model('CassetteAc29q', CassetteAc29qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc29q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc29q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc29q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc29q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc29q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
