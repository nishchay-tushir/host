...
const CassetteAc4qSchema = new mongoose.Schema({
...
const CassetteAc4q = mongoose.model('CassetteAc4q', CassetteAc4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
