...
const MetalSingleLeafDoor2qSchema = new mongoose.Schema({
...
const MetalSingleLeafDoor2q = mongoose.model('MetalSingleLeafDoor2q', MetalSingleLeafDoor2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await MetalSingleLeafDoor2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new MetalSingleLeafDoor2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MetalSingleLeafDoor2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await MetalSingleLeafDoor2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await MetalSingleLeafDoor2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
