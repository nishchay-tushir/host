...
const SplitAc9qSchema = new mongoose.Schema({
...
const SplitAc9q = mongoose.model('SplitAc9q', SplitAc9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
