...
const InhouseLight35qSchema = new mongoose.Schema({
...
const InhouseLight35q = mongoose.model('InhouseLight35q', InhouseLight35qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight35q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight35q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight35q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight35q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight35q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
