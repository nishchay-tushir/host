...
const GlassSingleLeafDoor2qSchema = new mongoose.Schema({
...
const GlassSingleLeafDoor2q = mongoose.model('GlassSingleLeafDoor2q', GlassSingleLeafDoor2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassSingleLeafDoor2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassSingleLeafDoor2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassSingleLeafDoor2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassSingleLeafDoor2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassSingleLeafDoor2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
