...
const WaterChiller2qSchema = new mongoose.Schema({
...
const WaterChiller2q = mongoose.model('WaterChiller2q', WaterChiller2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WaterChiller2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WaterChiller2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WaterChiller2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WaterChiller2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
