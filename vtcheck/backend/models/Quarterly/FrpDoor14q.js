...
const FrpDoor14qSchema = new mongoose.Schema({
...
const FrpDoor14q = mongoose.model('FrpDoor14q', FrpDoor14qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FrpDoor14q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FrpDoor14q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FrpDoor14q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FrpDoor14q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FrpDoor14q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
