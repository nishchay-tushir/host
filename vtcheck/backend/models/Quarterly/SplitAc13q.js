...
const SplitAc13qSchema = new mongoose.Schema({
...
const SplitAc13q = mongoose.model('SplitAc13q', SplitAc13qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc13q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc13q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc13q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc13q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc13q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
