...
const RollingShutter5qSchema = new mongoose.Schema({
...
const RollingShutter5q = mongoose.model('RollingShutter5q', RollingShutter5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
