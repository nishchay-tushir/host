...
const SplitAc14qSchema = new mongoose.Schema({
...
const SplitAc14q = mongoose.model('SplitAc14q', SplitAc14qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc14q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc14q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc14q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc14q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc14q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
