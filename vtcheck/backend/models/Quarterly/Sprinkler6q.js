...
const Sprinkler6qSchema = new mongoose.Schema({
...
const Sprinkler6q = mongoose.model('Sprinkler6q', Sprinkler6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Sprinkler6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new Sprinkler6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Sprinkler6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Sprinkler6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Sprinkler6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
