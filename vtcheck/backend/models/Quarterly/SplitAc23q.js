...
const SplitAc23qSchema = new mongoose.Schema({
...
const SplitAc23q = mongoose.model('SplitAc23q', SplitAc23qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc23q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc23q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc23q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc23q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc23q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
