...
const Panel39qSchema = new mongoose.Schema({
...
const Panel39q = mongoose.model('Panel39q', Panel39qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel39q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel39q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel39q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel39q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
