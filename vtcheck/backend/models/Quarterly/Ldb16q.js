...
const Ldb16qSchema = new mongoose.Schema({
...
const Ldb16q = mongoose.model('Ldb16q', Ldb16qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb16q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb16q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb16q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb16q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
