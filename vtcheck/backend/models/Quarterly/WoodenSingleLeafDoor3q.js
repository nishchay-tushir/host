...
const WoodenSingleLeafDoor3qSchema = new mongoose.Schema({
...
const WoodenSingleLeafDoor3q = mongoose.model('WoodenSingleLeafDoor3q', WoodenSingleLeafDoor3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WoodenSingleLeafDoor3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WoodenSingleLeafDoor3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenSingleLeafDoor3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WoodenSingleLeafDoor3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WoodenSingleLeafDoor3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
