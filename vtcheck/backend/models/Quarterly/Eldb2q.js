...
const Eldb2qSchema = new mongoose.Schema({
...
const Eldb2q = mongoose.model('Eldb2q', Eldb2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
