...
const RollingShutter7qSchema = new mongoose.Schema({
...
const RollingShutter7q = mongoose.model('RollingShutter7q', RollingShutter7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter7q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
