...
const FireHydrant5qSchema = new mongoose.Schema({
...
const FireHydrant5q = mongoose.model('FireHydrant5q', FireHydrant5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
