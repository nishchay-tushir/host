...
const CassetteAc22qSchema = new mongoose.Schema({
...
const CassetteAc22q = mongoose.model('CassetteAc22q', CassetteAc22qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc22q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc22q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc22q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc22q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc22q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
