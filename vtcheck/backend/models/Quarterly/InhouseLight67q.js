...
const InhouseLight67qSchema = new mongoose.Schema({
...
const InhouseLight67q = mongoose.model('InhouseLight67q', InhouseLight67qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight67q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight67q({
        ...req.body,
...
try {
    const records = await InhouseLight67q.find();
...
try {
    const updatedRecord = await InhouseLight67q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight67q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
