...
const SplitAc6qSchema = new mongoose.Schema({
...
const SplitAc6q = mongoose.model('SplitAc6q', SplitAc6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
