...
const Ldb20qSchema = new mongoose.Schema({
...
const Ldb20q = mongoose.model('Ldb20q', Ldb20qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb20q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb20q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb20q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb20q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
