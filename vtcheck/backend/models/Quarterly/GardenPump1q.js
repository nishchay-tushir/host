...
const GardenPump1qSchema = new mongoose.Schema({
...
const GardenPump1q = mongoose.model('GardenPump1q', GardenPump1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GardenPump1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GardenPump1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GardenPump1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GardenPump1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GardenPump1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
