...
const DG5001qSchema = new mongoose.Schema({
...
const DG5001q = mongoose.model('DG5001q', DG5001qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DG5001q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await DG5001q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG5001q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG5001q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
