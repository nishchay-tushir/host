...
const HooterStrobe3qSchema = new mongoose.Schema({
...
const HooterStrobe3q = mongoose.model('HooterStrobe3q', HooterStrobe3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await HooterStrobe3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new HooterStrobe3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HooterStrobe3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HooterStrobe3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HooterStrobe3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
