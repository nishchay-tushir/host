...
const Eldb8qSchema = new mongoose.Schema({
...
const Eldb8q = mongoose.model('Eldb8q', Eldb8qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb8q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb8q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb8q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb8q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
