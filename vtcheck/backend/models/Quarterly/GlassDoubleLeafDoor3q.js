...
const GlassDoubleLeafDoor3qSchema = new mongoose.Schema({
...
const GlassDoubleLeafDoor3q = mongoose.model('GlassDoubleLeafDoor3q', GlassDoubleLeafDoor3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassDoubleLeafDoor3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassDoubleLeafDoor3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassDoubleLeafDoor3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassDoubleLeafDoor3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassDoubleLeafDoor3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
