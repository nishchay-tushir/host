...
const AirDryer1qSchema = new mongoose.Schema({
...
const AirDryer1q = mongoose.model('AirDryer1q', AirDryer1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await AirDryer1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new AirDryer1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirDryer1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirDryer1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirDryer1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
