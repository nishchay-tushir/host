...
const Ups14qSchema = new mongoose.Schema({
...
const Ups14q = mongoose.model('Ups14q', Ups14qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups14q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups14q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups14q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups14q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
