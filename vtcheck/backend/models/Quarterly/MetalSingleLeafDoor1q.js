...
const MetalSingleLeafDoor1qSchema = new mongoose.Schema({
...
const MetalSingleLeafDoor1q = mongoose.model('MetalSingleLeafDoor1q', MetalSingleLeafDoor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await MetalSingleLeafDoor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new MetalSingleLeafDoor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MetalSingleLeafDoor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await MetalSingleLeafDoor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await MetalSingleLeafDoor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
