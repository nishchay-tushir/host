...
const InhouseLight79qSchema = new mongoose.Schema({
...
const InhouseLight79q = mongoose.model('InhouseLight79q', InhouseLight79qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight79q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight79q({
        ...req.body,
...
try {
    const records = await InhouseLight79q.find();
...
try {
    const updatedRecord = await InhouseLight79q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight79q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
