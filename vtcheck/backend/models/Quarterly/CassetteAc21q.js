...
const CassetteAc21qSchema = new mongoose.Schema({
...
const CassetteAc21q = mongoose.model('CassetteAc21q', CassetteAc21qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc21q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc21q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc21q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc21q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc21q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
