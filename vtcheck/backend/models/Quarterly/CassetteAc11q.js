...
const CassetteAc11qSchema = new mongoose.Schema({
...
const CassetteAc11q = mongoose.model('CassetteAc11q', CassetteAc11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc11q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
