...
const InhouseLight48qSchema = new mongoose.Schema({
...
const InhouseLight48q = mongoose.model('InhouseLight48q', InhouseLight48qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight48q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight48q({
        ...req.body,
...
try {
    const records = await InhouseLight48q.find();
...
try {
    const updatedRecord = await InhouseLight48q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight48q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
