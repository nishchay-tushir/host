...
const GlassSingleLeafDoor4qSchema = new mongoose.Schema({
...
const GlassSingleLeafDoor4q = mongoose.model('GlassSingleLeafDoor4q', GlassSingleLeafDoor4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await GlassSingleLeafDoor4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new GlassSingleLeafDoor4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassSingleLeafDoor4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await GlassSingleLeafDoor4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await GlassSingleLeafDoor4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
