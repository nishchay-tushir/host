...
const InhouseLight84qSchema = new mongoose.Schema({
...
const InhouseLight84q = mongoose.model('InhouseLight84q', InhouseLight84qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight84q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight84q({
        ...req.body,
...
try {
    const records = await InhouseLight84q.find();
...
try {
    const updatedRecord = await InhouseLight84q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight84q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
