...
const InhouseLight97qSchema = new mongoose.Schema({
...
const InhouseLight97q = mongoose.model('InhouseLight97q', InhouseLight97qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight97q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight97q({
        ...req.body,
...
try {
    const records = await InhouseLight97q.find();
...
try {
    const updatedRecord = await InhouseLight97q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight97q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
