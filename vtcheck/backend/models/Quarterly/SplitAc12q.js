...
const SplitAc12qSchema = new mongoose.Schema({
...
const SplitAc12q = mongoose.model('SplitAc12q', SplitAc12qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc12q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc12q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc12q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc12q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc12q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
