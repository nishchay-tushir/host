...
const Ldb3qSchema = new mongoose.Schema({
...
const Ldb3q = mongoose.model('Ldb3q', Ldb3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
