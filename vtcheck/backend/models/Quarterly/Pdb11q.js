...
const Pdb11qSchema = new mongoose.Schema({
...
const Pdb11q = mongoose.model('Pdb11q', Pdb11qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb11q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb11q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb11q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb11q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
