...
const InhouseLight13qSchema = new mongoose.Schema({
...
const InhouseLight13q = mongoose.model('InhouseLight13q', InhouseLight13qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight13q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight13q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight13q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight13q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight13q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
