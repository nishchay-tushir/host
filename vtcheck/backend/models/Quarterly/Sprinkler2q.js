...
const Sprinkler2qSchema = new mongoose.Schema({
...
const Sprinkler2q = mongoose.model('Sprinkler2q', Sprinkler2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Sprinkler2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new Sprinkler2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Sprinkler2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Sprinkler2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Sprinkler2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
