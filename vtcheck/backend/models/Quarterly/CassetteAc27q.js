...
const CassetteAc27qSchema = new mongoose.Schema({
...
const CassetteAc27q = mongoose.model('CassetteAc27q', CassetteAc27qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc27q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc27q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc27q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc27q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc27q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
