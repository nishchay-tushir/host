...
const FireHydrant7qSchema = new mongoose.Schema({
...
const FireHydrant7q = mongoose.model('FireHydrant7q', FireHydrant7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant7q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
