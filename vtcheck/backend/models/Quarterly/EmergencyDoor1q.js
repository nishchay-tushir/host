...
const EmergencyDoor1qSchema = new mongoose.Schema({
...
const EmergencyDoor1q = mongoose.model('EmergencyDoor1q', EmergencyDoor1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await EmergencyDoor1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new EmergencyDoor1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await EmergencyDoor1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await EmergencyDoor1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await EmergencyDoor1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
