...
const Eldb1qSchema = new mongoose.Schema({
...
const Eldb1q = mongoose.model('Eldb1q', Eldb1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
