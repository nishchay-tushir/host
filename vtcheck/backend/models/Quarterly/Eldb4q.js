...
const Eldb4qSchema = new mongoose.Schema({
...
const Eldb4q = mongoose.model('Eldb4q', Eldb4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Eldb4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Eldb4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Eldb4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Eldb4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
