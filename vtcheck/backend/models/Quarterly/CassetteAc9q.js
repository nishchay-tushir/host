...
const CassetteAc9qSchema = new mongoose.Schema({
...
const CassetteAc9q = mongoose.model('CassetteAc9q', CassetteAc9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc9q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
