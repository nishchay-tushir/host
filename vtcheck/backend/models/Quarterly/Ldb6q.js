...
const Ldb6qSchema = new mongoose.Schema({
...
const Ldb6q = mongoose.model('Ldb6q', Ldb6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
