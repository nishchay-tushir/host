...
const Panel21qSchema = new mongoose.Schema({
...
const Panel21q = mongoose.model('Panel21q', Panel21qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel21q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel21q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel21q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel21q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
