...
const InhouseLight88qSchema = new mongoose.Schema({
...
const InhouseLight88q = mongoose.model('InhouseLight88q', InhouseLight88qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight88q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight88q({
        ...req.body,
...
try {
    const records = await InhouseLight88q.find();
...
try {
    const updatedRecord = await InhouseLight88q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight88q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
