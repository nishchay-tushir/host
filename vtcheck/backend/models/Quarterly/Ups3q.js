...
const Ups3qSchema = new mongoose.Schema({
...
const Ups3q = mongoose.model('Ups3q', Ups3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
