...
const SplitAc21qSchema = new mongoose.Schema({
...
const SplitAc21q = mongoose.model('SplitAc21q', SplitAc21qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc21q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc21q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc21q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc21q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc21q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
