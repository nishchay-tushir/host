...
const Panel34qSchema = new mongoose.Schema({
...
const Panel34q = mongoose.model('Panel34q', Panel34qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Panel34q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel34q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Panel34q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Panel34q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
