...
const StpRingBlower3qSchema = new mongoose.Schema({
...
const StpRingBlower3q = mongoose.model('StpRingBlower3q', StpRingBlower3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StpRingBlower3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StpRingBlower3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpRingBlower3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StpRingBlower3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StpRingBlower3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
