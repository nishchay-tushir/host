...
const InhouseLight51qSchema = new mongoose.Schema({
...
const InhouseLight51q = mongoose.model('InhouseLight51q', InhouseLight51qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight51q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight51q({
        ...req.body,
...
try {
    const records = await InhouseLight51q.find();
...
try {
    const updatedRecord = await InhouseLight51q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight51q .findByIdAndDelete(req.params.id);
    if (deletedRecord) {
