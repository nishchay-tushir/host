...
const FireHydrant10qSchema = new mongoose.Schema({
...
const FireHydrant10q = mongoose.model('FireHydrant10q', FireHydrant10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await FireHydrant10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new FireHydrant10q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await FireHydrant10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await FireHydrant10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
