...
const InhouseLight22qSchema = new mongoose.Schema({
...
const InhouseLight22q = mongoose.model('InhouseLight22q', InhouseLight22qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight22q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight22q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight22q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight22q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight22q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
