...
const Ups2qSchema = new mongoose.Schema({
...
const Ups2q = mongoose.model('Ups2q', Ups2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
