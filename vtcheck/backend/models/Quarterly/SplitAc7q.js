...
const SplitAc7qSchema = new mongoose.Schema({
...
const SplitAc7q = mongoose.model('SplitAc7q', SplitAc7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
