...
const WaterChiller6qSchema = new mongoose.Schema({
...
const WaterChiller6q = mongoose.model('WaterChiller6q', WaterChiller6qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WaterChiller6q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WaterChiller6q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller6q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WaterChiller6q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WaterChiller6q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
