...
const InhouseLight83qSchema = new mongoose.Schema({
...
const InhouseLight83q = mongoose.model('InhouseLight83q', InhouseLight83qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight83q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight83q({
        ...req.body,
...
try {
    const records = await InhouseLight83q.find();
...
try {
    const updatedRecord = await InhouseLight83q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight83q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
