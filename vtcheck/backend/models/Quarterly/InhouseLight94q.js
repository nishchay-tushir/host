...
const InhouseLight94qSchema = new mongoose.Schema({
...
const InhouseLight94q = mongoose.model('InhouseLight94q', InhouseLight94qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight94q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight94q({
        ...req.body,
...
try {
    const records = await InhouseLight94q.find();
...
try {
    const updatedRecord = await InhouseLight94q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight94q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
