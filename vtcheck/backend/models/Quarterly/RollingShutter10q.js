...
const RollingShutter10qSchema = new mongoose.Schema({
...
const RollingShutter10q = mongoose.model('RollingShutter10q', RollingShutter10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RollingShutter10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RollingShutter10q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RollingShutter10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RollingShutter10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
