...
const PumpSection2qSchema = new mongoose.Schema({
...
const PumpSection2q = mongoose.model('PumpSection2q', PumpSection2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await PumpSection2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new PumpSection2q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await PumpSection2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await PumpSection2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await PumpSection2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
