...
const ManualCallPt4qSchema = new mongoose.Schema({
...
const ManualCallPt4q = mongoose.model('ManualCallPt4q', ManualCallPt4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await ManualCallPt4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new ManualCallPt4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await ManualCallPt4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ManualCallPt4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ManualCallPt4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
