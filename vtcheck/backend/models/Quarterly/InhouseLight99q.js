...
const InhouseLight99qSchema = new mongoose.Schema({
...
const InhouseLight99q = mongoose.model('InhouseLight99q', InhouseLight99qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight99q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight99q({
        ...req.body,
...
try {
    const records = await InhouseLight99q.find();
...
try {
    const updatedRecord = await InhouseLight99q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight99q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
