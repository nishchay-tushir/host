...
const Pdb13qSchema = new mongoose.Schema({
...
const Pdb13q = mongoose.model('Pdb13q', Pdb13qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb13q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb13q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb13q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb13q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
