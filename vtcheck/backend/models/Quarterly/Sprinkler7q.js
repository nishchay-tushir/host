...
const Sprinkler7qSchema = new mongoose.Schema({
...
const Sprinkler7q = mongoose.model('Sprinkler7q', Sprinkler7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Sprinkler7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new Sprinkler7q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Sprinkler7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Sprinkler7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Sprinkler7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
