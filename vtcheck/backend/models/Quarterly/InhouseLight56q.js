...
const InhouseLight56qSchema = new mongoose.Schema({
...
const InhouseLight56q = mongoose.model('InhouseLight56q', InhouseLight56qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight56q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight56q({
        ...req.body,
...
try {
    const records = await InhouseLight56q.find();
...
try {
    const updatedRecord = await InhouseLight56q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight56q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
