...
const CassetteAc14qSchema = new mongoose.Schema({
...
const CassetteAc14q = mongoose.model('CassetteAc14q', CassetteAc14qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc14q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc14q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc14q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc14q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc14q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
