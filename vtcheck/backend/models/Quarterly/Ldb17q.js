...
const Ldb17qSchema = new mongoose.Schema({
...
const Ldb17q = mongoose.model('Ldb17q', Ldb17qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ldb17q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ldb17q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ldb17q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ldb17q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
