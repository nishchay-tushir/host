...
const InhouseLight50qSchema = new mongoose.Schema({
...
const InhouseLight50q = mongoose.model('InhouseLight50q', InhouseLight50qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight50q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight50q({
        ...req.body,
...
try {
    const records = await InhouseLight50q.find();
...
try {
    const updatedRecord = await InhouseLight50q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight50q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
