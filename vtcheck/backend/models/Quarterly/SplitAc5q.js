...
const SplitAc5qSchema = new mongoose.Schema({
...
const SplitAc5q = mongoose.model('SplitAc5q', SplitAc5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
