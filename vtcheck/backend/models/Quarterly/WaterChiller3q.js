...
const WaterChiller3qSchema = new mongoose.Schema({
...
const WaterChiller3q = mongoose.model('WaterChiller3q', WaterChiller3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WaterChiller3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WaterChiller3q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WaterChiller3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WaterChiller3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
