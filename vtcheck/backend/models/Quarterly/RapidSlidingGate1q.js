...
const RapidSlidingGate1qSchema = new mongoose.Schema({
...
const RapidSlidingGate1q = mongoose.model('RapidSlidingGate1q', RapidSlidingGate1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await RapidSlidingGate1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new RapidSlidingGate1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidSlidingGate1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RapidSlidingGate1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RapidSlidingGate1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
