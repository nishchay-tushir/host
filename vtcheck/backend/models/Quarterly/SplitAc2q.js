...
const SplitAc2qSchema = new mongoose.Schema({
...
const SplitAc2q = mongoose.model('SplitAc2q', SplitAc2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
