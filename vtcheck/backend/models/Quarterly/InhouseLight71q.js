...
const InhouseLight71qSchema = new mongoose.Schema({
...
const InhouseLight71q = mongoose.model('InhouseLight71q', InhouseLight71qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight71q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight71q({
        ...req.body,
...
try {
    const records = await InhouseLight71q.find();
...
try {
    const updatedRecord = await InhouseLight71q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight71q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
