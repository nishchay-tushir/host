...
const StackerBattery5qSchema = new mongoose.Schema({
...
const StackerBattery5q = mongoose.model('StackerBattery5q', StackerBattery5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await StackerBattery5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new StackerBattery5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StackerBattery5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StackerBattery5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StackerBattery5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
