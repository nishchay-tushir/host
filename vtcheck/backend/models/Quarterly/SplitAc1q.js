...
const SplitAC1qSchema = new mongoose.Schema({
...
const SplitAC1q = mongoose.model('SplitAC1q', SplitAC1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAC1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAC1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAC1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAC1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
