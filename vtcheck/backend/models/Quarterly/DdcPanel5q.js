...
const DdcPanel5qSchema = new mongoose.Schema({
...
const DdcPanel5q = mongoose.model('DdcPanel5q', DdcPanel5qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DdcPanel5q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new DdcPanel5q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DdcPanel5q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DdcPanel5q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DdcPanel5q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
