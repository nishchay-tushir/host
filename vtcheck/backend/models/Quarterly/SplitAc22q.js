...
const SplitAc22qSchema = new mongoose.Schema({
...
const SplitAc22q = mongoose.model('SplitAc22q', SplitAc22qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc22q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new SplitAc22q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc22q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc22q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc22q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
