...
const InhouseLight9qSchema = new mongoose.Schema({
...
const InhouseLight9q = mongoose.model('InhouseLight9q', InhouseLight9qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight9q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight9q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight9q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight9q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight9q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
