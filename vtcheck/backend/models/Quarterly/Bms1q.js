...
const Bms1qSchema = new mongoose.Schema({
...
const Bms1q = mongoose.model('Bms1q', Bms1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Bms1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Bms1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Bms1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Bms1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
