...
const Pdb2qSchema = new mongoose.Schema({
...
const Pdb2q = mongoose.model('Pdb2q', Pdb2qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb2q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb2q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb2q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb2q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
