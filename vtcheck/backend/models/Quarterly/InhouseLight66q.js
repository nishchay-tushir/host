...
const InhouseLight66qSchema = new mongoose.Schema({
...
const InhouseLight66q = mongoose.model('InhouseLight66q', InhouseLight66qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight66q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight66q({
        ...req.body,
...
try {
    const records = await InhouseLight66q.find();
...
try {
    const updatedRecord = await InhouseLight66q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight66q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
