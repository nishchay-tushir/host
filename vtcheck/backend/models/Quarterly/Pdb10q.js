...
const Pdb10qSchema = new mongoose.Schema({
...
const Pdb10q = mongoose.model('Pdb10q', Pdb10qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Pdb10q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Pdb10q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pdb10q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pdb10q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
