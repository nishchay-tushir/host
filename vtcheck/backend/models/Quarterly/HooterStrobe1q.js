...
const HooterStrobe1qSchema = new mongoose.Schema({
...
const HooterStrobe1q = mongoose.model('HooterStrobe1q', HooterStrobe1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await HooterStrobe1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new HooterStrobe1q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HooterStrobe1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HooterStrobe1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HooterStrobe1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
