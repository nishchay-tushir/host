...
const InhouseLight42qSchema = new mongoose.Schema({
...
const InhouseLight42q = mongoose.model('InhouseLight42q', InhouseLight42qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await InhouseLight42q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new InhouseLight42q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await InhouseLight42q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await InhouseLight42q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await InhouseLight42q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
