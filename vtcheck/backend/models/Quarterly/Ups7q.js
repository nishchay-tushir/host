...
const Ups7qSchema = new mongoose.Schema({
...
const Ups7q = mongoose.model('Ups7q', Ups7qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Ups7q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Ups7q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Ups7q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Ups7q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
