...
const Mldb1qSchema = new mongoose.Schema({
...
const Mldb1q = mongoose.model('Mldb1q', Mldb1qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await Mldb1q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await Mldb1q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Mldb1q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Mldb1q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
