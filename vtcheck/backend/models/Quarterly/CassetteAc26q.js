...
const CassetteAc26qSchema = new mongoose.Schema({
...
const CassetteAc26q = mongoose.model('CassetteAc26q', CassetteAc26qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await CassetteAc26q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new CassetteAc26q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAc26q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CassetteAc26q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CassetteAc26q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
