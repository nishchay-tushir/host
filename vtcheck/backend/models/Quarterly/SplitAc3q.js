...
const SplitAc3qSchema = new mongoose.Schema({
...
const SplitAc3q = mongoose.model('SplitAc3q', SplitAc3qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await SplitAc3q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAc3q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAc3q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAc3q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
