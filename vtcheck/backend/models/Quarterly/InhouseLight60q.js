...
const InhouseLight60qSchema = new mongoose.Schema({
...
const InhouseLight60q = mongoose.model('InhouseLight60q', InhouseLight60qSchema);
...
try {
    ...
    const existingRecord = await InhouseLight60q.findOne({
        submissionDate: {
            $gte: startOfQuarter,
    ...
    const newRecord = new InhouseLight60q({
        ...req.body,
...
try {
    const records = await InhouseLight60q.find();
...
try {
    const updatedRecord = await InhouseLight60q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
try {
    const deletedRecord = await InhouseLight60q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
