...
const WoodenDoubleLeafDoor4qSchema = new mongoose.Schema({
...
const WoodenDoubleLeafDoor4q = mongoose.model('WoodenDoubleLeafDoor4q', WoodenDoubleLeafDoor4qSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WoodenDoubleLeafDoor4q.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WoodenDoubleLeafDoor4q({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenDoubleLeafDoor4q.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WoodenDoubleLeafDoor4q.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WoodenDoubleLeafDoor4q.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
