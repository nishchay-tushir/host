...
const HTLTpanel9DSchema = new mongoose.Schema({
...
const HTLTpanel9D = mongoose.model('HTLTpanel9D', HTLTpanel9DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await HTLTpanel9D.findOne({
    ...
    const newRecord = new HTLTpanel9D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await HTLTpanel9D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await HTLTpanel9D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HTLTpanel9D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HTLTpanel9D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
