...
const DG5001DSchema = new mongoose.Schema({
...
const DG5001D = mongoose.model('DG5001D', DG5001DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await DG5001D.findOne({
    ...
    const newRecord = new DG5001D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await DG5001D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await DG5001D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG5001D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG5001D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
