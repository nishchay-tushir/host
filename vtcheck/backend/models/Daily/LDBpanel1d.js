...
const LDBpanel1DSchema = new mongoose.Schema({
...
const LDBpanel1D = mongoose.model('LDBpanel1D', LDBpanel1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await LDBpanel1D.findOne({
    ...
    const newRecord = new LDBpanel1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await LDBpanel1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await LDBpanel1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await LDBpanel1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await LDBpanel1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
