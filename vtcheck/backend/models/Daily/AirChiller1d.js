...
const AirChiller1DSchema = new mongoose.Schema({
...
const AirChiller1D = mongoose.model('AirChiller1D', AirChiller1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AirChiller1D.findOne({
    ...
    const newRecord = new AirChiller1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AirChiller1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AirChiller1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirChiller1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirChiller1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
