...
const ACB35DSchema = new mongoose.Schema({
...
const ACB35D = mongoose.model('ACB35D', ACB35DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB35D.findOne({
    ...
    const newRecord = new ACB35D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB35D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB35D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB35D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB35D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
