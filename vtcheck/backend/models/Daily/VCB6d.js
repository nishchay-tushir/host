...
const VCB6DSchema = new mongoose.Schema({
...
const VCB6D = mongoose.model('VCB6D', VCB6DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB6D.findOne({
    ...
    const newRecord = new VCB6D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB6D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB6D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB6D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB6D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
