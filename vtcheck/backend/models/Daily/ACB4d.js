...
const ACB4DSchema = new mongoose.Schema({
...
const ACB4D = mongoose.model('ACB4D', ACB4DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB4D.findOne({
    ...
    const newRecord = new ACB4D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB4D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB4D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB4D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB4D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
