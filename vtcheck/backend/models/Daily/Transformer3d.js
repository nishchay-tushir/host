...
const Transformer3DSchema = new mongoose.Schema({
...
const Transformer3D = mongoose.model('Transformer3D', Transformer3DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await Transformer3D.findOne({
    ...
    const newRecord = new Transformer3D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await Transformer3D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await Transformer3D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Transformer3D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Transformer3D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
