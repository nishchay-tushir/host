...
const HTLTpanel3DSchema = new mongoose.Schema({
...
const HTLTpanel3D = mongoose.model('HTLTpanel3D', HTLTpanel3DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await HTLTpanel3D.findOne({
    ...
    const newRecord = new HTLTpanel3D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await HTLTpanel3D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await HTLTpanel3D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HTLTpanel3D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HTLTpanel3D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
