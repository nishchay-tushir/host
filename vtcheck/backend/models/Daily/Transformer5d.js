...
const Transformer5DSchema = new mongoose.Schema({
  ...
  SilicaGelColor: { type: String, required: true },
  Cleaning: { type: String, required: true },
  TapPosition: { type: String, required: true },
...
const Transformer5D = mongoose.model('Transformer5D', Transformer5DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await Transformer5D.findOne({
    ...
    const newRecord = new Transformer5D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await Transformer5D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await Transformer5D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Transformer5D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Transformer5D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
