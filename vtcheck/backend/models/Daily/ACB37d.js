...
const ACB37DSchema = new mongoose.Schema({
...
const ACB37D = mongoose.model('ACB37D', ACB37DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB37D.findOne({
    ...
    const newRecord = new ACB37D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB37D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB37D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB37D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB37D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
