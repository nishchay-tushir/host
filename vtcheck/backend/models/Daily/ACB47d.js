...
const ACB47DSchema = new mongoose.Schema({
...
const ACB47D = mongoose.model('ACB47D', ACB47DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB47D.findOne({
    ...
    const newRecord = new ACB47D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB47D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB47D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB47D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB47D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
