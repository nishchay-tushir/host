...
const UPSDBpanel1DSchema = new mongoose.Schema({
...
const UPSDBpanel1D = mongoose.model('UPSDBpanel1D', UPSDBpanel1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await UPSDBpanel1D.findOne({
    ...
    const newRecord = new UPSDBpanel1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await UPSDBpanel1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await UPSDBpanel1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await UPSDBpanel1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await UPSDBpanel1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
