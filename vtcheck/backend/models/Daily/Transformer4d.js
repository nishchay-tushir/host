...
const Transformer4DSchema = new mongoose.Schema({
...
const Transformer4D = mongoose.model('Transformer4D', Transformer4DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await Transformer4D.findOne({
    ...
    const newRecord = new Transformer4D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await Transformer4D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await Transformer4D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Transformer4D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Transformer4D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
