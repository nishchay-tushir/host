...
const AHU6DSchema = new mongoose.Schema({
...
const AHU6D = mongoose.model('AHU6D', AHU6DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU6D.findOne({
    ...
    const newRecord = new AHU6D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU6D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU6D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU6D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU6D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
