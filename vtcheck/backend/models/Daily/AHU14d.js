...
const AHU14DSchema = new mongoose.Schema({
...
const AHU14D = mongoose.model('AHU14D', AHU14DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU14D.findOne({
    ...
    const newRecord = new AHU14D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU14D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU14D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU14D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU14D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
