...
const VCB5DSchema = new mongoose.Schema({
...
const VCB5D = mongoose.model('VCB5D', VCB5DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB5D.findOne({
    ...
    const newRecord = new VCB5D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB5D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB5D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB5D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB5D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
