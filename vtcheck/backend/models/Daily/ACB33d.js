...
const ACB33DSchema = new mongoose.Schema({
...
const ACB33D = mongoose.model('ACB33D', ACB33DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB33D.findOne({
    ...
    const newRecord = new ACB33D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB33D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB33D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB33D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB33D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
