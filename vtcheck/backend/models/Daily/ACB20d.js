...
const ACB20DSchema = new mongoose.Schema({
...
const ACB20D = mongoose.model('ACB20D', ACB20DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB20D.findOne({
    ...
    const newRecord = new ACB20D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB20D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB20D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB20D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB20D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
