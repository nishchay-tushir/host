...
const ACB49DSchema = new mongoose.Schema({
...
const ACB49D = mongoose.model('ACB49D', ACB49DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB49D.findOne({
    ...
    const newRecord = new ACB49D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB49D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB49D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB49D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB49D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
