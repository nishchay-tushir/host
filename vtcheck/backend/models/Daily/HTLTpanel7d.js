...
const HTLTpanel7DSchema = new mongoose.Schema({
...
const HTLTpanel7D = mongoose.model('HTLTpanel7D', HTLTpanel7DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await HTLTpanel7D.findOne({
    ...
    const newRecord = new HTLTpanel7D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await HTLTpanel7D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await HTLTpanel7D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HTLTpanel7D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HTLTpanel7D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
