...
const ACB21DSchema = new mongoose.Schema({
...
const ACB21D = mongoose.model('ACB21D', ACB21DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB21D.findOne({
    ...
    const newRecord = new ACB21D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB21D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB21D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB21D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB21D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
