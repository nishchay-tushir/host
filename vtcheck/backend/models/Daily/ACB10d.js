...
const ACB10DSchema = new mongoose.Schema({
...
const ACB10D = mongoose.model('ACB10D', ACB10DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB10D.findOne({
    ...
    const newRecord = new ACB10D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB10D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB10D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB10D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB10D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
