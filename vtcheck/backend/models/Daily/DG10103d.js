...
const DG10103DSchema = new mongoose.Schema({
...
const DG10103D = mongoose.model('DG10103D', DG10103DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await DG10103D.findOne({
    ...
    const newRecord = new DG10103D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await DG10103D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await DG10103D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG10103D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG10103D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
