...
const UPS3DSchema = new mongoose.Schema({
...
const UPS3D = mongoose.model('UPS3D', UPS3DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await UPS3D.findOne({
    ...
    const newRecord = new UPS3D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await UPS3D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await UPS3D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await UPS3D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await UPS3D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
