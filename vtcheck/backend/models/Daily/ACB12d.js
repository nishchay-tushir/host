...
const ACB12DSchema = new mongoose.Schema({
...
const ACB12D = mongoose.model('ACB12D', ACB12DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB12D.findOne({
    ...
    const newRecord = new ACB12D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB12D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB12D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB12D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB12D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
