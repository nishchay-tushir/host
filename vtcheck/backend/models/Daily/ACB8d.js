...
const ACB8DSchema = new mongoose.Schema({
...
const ACB8D = mongoose.model('ACB8D', ACB8DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB8D.findOne({
    ...
    const newRecord = new ACB8D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB8D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB8D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB8D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB8D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
