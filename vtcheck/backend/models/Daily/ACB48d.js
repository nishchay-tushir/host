...
const ACB48DSchema = new mongoose.Schema({
...
const ACB48D = mongoose.model('ACB48D', ACB48DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB48D.findOne({
    ...
    const newRecord = new ACB48D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB48D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB48D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB48D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB48D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
