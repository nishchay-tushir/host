...
const ACB15DSchema = new mongoose.Schema({
...
const ACB15D = mongoose.model('ACB15D', ACB15DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB15D.findOne({
    ...
    const newRecord = new ACB15D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB15D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB15D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB15D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB15D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
