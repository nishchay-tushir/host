...
const ACB31DSchema = new mongoose.Schema({
...
const ACB31D = mongoose.model('ACB31D', ACB31DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB31D.findOne({
    ...
    const newRecord = new ACB31D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB31D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB31D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB31D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB31D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
