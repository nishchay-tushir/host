...
const SwagonCompressor1DSchema = new mongoose.Schema({
...
const SwagonCompressor1D = mongoose.model('SwagonCompressor1D', SwagonCompressor1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await SwagonCompressor1D.findOne({
    ...
    const newRecord = new SwagonCompressor1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await SwagonCompressor1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await SwagonCompressor1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SwagonCompressor1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SwagonCompressor1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
