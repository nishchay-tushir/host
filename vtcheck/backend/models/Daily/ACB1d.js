...
const ACB1DSchema = new mongoose.Schema({
...
const ACB1D = mongoose.model('ACB1D', ACB1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB1D.findOne({
    ...
    const newRecord = new ACB1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
