...
const ACB30DSchema = new mongoose.Schema({
...
const ACB30D = mongoose.model('ACB30D', ACB30DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB30D.findOne({
    ...
    const newRecord = new ACB30D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB30D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB30D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB30D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB30D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
