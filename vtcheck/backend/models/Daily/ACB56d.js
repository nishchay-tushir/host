...
const ACB56DSchema = new mongoose.Schema({
...
const ACB56D = mongoose.model('ACB56D', ACB56DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB56D.findOne({
    ...
    const newRecord = new ACB56D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB56D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB56D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB56D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB56D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
