...
const ACB25DSchema = new mongoose.Schema({
...
const ACB25D = mongoose.model('ACB25D', ACB25DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB25D.findOne({
    ...
    const newRecord = new ACB25D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB25D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB25D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB25D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB25D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
