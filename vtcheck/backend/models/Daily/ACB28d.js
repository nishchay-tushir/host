...
const ACB28DSchema = new mongoose.Schema({
...
const ACB28D = mongoose.model('ACB28D', ACB28DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB28D.findOne({
    ...
    const newRecord = new ACB28D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB28D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB28D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB28D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB28D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
