...
const AHU5DSchema = new mongoose.Schema({
...
const AHU5D = mongoose.model('AHU5D', AHU5DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU5D.findOne({
    ...
    const newRecord = new AHU5D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU5D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU5D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU5D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU5D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
