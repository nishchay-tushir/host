...
const UPS1DSchema = new mongoose.Schema({
...
const UPS1D = mongoose.model('UPS1D', UPS1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await UPS1D.findOne({
    ...
    const newRecord = new UPS1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await UPS1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await UPS1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await UPS1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await UPS1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
