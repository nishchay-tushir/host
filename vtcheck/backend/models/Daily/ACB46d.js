...
const ACB46DSchema = new mongoose.Schema({
...
const ACB46D = mongoose.model('ACB46D', ACB46DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB46D.findOne({
    ...
    const newRecord = new ACB46D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB46D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB46D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB46D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB46D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
