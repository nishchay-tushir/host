...
const ACB57DSchema = new mongoose.Schema({
...
const ACB57D = mongoose.model('ACB57D', ACB57DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB57D.findOne({
    ...
    const newRecord = new ACB57D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB57D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB57D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB57D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB57D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
