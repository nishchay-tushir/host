...
const HTLTpanel1DSchema = new mongoose.Schema({
...
const HTLTpanel1D = mongoose.model('HTLTpanel1D', HTLTpanel1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await HTLTpanel1D.findOne({
    ...
    const newRecord = new HTLTpanel1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await HTLTpanel1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await HTLTpanel1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HTLTpanel1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HTLTpanel1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
