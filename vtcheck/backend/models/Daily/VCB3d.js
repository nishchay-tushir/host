...
const VCB3DSchema = new mongoose.Schema({
...
const VCB3D = mongoose.model('VCB3D', VCB3DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB3D.findOne({
    ...
    const newRecord = new VCB3D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB3D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB3D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB3D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB3D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
