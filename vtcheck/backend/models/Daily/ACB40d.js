...
const ACB40DSchema = new mongoose.Schema({
...
const ACB40D = mongoose.model('ACB40D', ACB40DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB40D.findOne({
    ...
    const newRecord = new ACB40D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB40D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB40D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB40D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB40D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
