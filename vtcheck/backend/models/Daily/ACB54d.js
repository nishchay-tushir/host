...
const ACB54DSchema = new mongoose.Schema({
...
const ACB54D = mongoose.model('ACB54D', ACB54DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB54D.findOne({
    ...
    const newRecord = new ACB54D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB54D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB54D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB54D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB54D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
