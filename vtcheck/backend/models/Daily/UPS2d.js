...
const UPS2DSchema = new mongoose.Schema({
...
const UPS2D = mongoose.model('UPS2D', UPS2DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await UPS2D.findOne({
    ...
    const newRecord = new UPS2D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await UPS2D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await UPS2D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await UPS2D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await UPS2D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
