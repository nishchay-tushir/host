...
const ACB58DSchema = new mongoose.Schema({
...
const ACB58D = mongoose.model('ACB58D', ACB58DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB58D.findOne({
    ...
    const newRecord = new ACB58D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB58D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB58D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB58D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB58D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
