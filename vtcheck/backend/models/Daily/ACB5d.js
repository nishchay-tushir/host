...
const ACB5DSchema = new mongoose.Schema({
...
const ACB5D = mongoose.model('ACB5D', ACB5DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB5D.findOne({
    ...
    const newRecord = new ACB5D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB5D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB5D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB5D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB5D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
