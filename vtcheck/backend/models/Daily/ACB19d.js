...
const ACB19DSchema = new mongoose.Schema({
...
const ACB19D = mongoose.model('ACB19D', ACB19DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB19D.findOne({
    ...
    const newRecord = new ACB19D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB19D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB19D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB19D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB19D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
