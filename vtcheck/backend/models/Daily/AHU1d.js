...
const AHU1DSchema = new mongoose.Schema({
...
const AHU1D = mongoose.model('AHU1D', AHU1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU1D.findOne({
    ...
    const newRecord = new AHU1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
