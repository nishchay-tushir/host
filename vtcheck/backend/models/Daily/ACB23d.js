...
const ACB23DSchema = new mongoose.Schema({
...
const ACB23D = mongoose.model('ACB23D', ACB23DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB23D.findOne({
    ...
    const newRecord = new ACB23D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB23D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB23D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB23D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB23D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
