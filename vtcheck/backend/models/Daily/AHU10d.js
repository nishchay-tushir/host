...
const AHU10DSchema = new mongoose.Schema({
...
const AHU10D = mongoose.model('AHU10D', AHU10DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU10D.findOne({
    ...
    const newRecord = new AHU10D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU10D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU10D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU10D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU10D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
