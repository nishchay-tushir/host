...
const Stacker1DSchema = new mongoose.Schema({
...
const Stacker1D = mongoose.model('Stacker1D', Stacker1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await Stacker1D.findOne({
    ...
    const newRecord = new Stacker1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await Stacker1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await Stacker1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Stacker1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Stacker1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
