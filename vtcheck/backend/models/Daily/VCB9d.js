...
const VCB9DSchema = new mongoose.Schema({
...
const VCB9D = mongoose.model('VCB9D', VCB9DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB9D.findOne({
    ...
    const newRecord = new VCB9D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB9D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB9D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB9D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB9D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
