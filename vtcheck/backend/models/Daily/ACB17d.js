...
const ACB17DSchema = new mongoose.Schema({
...
const ACB17D = mongoose.model('ACB17D', ACB17DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB17D.findOne({
    ...
    const newRecord = new ACB17D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB17D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB17D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB17D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB17D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
