...
const Transformer8DSchema = new mongoose.Schema({
...
const Transformer8D = mongoose.model('Transformer8D', Transformer8DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await Transformer8D.findOne({
    ...
    const newRecord = new Transformer8D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await Transformer8D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await Transformer8D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Transformer8D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Transformer8D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
