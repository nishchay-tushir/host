...
const ACB52DSchema = new mongoose.Schema({
...
const ACB52D = mongoose.model('ACB52D', ACB52DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB52D.findOne({
    ...
    const newRecord = new ACB52D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB52D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB52D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB52D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB52D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
