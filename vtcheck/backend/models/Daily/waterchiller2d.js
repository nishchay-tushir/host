...
const WaterChiller2DSchema = new mongoose.Schema({
...
const WaterChiller2D = mongoose.model('WaterChiller2D', WaterChiller2DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await WaterChiller2D.findOne({
    ...
    const newRecord = new WaterChiller2D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller2D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await WaterChiller2D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WaterChiller2D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WaterChiller2D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
