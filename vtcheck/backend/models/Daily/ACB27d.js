...
const ACB27DSchema = new mongoose.Schema({
...
const ACB27D = mongoose.model('ACB27D', ACB27DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB27D.findOne({
    ...
    const newRecord = new ACB27D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB27D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB27D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB27D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB27D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
