...
const ACB18DSchema = new mongoose.Schema({
...
const ACB18D = mongoose.model('ACB18D', ACB18DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB18D.findOne({
    ...
    const newRecord = new ACB18D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB18D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB18D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB18D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB18D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
