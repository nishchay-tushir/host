...
const ACB2DSchema = new mongoose.Schema({
...
const ACB2D = mongoose.model('ACB2D', ACB2DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB2D.findOne({
    ...
    const newRecord = new ACB2D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB2D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB2D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB2D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB2D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
