...
const ACB50DSchema = new mongoose.Schema({
...
const ACB50D = mongoose.model('ACB50D', ACB50DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB50D.findOne({
    ...
    const newRecord = new ACB50D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB50D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB50D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB50D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB50D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
