...
const VCB4DSchema = new mongoose.Schema({
...
const VCB4D = mongoose.model('VCB4D', VCB4DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB4D.findOne({
    ...
    const newRecord = new VCB4D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB4D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB4D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB4D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB4D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
