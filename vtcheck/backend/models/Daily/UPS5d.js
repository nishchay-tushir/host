...
const UPS5DSchema = new mongoose.Schema({
...
const UPS5D = mongoose.model('UPS5D', UPS5DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await UPS5D.findOne({
    ...
    const newRecord = new UPS5D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await UPS5D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await UPS5D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await UPS5D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await UPS5D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
