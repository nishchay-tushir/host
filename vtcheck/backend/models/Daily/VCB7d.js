...
const VCB7DSchema = new mongoose.Schema({
...
const VCB7D = mongoose.model('VCB7D', VCB7DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB7D.findOne({
    ...
    const newRecord = new VCB7D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB7D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB7D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB7D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB7D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
