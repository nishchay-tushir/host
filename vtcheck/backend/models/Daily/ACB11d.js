...
const ACB11DSchema = new mongoose.Schema({
...
const ACB11D = mongoose.model('ACB11D', ACB11DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB11D.findOne({
    ...
    const newRecord = new ACB11D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB11D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB11D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB11D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB11D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
