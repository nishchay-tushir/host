...
const ACB13DSchema = new mongoose.Schema({
...
const ACB13D = mongoose.model('ACB13D', ACB13DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB13D.findOne({
    ...
    const newRecord = new ACB13D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB13D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB13D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB13D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB13D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
