...
const AHU8DSchema = new mongoose.Schema({
...
const AHU8D = mongoose.model('AHU8D', AHU8DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU8D.findOne({
    ...
    const newRecord = new AHU8D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU8D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU8D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU8D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU8D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
