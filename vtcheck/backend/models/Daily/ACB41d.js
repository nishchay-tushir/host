...
const ACB41DSchema = new mongoose.Schema({
...
const ACB41D = mongoose.model('ACB41D', ACB41DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB41D.findOne({
    ...
    const newRecord = new ACB41D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB41D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB41D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB41D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB41D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
