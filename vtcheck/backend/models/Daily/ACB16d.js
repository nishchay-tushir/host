...
const ACB16DSchema = new mongoose.Schema({
...
const ACB16D = mongoose.model('ACB16D', ACB16DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB16D.findOne({
    ...
    const newRecord = new ACB16D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB16D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB16D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB16D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB16D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
