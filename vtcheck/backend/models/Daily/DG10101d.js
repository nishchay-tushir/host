...
const DG10101DSchema = new mongoose.Schema({
...
const DG10101D = mongoose.model('DG10101D', DG10101DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await DG10101D.findOne({
    ...
    const newRecord = new DG10101D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await DG10101D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await DG10101D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG10101D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG10101D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
