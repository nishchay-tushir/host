...
const ACB24DSchema = new mongoose.Schema({
...
const ACB24D = mongoose.model('ACB24D', ACB24DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB24D.findOne({
    ...
    const newRecord = new ACB24D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB24D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB24D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB24D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB24D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
