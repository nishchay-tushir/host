...
const ACB29DSchema = new mongoose.Schema({
...
const ACB29D = mongoose.model('ACB29D', ACB29DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB29D.findOne({
    ...
    const newRecord = new ACB29D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB29D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB29D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB29D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB29D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
