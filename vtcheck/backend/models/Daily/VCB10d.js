...
const VCB10DSchema = new mongoose.Schema({
...
const VCB10D = mongoose.model('VCB10D', VCB10DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB10D.findOne({
    ...
    const newRecord = new VCB10D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB10D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB10D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB10D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB10D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
