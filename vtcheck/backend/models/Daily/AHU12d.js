...
const AHU12DSchema = new mongoose.Schema({
...
const AHU12D = mongoose.model('AHU12D', AHU12DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU12D.findOne({
    ...
    const newRecord = new AHU12D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU12D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU12D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU12D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU12D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
