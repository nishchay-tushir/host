...
const ACB51DSchema = new mongoose.Schema({
...
const ACB51D = mongoose.model('ACB51D', ACB51DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB51D.findOne({
    ...
    const newRecord = new ACB51D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB51D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB51D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB51D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB51D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
