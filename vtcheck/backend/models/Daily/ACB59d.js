...
const ACB59DSchema = new mongoose.Schema({
...
const ACB59D = mongoose.model('ACB59D', ACB59DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB59D.findOne({
    ...
    const newRecord = new ACB59D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB59D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB59D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB59D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB59D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
