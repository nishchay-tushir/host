...
const VCB8DSchema = new mongoose.Schema({
...
const VCB8D = mongoose.model('VCB8D', VCB8DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB8D.findOne({
    ...
    const newRecord = new VCB8D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB8D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB8D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB8D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB8D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
