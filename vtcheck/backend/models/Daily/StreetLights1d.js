...
const StreetLights1DSchema = new mongoose.Schema({
  WorkingCondition: { type: String, required: true },
  Fluctuation: { type: String, required: true },
  PoleCondition: { type: String, required: true },
...
const StreetLights1D = mongoose.model('StreetLights1D', StreetLights1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await StreetLights1D.findOne({
    ...
    const newRecord = new StreetLights1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await StreetLights1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await StreetLights1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StreetLights1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StreetLights1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
