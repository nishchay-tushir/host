...
const ACB14DSchema = new mongoose.Schema({
...
const ACB14D = mongoose.model('ACB14D', ACB14DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB14D.findOne({
    ...
    const newRecord = new ACB14D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB14D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB14D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB14D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB14D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
