...
const AHU3DSchema = new mongoose.Schema({
...
const AHU3D = mongoose.model('AHU3D', AHU3DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU3D.findOne({
    ...
    const newRecord = new AHU3D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU3D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU3D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU3D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU3D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
