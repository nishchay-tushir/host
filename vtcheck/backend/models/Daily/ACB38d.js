...
const ACB38DSchema = new mongoose.Schema({
...
const ACB38D = mongoose.model('ACB38D', ACB38DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB38D.findOne({
    ...
    const newRecord = new ACB38D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB38D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB38D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB38D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB38D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
