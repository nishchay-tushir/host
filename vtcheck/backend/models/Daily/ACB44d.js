...
const ACB44DSchema = new mongoose.Schema({
...
const ACB44D = mongoose.model('ACB44D', ACB44DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB44D.findOne({
    ...
    const newRecord = new ACB44D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB44D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB44D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB44D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB44D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
