...
const AirCompressor1DSchema = new mongoose.Schema({
...
const AirCompressor1D = mongoose.model('AirCompressor1D', AirCompressor1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AirCompressor1D.findOne({
    ...
    const newRecord = new AirCompressor1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AirCompressor1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AirCompressor1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirCompressor1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirCompressor1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
