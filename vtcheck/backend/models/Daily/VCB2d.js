...
const VCB2DSchema = new mongoose.Schema({
...
const VCB2D = mongoose.model('VCB2D', VCB2DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB2D.findOne({
    ...
    const newRecord = new VCB2D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB2D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB2D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB2D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB2D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
