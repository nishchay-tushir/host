...
const ACB42DSchema = new mongoose.Schema({
...
const ACB42D = mongoose.model('ACB42D', ACB42DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB42D.findOne({
    ...
    const newRecord = new ACB42D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB42D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB42D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB42D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB42D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
