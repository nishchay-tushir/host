...
const AHU7DSchema = new mongoose.Schema({
...
const AHU7D = mongoose.model('AHU7D', AHU7DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU7D.findOne({
    ...
    const newRecord = new AHU7D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU7D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU7D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU7D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU7D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
