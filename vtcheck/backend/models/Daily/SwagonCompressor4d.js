...
const SwagonCompressor4DSchema = new mongoose.Schema({
...
const SwagonCompressor4D = mongoose.model('SwagonCompressor4D', SwagonCompressor4DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await SwagonCompressor4D.findOne({
    ...
    const newRecord = new SwagonCompressor4D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await SwagonCompressor4D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await SwagonCompressor4D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SwagonCompressor4D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SwagonCompressor4D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
