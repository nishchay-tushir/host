...
const HTLTpanel6DSchema = new mongoose.Schema({
...
const HTLTpanel6D = mongoose.model('HTLTpanel6D', HTLTpanel6DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await HTLTpanel6D.findOne({
    ...
    const newRecord = new HTLTpanel6D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await HTLTpanel6D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await HTLTpanel6D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HTLTpanel6D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HTLTpanel6D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
