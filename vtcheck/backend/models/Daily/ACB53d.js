...
const ACB53DSchema = new mongoose.Schema({
...
const ACB53D = mongoose.model('ACB53D', ACB53DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB53D.findOne({
    ...
    const newRecord = new ACB53D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB53D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB53D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB53D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB53D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
