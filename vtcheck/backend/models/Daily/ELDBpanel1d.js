...
const ELDBpanel1DSchema = new mongoose.Schema({
...
const ELDBpanel1D = mongoose.model('ELDBpanel1D', ELDBpanel1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ELDBpanel1D.findOne({
    ...
    const newRecord = new ELDBpanel1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ELDBpanel1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ELDBpanel1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ELDBpanel1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ELDBpanel1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
