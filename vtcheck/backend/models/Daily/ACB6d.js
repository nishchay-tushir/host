...
const ACB6DSchema = new mongoose.Schema({
...
const ACB6D = mongoose.model('ACB6D', ACB6DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB6D.findOne({
    ...
    const newRecord = new ACB6D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB6D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB6D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB6D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB6D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
