...
const DDCpanel1DSchema = new mongoose.Schema({
...
const DDCpanel1D = mongoose.model('DDCpanel1D', DDCpanel1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await DDCpanel1D.findOne({
    ...
    const newRecord = new DDCpanel1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await DDCpanel1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await DDCpanel1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DDCpanel1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DDCpanel1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
