...
const Transformer6DSchema = new mongoose.Schema({
...
const Transformer6D = mongoose.model('Transformer6D', Transformer6DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await Transformer6D.findOne({
    ...
    const newRecord = new Transformer6D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await Transformer6D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await Transformer6D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Transformer6D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Transformer6D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
