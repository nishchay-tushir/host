...
const SwagonCompressor2DSchema = new mongoose.Schema({
...
const SwagonCompressor2D = mongoose.model('SwagonCompressor2D', SwagonCompressor2DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await SwagonCompressor2D.findOne({
    ...
    const newRecord = new SwagonCompressor2D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await SwagonCompressor2D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await SwagonCompressor2D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SwagonCompressor2D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SwagonCompressor2D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
