...
const VCB1DSchema = new mongoose.Schema({
...
const VCB1D = mongoose.model('VCB1D', VCB1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await VCB1D.findOne({
    ...
    const newRecord = new VCB1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await VCB1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await VCB1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await VCB1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
