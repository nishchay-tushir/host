...
const DG5003DSchema = new mongoose.Schema({
...
const DG5003D = mongoose.model('DG5003D', DG5003DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await DG5003D.findOne({
    ...
    const newRecord = new DG5003D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await DG5003D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await DG5003D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG5003D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG5003D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
