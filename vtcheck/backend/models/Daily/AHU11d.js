...
const AHU11DSchema = new mongoose.Schema({
...
const AHU11D = mongoose.model('AHU11D', AHU11DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU11D.findOne({
    ...
    const newRecord = new AHU11D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU11D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU11D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU11D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU11D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
