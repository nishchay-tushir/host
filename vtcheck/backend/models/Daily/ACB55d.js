...
const ACB55DSchema = new mongoose.Schema({
...
const ACB55D = mongoose.model('ACB55D', ACB55DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB55D.findOne({
    ...
    const newRecord = new ACB55D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB55D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB55D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB55D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB55D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
