...
const CoolingTower1DSchema = new mongoose.Schema({
...
const CoolingTower1D = mongoose.model('CoolingTower1D', CoolingTower1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await CoolingTower1D.findOne({
    ...
    const newRecord = new CoolingTower1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await CoolingTower1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await CoolingTower1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await CoolingTower1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await CoolingTower1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
