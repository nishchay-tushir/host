...
const AHU13DSchema = new mongoose.Schema({
...
const AHU13D = mongoose.model('AHU13D', AHU13DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU13D.findOne({
    ...
    const newRecord = new AHU13D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU13D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU13D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU13D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU13D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
