...
const Transformer1DSchema = new mongoose.Schema({
...
const Transformer1D = mongoose.model('Transformer1D', Transformer1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await Transformer1D.findOne({
    ...
    const newRecord = new Transformer1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await Transformer1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await Transformer1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Transformer1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Transformer1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
