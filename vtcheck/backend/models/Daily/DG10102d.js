...
const DG10102DSchema = new mongoose.Schema({
...
const DG10102D = mongoose.model('DG10102D', DG10102DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await DG10102D.findOne({
    ...
    const newRecord = new DG10102D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await DG10102D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await DG10102D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG10102D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG10102D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
