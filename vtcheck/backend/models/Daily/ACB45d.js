...
const ACB45DSchema = new mongoose.Schema({
...
const ACB45D = mongoose.model('ACB45D', ACB45DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB45D.findOne({
    ...
    const newRecord = new ACB45D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB45D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB45D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB45D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB45D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
