...
const ACB3DSchema = new mongoose.Schema({
...
const ACB3D = mongoose.model('ACB3D', ACB3DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB3D.findOne({
    ...
    const newRecord = new ACB3D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB3D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB3D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB3D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB3D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
