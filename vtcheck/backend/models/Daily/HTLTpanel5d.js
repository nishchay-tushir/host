...
const HTLTpanel5DSchema = new mongoose.Schema({
...
const HTLTpanel5D = mongoose.model('HTLTpanel5D', HTLTpanel5DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await HTLTpanel5D.findOne({
    ...
    const newRecord = new HTLTpanel5D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await HTLTpanel5D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await HTLTpanel5D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await HTLTpanel5D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await HTLTpanel5D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
