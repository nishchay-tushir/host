...
const ACB26DSchema = new mongoose.Schema({
...
const ACB26D = mongoose.model('ACB26D', ACB26DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB26D.findOne({
    ...
    const newRecord = new ACB26D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB26D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB26D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB26D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB26D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
