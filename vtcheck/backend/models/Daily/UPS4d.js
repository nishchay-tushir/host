...
const UPS4DSchema = new mongoose.Schema({
...
const UPS4D = mongoose.model('UPS4D', UPS4DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await UPS4D.findOne({
    ...
    const newRecord = new UPS4D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await UPS4D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await UPS4D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await UPS4D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await UPS4D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
