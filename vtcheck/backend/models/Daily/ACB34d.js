...
const ACB34DSchema = new mongoose.Schema({
...
const ACB34D = mongoose.model('ACB34D', ACB34DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB34D.findOne({
    ...
    const newRecord = new ACB34D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB34D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB34D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB34D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB34D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
