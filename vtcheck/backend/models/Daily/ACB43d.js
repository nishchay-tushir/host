...
const ACB43DSchema = new mongoose.Schema({
...
const ACB43D = mongoose.model('ACB43D', ACB43DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB43D.findOne({
    ...
    const newRecord = new ACB43D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB43D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB43D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB43D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB43D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
