...
const ACB39DSchema = new mongoose.Schema({
...
const ACB39D = mongoose.model('ACB39D', ACB39DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB39D.findOne({
    ...
    const newRecord = new ACB39D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB39D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB39D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB39D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB39D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
