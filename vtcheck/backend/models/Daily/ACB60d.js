...
const ACB60DSchema = new mongoose.Schema({
...
const ACB60D = mongoose.model('ACB60D', ACB60DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB60D.findOne({
    ...
    const newRecord = new ACB60D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB60D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB60D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB60D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB60D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
