...
const ACB32DSchema = new mongoose.Schema({
...
const ACB32D = mongoose.model('ACB32D', ACB32DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB32D.findOne({
    ...
    const newRecord = new ACB32D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB32D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB32D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB32D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB32D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
