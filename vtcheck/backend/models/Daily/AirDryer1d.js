...
const AirDryer1DSchema = new mongoose.Schema({
...
const AirDryer1D = mongoose.model('AirDryer1D', AirDryer1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AirDryer1D.findOne({
    ...
    const newRecord = new AirDryer1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AirDryer1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AirDryer1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirDryer1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirDryer1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
