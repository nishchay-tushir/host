...
const ACB7DSchema = new mongoose.Schema({
...
const ACB7D = mongoose.model('ACB7D', ACB7DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB7D.findOne({
    ...
    const newRecord = new ACB7D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB7D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB7D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB7D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB7D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
