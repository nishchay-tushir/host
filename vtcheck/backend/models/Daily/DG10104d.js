...
const DG10104DSchema = new mongoose.Schema({
...
const DG10104D = mongoose.model('DG10104D', DG10104DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await DG10104D.findOne({
    ...
    const newRecord = new DG10104D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await DG10104D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await DG10104D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG10104D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG10104D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
