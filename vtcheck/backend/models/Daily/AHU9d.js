...
const AHU9DSchema = new mongoose.Schema({
...
const AHU9D = mongoose.model('AHU9D', AHU9DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await AHU9D.findOne({
    ...
    const newRecord = new AHU9D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU9D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await AHU9D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AHU9D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AHU9D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
