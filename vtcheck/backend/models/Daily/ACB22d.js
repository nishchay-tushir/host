...
const ACB22DSchema = new mongoose.Schema({
...
const ACB22D = mongoose.model('ACB22D', ACB22DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB22D.findOne({
    ...
    const newRecord = new ACB22D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB22D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB22D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB22D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB22D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
