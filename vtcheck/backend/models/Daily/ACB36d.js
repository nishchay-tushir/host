...
const ACB36DSchema = new mongoose.Schema({
...
const ACB36D = mongoose.model('ACB36D', ACB36DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await ACB36D.findOne({
    ...
    const newRecord = new ACB36D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await ACB36D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await ACB36D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ACB36D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ACB36D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
