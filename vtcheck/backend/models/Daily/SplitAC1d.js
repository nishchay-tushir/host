...
const SplitAC1DSchema = new mongoose.Schema({
...
const SplitAC1D = mongoose.model('SplitAC1D', SplitAC1DSchema);
...
router.post('/', async (req, res) => {
  try {
    ...
    const existingRecord = await SplitAC1D.findOne({
    ...
    const newRecord = new SplitAC1D({ ...req.body, submissionDate: new Date() });
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAC1D.find();
...
router.get('/status', async (req, res) => {
  try {
    ...
    const record = await SplitAC1D.findOne({
      submissionDate: { $gte: today, $lt: tomorrow }
    });
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SplitAC1D.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SplitAC1D.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
