...
const FireExtinguisher48mSchema = new mongoose.Schema({
...
const FireExtinguisher48m = mongoose.model('FireExtinguisher48m', FireExtinguisher48mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher48m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher48m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher48m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher48m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher48m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher48m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
