...
const MotorisedSlidingGate3mSchema = new mongoose.Schema({
...
const MotorisedSlidingGate3m = mongoose.model('MotorisedSlidingGate3m', MotorisedSlidingGate3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await MotorisedSlidingGate3m.findOne({
        submissionDate: {
      ...
      const newRecord = new MotorisedSlidingGate3m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MotorisedSlidingGate3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await MotorisedSlidingGate3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await MotorisedSlidingGate3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await MotorisedSlidingGate3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
