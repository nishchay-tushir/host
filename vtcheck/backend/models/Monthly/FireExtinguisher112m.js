...
const FireExtinguisher112mSchema = new mongoose.Schema({
...
const FireExtinguisher112m = mongoose.model('FireExtinguisher112m', FireExtinguisher112mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher112m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher112m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher112m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher112m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher112m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher112m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
