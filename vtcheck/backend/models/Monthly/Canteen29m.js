...
const Canteen29mSchema = new mongoose.Schema({
...
const Canteen29m = mongoose.model('Canteen29m', Canteen29mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen29m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen29m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen29m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen29m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen29m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen29m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
