...
const RapidSlidingGate6mSchema = new mongoose.Schema({
...
const RapidSlidingGate6m = mongoose.model('RapidSlidingGate6m', RapidSlidingGate6mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidSlidingGate6m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidSlidingGate6m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidSlidingGate6m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidSlidingGate6m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidSlidingGate6m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidSlidingGate6m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
