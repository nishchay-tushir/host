...
const CoolingTower1mSchema = new mongoose.Schema({
...
const CoolingTower1m = mongoose.model('CoolingTower1m', CoolingTower1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await CoolingTower1m.findOne({
        submissionDate: {
      ...
      const newRecord = new CoolingTower1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CoolingTower1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await CoolingTower1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await CoolingTower1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await CoolingTower1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
