...
const CassetteAC1m10mSchema = new mongoose.Schema({
...
const CassetteAC1m10m = mongoose.model('CassetteAC1m10m', CassetteAC1m10mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await CassetteAC1m10m.findOne({
        submissionDate: {
      ...
      const newRecord = new CassetteAC1m10m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAC1m10m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await CassetteAC1m10m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await CassetteAC1m10m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await CassetteAC1m10m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
