...
const FireExtinguisher146mSchema = new mongoose.Schema({
...
const FireExtinguisher146m = mongoose.model('FireExtinguisher146m', FireExtinguisher146mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher146m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher146m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher146m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher146m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher146m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher146m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
