...
const MetalDoubleLeafDoor1mSchema = new mongoose.Schema({
...
const MetalDoubleLeafDoor1m = mongoose.model('MetalDoubleLeafDoor1m', MetalDoubleLeafDoor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await MetalDoubleLeafDoor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new MetalDoubleLeafDoor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MetalDoubleLeafDoor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await MetalDoubleLeafDoor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await MetalDoubleLeafDoor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await MetalDoubleLeafDoor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
