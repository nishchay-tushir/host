...
const AHU6mSchema = new mongoose.Schema({
...
const AHU6m = mongoose.model('AHU6m', AHU6mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU6m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU6m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU6m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU6m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU6m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
