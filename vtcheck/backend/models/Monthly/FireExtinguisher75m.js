...
const FireExtinguisher75mSchema = new mongoose.Schema({
...
const FireExtinguisher75m = mongoose.model('FireExtinguisher75m', FireExtinguisher75mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher75m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher75m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher75m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher75m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher75m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher75m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
