...
const Canteen23mSchema = new mongoose.Schema({
...
const Canteen23m = mongoose.model('Canteen23m', Canteen23mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen23m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen23m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen23m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen23m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen23m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen23m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
