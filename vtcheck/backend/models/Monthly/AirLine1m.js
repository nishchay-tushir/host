...
const AirLine1mSchema = new mongoose.Schema({
...
const AirLine1m = mongoose.model('AirLine1m', AirLine1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AirLine1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AirLine1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AirLine1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AirLine1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AirLine1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
