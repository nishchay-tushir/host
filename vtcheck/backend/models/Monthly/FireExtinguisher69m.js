...
const FireExtinguisher69mSchema = new mongoose.Schema({
...
const FireExtinguisher69m = mongoose.model('FireExtinguisher69m', FireExtinguisher69mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher69m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher69m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher69m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher69m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher69m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher69m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
