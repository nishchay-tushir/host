...
const FireExtinguisher110mSchema = new mongoose.Schema({
...
const FireExtinguisher110m = mongoose.model('FireExtinguisher110m', FireExtinguisher110mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher110m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher110m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher110m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher110m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher110m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher110m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
