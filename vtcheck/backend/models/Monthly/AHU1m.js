...
const AHU1mSchema = new mongoose.Schema({
...
const AHU1m = mongoose.model('AHU1m', AHU1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
