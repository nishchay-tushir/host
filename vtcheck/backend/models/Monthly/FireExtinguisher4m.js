...
const FireExtinguisher4mSchema = new mongoose.Schema({
...
const FireExtinguisher4m = mongoose.model('FireExtinguisher4m', FireExtinguisher4mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher4m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher4m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher4m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher4m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher4m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher4m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
