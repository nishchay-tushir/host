...
const WaterLine2mSchema = new mongoose.Schema({
...
const WaterLine2m = mongoose.model('WaterLine2m', WaterLine2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WaterLine2m.findOne({
        submissionDate: {
      ...
      const newRecord = new WaterLine2m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterLine2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WaterLine2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WaterLine2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WaterLine2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
