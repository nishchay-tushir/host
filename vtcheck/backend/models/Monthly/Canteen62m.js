...
const Canteen62mSchema = new mongoose.Schema({
...
const Canteen62m = mongoose.model('Canteen62m', Canteen62mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen62m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen62m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen62m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen62m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen62m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen62m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
