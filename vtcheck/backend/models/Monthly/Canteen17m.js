...
const Canteen17mSchema = new mongoose.Schema({
...
const Canteen17m = mongoose.model('Canteen17m', Canteen17mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen17m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen17m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen17m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen17m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen17m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen17m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
