...
const FireExtinguisher6mSchema = new mongoose.Schema({
...
const FireExtinguisher6m = mongoose.model('FireExtinguisher6m', FireExtinguisher6mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher6m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher6m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher6m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher6m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher6m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher6m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
