...
const MAU2mSchema = new mongoose.Schema({
...
const MAU2m = mongoose.model('MAU2m', MAU2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await MAU2m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await MAU2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await MAU2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await MAU2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await MAU2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
