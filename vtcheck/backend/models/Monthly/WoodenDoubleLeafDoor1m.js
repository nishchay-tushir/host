...
const WoodenDoubleLeafDoor1mSchema = new mongoose.Schema({
...
const WoodenDoubleLeafDoor1m = mongoose.model('WoodenDoubleLeafDoor1m', WoodenDoubleLeafDoor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WoodenDoubleLeafDoor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new WoodenDoubleLeafDoor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenDoubleLeafDoor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WoodenDoubleLeafDoor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WoodenDoubleLeafDoor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WoodenDoubleLeafDoor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
