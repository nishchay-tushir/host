...
const Canteen5mSchema = new mongoose.Schema({
...
const Canteen5m = mongoose.model('Canteen5m', Canteen5mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen5m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen5m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen5m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen5m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen5m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
