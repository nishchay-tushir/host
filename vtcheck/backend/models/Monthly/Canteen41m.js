...
const Canteen41mSchema = new mongoose.Schema({
...
const Canteen41m = mongoose.model('Canteen41m', Canteen41mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen41m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen41m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen41m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen41m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen41m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen41m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
