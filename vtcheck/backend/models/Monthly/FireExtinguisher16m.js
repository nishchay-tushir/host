...
const FireExtinguisher16mSchema = new mongoose.Schema({
...
const FireExtinguisher16m = mongoose.model('FireExtinguisher16m', FireExtinguisher16mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher16m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher16m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher16m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher16m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher16m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher16m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
