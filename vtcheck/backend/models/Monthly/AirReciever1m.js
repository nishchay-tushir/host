...
const AirReciever1mSchema = new mongoose.Schema({
...
const AirReciever1m = mongoose.model('AirReciever1m', AirReciever1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AirReciever1m.findOne({
        submissionDate: {
      ...
      const newRecord = new AirReciever1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirReciever1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AirReciever1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirReciever1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirReciever1m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
