...
const FireExtinguisher95mSchema = new mongoose.Schema({
...
const FireExtinguisher95m = mongoose.model('FireExtinguisher95m', FireExtinguisher95mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher95m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher95m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher95m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher95m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher95m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher95m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
