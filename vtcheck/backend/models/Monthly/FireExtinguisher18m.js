...
const FireExtinguisher18mSchema = new mongoose.Schema({
...
const FireExtinguisher18m = mongoose.model('FireExtinguisher18m', FireExtinguisher18mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher18m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher18m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher18m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher18m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher18m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher18m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
