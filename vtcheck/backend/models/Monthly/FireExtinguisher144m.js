...
const FireExtinguisher144mSchema = new mongoose.Schema({
...
const FireExtinguisher144m = mongoose.model('FireExtinguisher144m', FireExtinguisher144mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher144m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher144m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher144m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher144m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher144m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher144m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
