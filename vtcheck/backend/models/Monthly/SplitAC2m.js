...
const SplitAC2mSchema = new mongoose.Schema({
...
const SplitAC2m = mongoose.model('SplitAC2m', SplitAC2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SplitAC2m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAC2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SplitAC2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SplitAC2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SplitAC2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
