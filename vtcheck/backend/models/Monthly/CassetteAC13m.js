...
const CassetteAC1m13mSchema = new mongoose.Schema({
...
const CassetteAC1m13m = mongoose.model('CassetteAC1m13m', CassetteAC1m13mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await CassetteAC1m13m.findOne({
        submissionDate: {
      ...
      const newRecord = new CassetteAC1m13m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAC1m13m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await CassetteAC1m13m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await CassetteAC1m13m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await CassetteAC1m13m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
