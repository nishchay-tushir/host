...
const FireExtinguisher78mSchema = new mongoose.Schema({
...
const FireExtinguisher78m = mongoose.model('FireExtinguisher78m', FireExtinguisher78mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher78m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher78m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher78m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher78m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher78m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher78m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
