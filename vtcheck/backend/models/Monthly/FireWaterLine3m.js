...
const FireWaterLine3mSchema = new mongoose.Schema({
...
const FireWaterLine3m = mongoose.model('FireWaterLine3m', FireWaterLine3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireWaterLine3m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireWaterLine3m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireWaterLine3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireWaterLine3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireWaterLine3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireWaterLine3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
