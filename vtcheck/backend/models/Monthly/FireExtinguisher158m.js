...
const FireExtinguisher158mSchema = new mongoose.Schema({
...
const FireExtinguisher158m = mongoose.model('FireExtinguisher158m', FireExtinguisher158mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher158m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher158m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher158m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher158m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher158m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher158m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
