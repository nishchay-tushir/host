...
const AirDryer2mSchema = new mongoose.Schema({
...
const AirDryer2m = mongoose.model('AirDryer2m', AirDryer2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AirDryer2m.findOne({
        submissionDate: {
      ...
      const newRecord = new AirDryer2m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirDryer2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AirDryer2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AirDryer2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AirDryer2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
