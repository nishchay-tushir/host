...
const Canteen54mSchema = new mongoose.Schema({
...
const Canteen54m = mongoose.model('Canteen54m', Canteen54mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen54m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen54m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen54m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen54m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen54m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen54m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
