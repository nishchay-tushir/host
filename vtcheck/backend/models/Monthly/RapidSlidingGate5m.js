...
const RapidSlidingGate5mSchema = new mongoose.Schema({
...
const RapidSlidingGate5m = mongoose.model('RapidSlidingGate5m', RapidSlidingGate5mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidSlidingGate5m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidSlidingGate5m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidSlidingGate5m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidSlidingGate5m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidSlidingGate5m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidSlidingGate5m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
