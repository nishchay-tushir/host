...
const FireExtinguisher116mSchema = new mongoose.Schema({
...
const FireExtinguisher116m = mongoose.model('FireExtinguisher116m', FireExtinguisher116mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher116m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher116m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher116m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher116m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher116m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher116m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
