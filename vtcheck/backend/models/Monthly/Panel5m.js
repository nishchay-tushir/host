...
const Panel5mSchema = new mongoose.Schema({
...
const Panel5m = mongoose.model('Panel5m', Panel5mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Panel5m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel5m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Panel5m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await Panel5m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await Panel5m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
