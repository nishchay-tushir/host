...
const Canteen15mSchema = new mongoose.Schema({
...
const Canteen15m = mongoose.model('Canteen15m', Canteen15mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen15m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen15m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen15m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen15m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen15m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen15m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
