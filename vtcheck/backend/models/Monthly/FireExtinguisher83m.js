...
const FireExtinguisher83mSchema = new mongoose.Schema({
...
const FireExtinguisher83m = mongoose.model('FireExtinguisher83m', FireExtinguisher83mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher83m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher83m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher83m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher83m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher83m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher83m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
