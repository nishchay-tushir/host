...
const ShopFloor1mSchema = new mongoose.Schema({
...
const ShopFloor1m = mongoose.model('ShopFloor1m', ShopFloor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await ShopFloor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new ShopFloor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await ShopFloor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await ShopFloor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await ShopFloor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await ShopFloor1m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
