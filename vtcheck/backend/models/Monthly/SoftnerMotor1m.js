...
const SoftnerMotor1mSchema = new mongoose.Schema({
...
const SoftnerMotor1m = mongoose.model('SoftnerMotor1m', SoftnerMotor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SoftnerMotor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new SoftnerMotor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SoftnerMotor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SoftnerMotor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SoftnerMotor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SoftnerMotor1m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
