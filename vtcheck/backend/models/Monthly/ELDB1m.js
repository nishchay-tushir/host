...
const ELDB1mSchema = new mongoose.Schema({
...
const ELDB1m = mongoose.model('ELDB1m', ELDB1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await ELDB1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await ELDB1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await ELDB1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await ELDB1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await ELDB1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
