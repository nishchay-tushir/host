...
const WaterLine1mSchema = new mongoose.Schema({
...
const WaterLine1m = mongoose.model('WaterLine1m', WaterLine1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WaterLine1m.findOne({
        submissionDate: {
      ...
      const newRecord = new WaterLine1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterLine1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WaterLine1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WaterLine1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WaterLine1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
