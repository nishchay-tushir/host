...
const FireExtinguisher109mSchema = new mongoose.Schema({
...
const FireExtinguisher109m = mongoose.model('FireExtinguisher109m', FireExtinguisher109mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher109m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher109m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher109m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher109m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher109m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher109m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
