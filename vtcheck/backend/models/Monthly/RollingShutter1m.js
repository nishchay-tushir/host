...
const RollingShutter1mSchema = new mongoose.Schema({
...
const RollingShutter1m = mongoose.model('RollingShutter1m', RollingShutter1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RollingShutter1m.findOne({
        submissionDate: {
      ...
      const newRecord = new RollingShutter1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RollingShutter1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RollingShutter1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RollingShutter1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RollingShutter1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
