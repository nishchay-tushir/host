...
const CoolingTower2mSchema = new mongoose.Schema({
...
const CoolingTower2m = mongoose.model('CoolingTower2m', CoolingTower2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await CoolingTower2m.findOne({
        submissionDate: {
      ...
      const newRecord = new CoolingTower2m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CoolingTower2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await CoolingTower2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await CoolingTower2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await CoolingTower2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
