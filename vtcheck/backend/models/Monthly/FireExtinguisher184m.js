...
const FireExtinguisher184mSchema = new mongoose.Schema({
...
const FireExtinguisher184m = mongoose.model('FireExtinguisher184m', FireExtinguisher184mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher184m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher184m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher184m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher184m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher184m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher184m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
