...
const WoodenSingleLeafDoor1mSchema = new mongoose.Schema({
...
const WoodenSingleLeafDoor1m = mongoose.model('WoodenSingleLeafDoor1m', WoodenSingleLeafDoor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WoodenSingleLeafDoor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new WoodenSingleLeafDoor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WoodenSingleLeafDoor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WoodenSingleLeafDoor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WoodenSingleLeafDoor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WoodenSingleLeafDoor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
