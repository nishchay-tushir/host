...
const FireExtinguisher176mSchema = new mongoose.Schema({
...
const FireExtinguisher176m = mongoose.model('FireExtinguisher176m', FireExtinguisher176mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher176m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher176m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher176m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher176m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher176m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher176m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
