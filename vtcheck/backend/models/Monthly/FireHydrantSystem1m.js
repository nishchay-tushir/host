...
const FireHydrantSystem1mSchema = new mongoose.Schema({
...
const FireHydrantSystem1m = mongoose.model('FireHydrantSystem1m', FireHydrantSystem1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireHydrantSystem1m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireHydrantSystem1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrantSystem1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireHydrantSystem1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireHydrantSystem1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireHydrantSystem1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
