...
const FireExtinguisher87mSchema = new mongoose.Schema({
...
const FireExtinguisher87m = mongoose.model('FireExtinguisher87m', FireExtinguisher87mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher87m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher87m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher87m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher87m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher87m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher87m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
