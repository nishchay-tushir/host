...
const FireExtinguisher62mSchema = new mongoose.Schema({
...
const FireExtinguisher62m = mongoose.model('FireExtinguisher62m', FireExtinguisher62mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher62m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher62m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher62m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher62m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher62m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher62m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
