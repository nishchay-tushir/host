...
const FireWaterLine1mSchema = new mongoose.Schema({
...
const FireWaterLine1m = mongoose.model('FireWaterLine1m', FireWaterLine1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireWaterLine1m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireWaterLine1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireWaterLine1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireWaterLine1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireWaterLine1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireWaterLine1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
