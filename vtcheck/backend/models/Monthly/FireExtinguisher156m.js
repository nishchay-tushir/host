...
const FireExtinguisher156mSchema = new mongoose.Schema({
...
const FireExtinguisher156m = mongoose.model('FireExtinguisher156m', FireExtinguisher156mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher156m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher156m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher156m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher156m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher156m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher156m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
