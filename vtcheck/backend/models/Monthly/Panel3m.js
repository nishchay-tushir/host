...
const Panel3mSchema = new mongoose.Schema({
...
const Panel3m = mongoose.model('Panel3m', Panel3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Panel3m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Panel3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await Panel3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await Panel3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
