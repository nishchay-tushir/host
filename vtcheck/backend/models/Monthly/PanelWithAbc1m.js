...
const PanelWithAbc1mSchema = new mongoose.Schema({
...
const PanelWithAbc1m = mongoose.model('PanelWithAbc1m', PanelWithAbc1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await PanelWithAbc1m.findOne({
        submissionDate: {
      ...
      const newRecord = new PanelWithAbc1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await PanelWithAbc1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await PanelWithAbc1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await PanelWithAbc1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await PanelWithAbc1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
