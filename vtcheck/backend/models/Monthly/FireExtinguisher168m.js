...
const FireExtinguisher168mSchema = new mongoose.Schema({
...
const FireExtinguisher168m = mongoose.model('FireExtinguisher168m', FireExtinguisher168mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher168m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher168m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher168m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher168m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher168m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher168m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
