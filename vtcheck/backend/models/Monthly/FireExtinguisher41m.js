...
const FireExtinguisher41mSchema = new mongoose.Schema({
...
const FireExtinguisher41m = mongoose.model('FireExtinguisher41m', FireExtinguisher41mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher41m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher41m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher41m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher41m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher41m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher41m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
