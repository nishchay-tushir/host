...
const Pump2mSchema = new mongoose.Schema({
...
const Pump2m = mongoose.model('Pump2m', Pump2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Pump2m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Pump2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Pump2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pump2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pump2m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
