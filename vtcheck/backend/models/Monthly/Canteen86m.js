...
const Canteen86mSchema = new mongoose.Schema({
...
const Canteen86m = mongoose.model('Canteen86m', Canteen86mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen86m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen86m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen86m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen86m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen86m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen86m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
