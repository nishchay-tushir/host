...
const WaterLine6mSchema = new mongoose.Schema({
...
const WaterLine6m = mongoose.model('WaterLine6m', WaterLine6mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WaterLine6m.findOne({
        submissionDate: {
      ...
      const newRecord = new WaterLine6m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterLine6m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WaterLine6m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WaterLine6m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WaterLine6m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
