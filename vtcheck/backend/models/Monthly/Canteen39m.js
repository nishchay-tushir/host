...
const Canteen39mSchema = new mongoose.Schema({
...
const Canteen39m = mongoose.model('Canteen39m', Canteen39mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen39m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen39m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen39m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen39m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen39m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen39m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
