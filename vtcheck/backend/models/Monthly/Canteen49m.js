...
const Canteen49mSchema = new mongoose.Schema({
...
const Canteen49m = mongoose.model('Canteen49m', Canteen49mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen49m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen49m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen49m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen49m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen49m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen49m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
