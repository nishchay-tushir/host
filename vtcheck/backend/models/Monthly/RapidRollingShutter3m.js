...
const RapidRollingShutter3mSchema = new mongoose.Schema({
...
const RapidRollingShutter3m = mongoose.model('RapidRollingShutter3m', RapidRollingShutter3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidRollingShutter3m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidRollingShutter3m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidRollingShutter3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidRollingShutter3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidRollingShutter3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidRollingShutter3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
