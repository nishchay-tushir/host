...
const SplitAC4mSchema = new mongoose.Schema({
...
const SplitAC4m = mongoose.model('SplitAC4m', SplitAC4mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SplitAC4m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAC4m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SplitAC4m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SplitAC4m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SplitAC4m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
