...
const FireExtinguisher26mSchema = new mongoose.Schema({
...
const FireExtinguisher26m = mongoose.model('FireExtinguisher26m', FireExtinguisher26mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher26m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher26m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher26m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher26m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher26m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher26m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
