...
const FireExtinguisher118mSchema = new mongoose.Schema({
...
const FireExtinguisher118m = mongoose.model('FireExtinguisher118m', FireExtinguisher118mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher118m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher118m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher118m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher118m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher118m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher118m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
