...
const AirDryer1mSchema = new mongoose.Schema({
...
const AirDryer1m = mongoose.model('AirDryer1m', AirDryer1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AirDryer1m.findOne({
        submissionDate: {
      ...
      const newRecord = new AirDryer1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirDryer1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AirDryer1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AirDryer1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AirDryer1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
