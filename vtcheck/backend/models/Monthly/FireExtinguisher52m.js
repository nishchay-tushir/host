...
const FireExtinguisher52mSchema = new mongoose.Schema({
...
const FireExtinguisher52m = mongoose.model('FireExtinguisher52m', FireExtinguisher52mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher52m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher52m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher52m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher52m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher52m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher52m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
