...
const FireExtinguisher63mSchema = new mongoose.Schema({
...
const FireExtinguisher63m = mongoose.model('FireExtinguisher63m', FireExtinguisher63mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher63m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher63m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher63m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher63m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher63m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher63m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
