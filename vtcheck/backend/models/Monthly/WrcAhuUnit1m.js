...
const WrcAhuUnit1mSchema = new mongoose.Schema({
...
const WrcAhuUnit1m = mongoose.model('WrcAhuUnit1m', WrcAhuUnit1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WrcAhuUnit1m.findOne({
        submissionDate: {
      ...
      const newRecord = new WrcAhuUnit1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WrcAhuUnit1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WrcAhuUnit1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WrcAhuUnit1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WrcAhuUnit1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
