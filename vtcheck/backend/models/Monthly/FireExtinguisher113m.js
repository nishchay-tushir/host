...
const FireExtinguisher113mSchema = new mongoose.Schema({
...
const FireExtinguisher113m = mongoose.model('FireExtinguisher113m', FireExtinguisher113mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher113m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher113m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher113m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher113m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher113m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher113m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
