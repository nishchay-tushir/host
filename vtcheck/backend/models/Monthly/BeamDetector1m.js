...
const BeamDetector1mSchema = new mongoose.Schema({
...
const BeamDetector1m = mongoose.model('BeamDetector1m', BeamDetector1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await BeamDetector1m.findOne({
        submissionDate: {
      ...
      const newRecord = new BeamDetector1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await BeamDetector1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await BeamDetector1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await BeamDetector1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await BeamDetector1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
