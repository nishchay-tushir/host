...
const Canteen64mSchema = new mongoose.Schema({
...
const Canteen64m = mongoose.model('Canteen64m', Canteen64mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen64m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen64m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen64m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen64m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen64m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen64m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
