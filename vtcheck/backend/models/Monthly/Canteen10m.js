...
const Canteen10mSchema = new mongoose.Schema({
...
const Canteen10m = mongoose.model('Canteen10m', Canteen10mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen10m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen10m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen10m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen10m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen10m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen10m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
