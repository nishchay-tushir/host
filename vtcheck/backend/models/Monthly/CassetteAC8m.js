...
const CassetteAC1m8mSchema = new mongoose.Schema({
...
const CassetteAC1m8m = mongoose.model('CassetteAC1m8m', CassetteAC1m8mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await CassetteAC1m8m.findOne({
        submissionDate: {
      ...
      const newRecord = new CassetteAC1m8m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await CassetteAC1m8m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await CassetteAC1m8m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await CassetteAC1m8m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await CassetteAC1m8m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
