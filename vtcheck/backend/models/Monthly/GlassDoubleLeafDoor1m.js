...
const GlassDoubleLeafDoor1mSchema = new mongoose.Schema({
...
const GlassDoubleLeafDoor1m = mongoose.model('GlassDoubleLeafDoor1m', GlassDoubleLeafDoor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await GlassDoubleLeafDoor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new GlassDoubleLeafDoor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassDoubleLeafDoor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await GlassDoubleLeafDoor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await GlassDoubleLeafDoor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await GlassDoubleLeafDoor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
