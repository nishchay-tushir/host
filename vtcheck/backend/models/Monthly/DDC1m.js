...
const DDC1mSchema = new mongoose.Schema({
...
const DDC1m = mongoose.model('DDC1m', DDC1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await DDC1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await DDC1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await DDC1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await DDC1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await DDC1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
