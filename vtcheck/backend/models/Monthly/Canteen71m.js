...
const Canteen71mSchema = new mongoose.Schema({
...
const Canteen71m = mongoose.model('Canteen71m', Canteen71mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen71m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen71m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen71m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen71m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen71m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen71m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
