...
const FireExtinguisher140mSchema = new mongoose.Schema({
...
const FireExtinguisher140m = mongoose.model('FireExtinguisher140m', FireExtinguisher140mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher140m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher140m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher140m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher140m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher140m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher140m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
