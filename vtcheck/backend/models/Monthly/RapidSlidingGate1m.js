...
const RapidSlidingGate1mSchema = new mongoose.Schema({
...
const RapidSlidingGate1m = mongoose.model('RapidSlidingGate1m', RapidSlidingGate1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidSlidingGate1m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidSlidingGate1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidSlidingGate1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidSlidingGate1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidSlidingGate1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidSlidingGate1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
