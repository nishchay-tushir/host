...
const FireExtinguisher171mSchema = new mongoose.Schema({
...
const FireExtinguisher171m = mongoose.model('FireExtinguisher171m', FireExtinguisher171mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher171m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher171m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher171m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher171m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher171m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher171m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
