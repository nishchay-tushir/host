...
const Canteen89mSchema = new mongoose.Schema({
...
const Canteen89m = mongoose.model('Canteen89m', Canteen89mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen89m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen89m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen89m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen89m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen89m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen89m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
