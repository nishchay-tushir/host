...
const FireWaterLine6mSchema = new mongoose.Schema({
...
const FireWaterLine6m = mongoose.model('FireWaterLine6m', FireWaterLine6mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireWaterLine6m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireWaterLine6m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireWaterLine6m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireWaterLine6m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireWaterLine6m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireWaterLine6m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
