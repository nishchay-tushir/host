...
const DG5002mSchema = new mongoose.Schema({
...
const DG5002M = mongoose.model('DG5002M', DG5002mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await DG5002M.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await DG5002M.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await DG5002M.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await DG5002M.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await DG5002M.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
