...
const Canteen81mSchema = new mongoose.Schema({
...
const Canteen81m = mongoose.model('Canteen81m', Canteen81mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen81m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen81m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen81m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen81m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen81m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen81m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
