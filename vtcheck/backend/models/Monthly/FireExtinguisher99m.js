...
const FireExtinguisher99mSchema = new mongoose.Schema({
...
const FireExtinguisher99m = mongoose.model('FireExtinguisher99m', FireExtinguisher99mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher99m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher99m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher99m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher99m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher99m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher99m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
