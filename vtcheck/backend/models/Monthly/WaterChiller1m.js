...
const WaterChiller1mSchema = new mongoose.Schema({
...
const WaterChiller1m = mongoose.model('WaterChiller1m', WaterChiller1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WaterChiller1m.findOne({
        submissionDate: {
      ...
      const newRecord = new WaterChiller1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WaterChiller1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WaterChiller1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WaterChiller1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
