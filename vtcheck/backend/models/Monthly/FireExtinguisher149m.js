...
const FireExtinguisher149mSchema = new mongoose.Schema({
...
const FireExtinguisher149m = mongoose.model('FireExtinguisher149m', FireExtinguisher149mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher149m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher149m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher149m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher149m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher149m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher149m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
