...
const FireExtinguisher119mSchema = new mongoose.Schema({
...
const FireExtinguisher119m = mongoose.model('FireExtinguisher119m', FireExtinguisher119mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher119m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher119m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher119m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher119m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher119m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher119m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
