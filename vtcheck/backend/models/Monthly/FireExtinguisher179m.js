...
const FireExtinguisher179mSchema = new mongoose.Schema({
...
const FireExtinguisher179m = mongoose.model('FireExtinguisher179m', FireExtinguisher179mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher179m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher179m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher179m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher179m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher179m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher179m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
