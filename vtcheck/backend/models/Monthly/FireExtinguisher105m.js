...
const FireExtinguisher105mSchema = new mongoose.Schema({
...
const FireExtinguisher105m = mongoose.model('FireExtinguisher105m', FireExtinguisher105mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher105m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher105m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher105m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher105m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher105m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher105m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
