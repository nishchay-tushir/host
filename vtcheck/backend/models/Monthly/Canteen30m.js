...
const Canteen30mSchema = new mongoose.Schema({
...
const Canteen30m = mongoose.model('Canteen30m', Canteen30mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen30m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen30m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen30m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen30m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen30m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen30m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
