...
const StreetLight1m5mSchema = new mongoose.Schema({
...
const StreetLight1m5m = mongoose.model('StreetLight1m5m', StreetLight1m5mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await StreetLight1m5m.findOne({
        submissionDate: {
      ...
      const newRecord = new StreetLight1m5m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StreetLight1m5m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await StreetLight1m5m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StreetLight1m5m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StreetLight1m5m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
