...
const FireExtinguisher160mSchema = new mongoose.Schema({
...
const FireExtinguisher160m = mongoose.model('FireExtinguisher160m', FireExtinguisher160mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher160m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher160m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher160m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher160m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher160m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher160m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
