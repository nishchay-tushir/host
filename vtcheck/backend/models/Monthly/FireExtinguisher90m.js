...
const FireExtinguisher90mSchema = new mongoose.Schema({
...
const FireExtinguisher90m = mongoose.model('FireExtinguisher90m', FireExtinguisher90mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher90m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher90m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher90m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher90m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher90m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher90m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
