...
const Pump4mSchema = new mongoose.Schema({
...
const Pump4m = mongoose.model('Pump4m', Pump4mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Pump4m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Pump4m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Pump4m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pump4m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pump4m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
