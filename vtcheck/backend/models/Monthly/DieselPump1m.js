...
const DieselPump1mSchema = new mongoose.Schema({
...
const DieselPump1m = mongoose.model('DieselPump1m', DieselPump1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await DieselPump1m.findOne({
        submissionDate: {
      ...
      const newRecord = new DieselPump1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DieselPump1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await DieselPump1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await DieselPump1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await DieselPump1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
