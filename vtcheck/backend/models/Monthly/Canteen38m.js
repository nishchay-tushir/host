...
const Canteen38mSchema = new mongoose.Schema({
...
const Canteen38m = mongoose.model('Canteen38m', Canteen38mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen38m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen38m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen38m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen38m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen38m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen38m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
