...
const FireExtinguisher80mSchema = new mongoose.Schema({
...
const FireExtinguisher80m = mongoose.model('FireExtinguisher80m', FireExtinguisher80mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher80m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher80m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher80m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher80m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher80m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher80m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
