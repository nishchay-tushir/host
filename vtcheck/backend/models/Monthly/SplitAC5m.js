...
const SplitAC5mSchema = new mongoose.Schema({
...
const SplitAC5m = mongoose.model('SplitAC5m', SplitAC5mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SplitAC5m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAC5m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SplitAC5m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SplitAC5m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SplitAC5m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
