...
const Canteen45mSchema = new mongoose.Schema({
...
const Canteen45m = mongoose.model('Canteen45m', Canteen45mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen45m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen45m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen45m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen45m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen45m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen45m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
