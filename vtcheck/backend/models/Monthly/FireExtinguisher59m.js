...
const FireExtinguisher59mSchema = new mongoose.Schema({
...
const FireExtinguisher59m = mongoose.model('FireExtinguisher59m', FireExtinguisher59mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher59m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher59m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher59m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher59m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher59m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher59m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
