...
const FireExtinguisher56mSchema = new mongoose.Schema({
...
const FireExtinguisher56m = mongoose.model('FireExtinguisher56m', FireExtinguisher56mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher56m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher56m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher56m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher56m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher56m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher56m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
