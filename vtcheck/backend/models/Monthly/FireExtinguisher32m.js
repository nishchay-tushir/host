...
const FireExtinguisher32mSchema = new mongoose.Schema({
...
const FireExtinguisher32m = mongoose.model('FireExtinguisher32m', FireExtinguisher32mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher32m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher32m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher32m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher32m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher32m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher32m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
