...
const SwagonCompressor1mSchema = new mongoose.Schema({
...
const SwagonCompressor1m = mongoose.model('SwagonCompressor1m', SwagonCompressor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SwagonCompressor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new SwagonCompressor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SwagonCompressor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SwagonCompressor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SwagonCompressor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SwagonCompressor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
