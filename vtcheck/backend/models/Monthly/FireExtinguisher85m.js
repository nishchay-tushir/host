...
const FireExtinguisher85mSchema = new mongoose.Schema({
...
const FireExtinguisher85m = mongoose.model('FireExtinguisher85m', FireExtinguisher85mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher85m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher85m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher85m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher85m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher85m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher85m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
