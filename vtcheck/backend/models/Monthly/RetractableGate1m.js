...
const RetractableGate1mSchema = new mongoose.Schema({
...
const RetractableGate1m = mongoose.model('RetractableGate1m', RetractableGate1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RetractableGate1m.findOne({
        submissionDate: {
      ...
      const newRecord = new RetractableGate1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RetractableGate1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RetractableGate1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await RetractableGate1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await RetractableGate1m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
