...
const FireExtinguisher70mSchema = new mongoose.Schema({
...
const FireExtinguisher70m = mongoose.model('FireExtinguisher70m', FireExtinguisher70mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher70m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher70m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher70m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher70m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher70m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher70m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
