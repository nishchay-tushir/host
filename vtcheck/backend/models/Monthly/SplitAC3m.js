...
const SplitAC3mSchema = new mongoose.Schema({
...
const SplitAC3m = mongoose.model('SplitAC3m', SplitAC3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SplitAC3m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAC3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SplitAC3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SplitAC3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SplitAC3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
