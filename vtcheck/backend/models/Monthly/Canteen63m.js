...
const Canteen63mSchema = new mongoose.Schema({
...
const Canteen63m = mongoose.model('Canteen63m', Canteen63mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen63m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen63m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen63m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen63m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen63m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen63m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
