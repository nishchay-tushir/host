...
const Canteen11mSchema = new mongoose.Schema({
...
const Canteen11m = mongoose.model('Canteen11m', Canteen11mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen11m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen11m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen11m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen11m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen11m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen11m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
