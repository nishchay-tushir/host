...
const FireWaterLine4mSchema = new mongoose.Schema({
...
const FireWaterLine4m = mongoose.model('FireWaterLine4m', FireWaterLine4mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireWaterLine4m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireWaterLine4m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireWaterLine4m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireWaterLine4m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireWaterLine4m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireWaterLine4m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
