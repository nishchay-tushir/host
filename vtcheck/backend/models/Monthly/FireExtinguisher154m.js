...
const FireExtinguisher154mSchema = new mongoose.Schema({
...
const FireExtinguisher154m = mongoose.model('FireExtinguisher154m', FireExtinguisher154mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher154m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher154m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher154m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher154m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher154m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher154m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
