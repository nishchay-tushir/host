...
const StpBlower1mSchema = new mongoose.Schema({
...
const StpBlower1m = mongoose.model('StpBlower1m', StpBlower1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await StpBlower1m.findOne({
        submissionDate: {
      ...
      const newRecord = new StpBlower1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpBlower1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await StpBlower1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await StpBlower1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await StpBlower1m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
