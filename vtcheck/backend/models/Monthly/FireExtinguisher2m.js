...
const FireExtinguisher2mSchema = new mongoose.Schema({
...
const FireExtinguisher2m = mongoose.model('FireExtinguisher2m', FireExtinguisher2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher2m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher2m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
