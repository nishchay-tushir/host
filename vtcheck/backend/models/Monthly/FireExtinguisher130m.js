...
const FireExtinguisher130mSchema = new mongoose.Schema({
...
const FireExtinguisher130m = mongoose.model('FireExtinguisher130m', FireExtinguisher130mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher130m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher130m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher130m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher130m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher130m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher130m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
