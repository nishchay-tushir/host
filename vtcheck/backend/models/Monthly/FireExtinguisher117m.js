...
const FireExtinguisher117mSchema = new mongoose.Schema({
...
const FireExtinguisher117m = mongoose.model('FireExtinguisher117m', FireExtinguisher117mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher117m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher117m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher117m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher117m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher117m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher117m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
