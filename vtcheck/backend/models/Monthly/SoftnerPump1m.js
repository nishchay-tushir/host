...
const SoftnerPump1mSchema = new mongoose.Schema({
...
const SoftnerPump1m = mongoose.model('SoftnerPump1m', SoftnerPump1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SoftnerPump1m.findOne({
        submissionDate: {
      ...
      const newRecord = new SoftnerPump1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SoftnerPump1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SoftnerPump1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SoftnerPump1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SoftnerPump1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
