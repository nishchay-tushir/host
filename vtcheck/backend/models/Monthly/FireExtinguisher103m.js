...
const FireExtinguisher103mSchema = new mongoose.Schema({
...
const FireExtinguisher103m = mongoose.model('FireExtinguisher103m', FireExtinguisher103mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher103m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher103m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher103m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher103m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher103m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher103m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
