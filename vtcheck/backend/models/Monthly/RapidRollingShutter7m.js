...
const RapidRollingShutter7mSchema = new mongoose.Schema({
...
const RapidRollingShutter7m = mongoose.model('RapidRollingShutter7m', RapidRollingShutter7mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidRollingShutter7m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidRollingShutter7m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidRollingShutter7m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidRollingShutter7m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidRollingShutter7m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidRollingShutter7m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
