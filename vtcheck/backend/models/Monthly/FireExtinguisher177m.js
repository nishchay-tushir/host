...
const FireExtinguisher177mSchema = new mongoose.Schema({
...
const FireExtinguisher177m = mongoose.model('FireExtinguisher177m', FireExtinguisher177mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher177m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher177m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher177m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher177m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher177m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher177m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
