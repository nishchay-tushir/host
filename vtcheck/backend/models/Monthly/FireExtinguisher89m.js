...
const FireExtinguisher89mSchema = new mongoose.Schema({
...
const FireExtinguisher89m = mongoose.model('FireExtinguisher89m', FireExtinguisher89mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher89m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher89m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher89m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher89m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher89m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher89m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
