...
const FireExtinguisher106mSchema = new mongoose.Schema({
...
const FireExtinguisher106m = mongoose.model('FireExtinguisher106m', FireExtinguisher106mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher106m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher106m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher106m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher106m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher106m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher106m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
