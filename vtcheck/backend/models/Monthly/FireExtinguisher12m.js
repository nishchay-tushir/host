...
const FireExtinguisher12mSchema = new mongoose.Schema({
...
const FireExtinguisher12m = mongoose.model('FireExtinguisher12m', FireExtinguisher12mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher12m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher12m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher12m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher12m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher12m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher12m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
