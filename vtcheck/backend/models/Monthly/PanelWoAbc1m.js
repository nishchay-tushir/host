...
const PanelWoAbc1mSchema = new mongoose.Schema({
...
const PanelWoAbc1m = mongoose.model('PanelWoAbc1m', PanelWoAbc1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await PanelWoAbc1m.findOne({
        submissionDate: {
      ...
      const newRecord = new PanelWoAbc1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await PanelWoAbc1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await PanelWoAbc1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await PanelWoAbc1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await PanelWoAbc1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
