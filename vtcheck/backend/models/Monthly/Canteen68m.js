...
const Canteen68mSchema = new mongoose.Schema({
...
const Canteen68m = mongoose.model('Canteen68m', Canteen68mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen68m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen68m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen68m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen68m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen68m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen68m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
