...
const FireExtinguisher172mSchema = new mongoose.Schema({
...
const FireExtinguisher172m = mongoose.model('FireExtinguisher172m', FireExtinguisher172mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher172m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher172m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher172m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher172m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher172m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher172m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
