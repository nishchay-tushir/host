...
const AHU11mSchema = new mongoose.Schema({
...
const AHU11m = mongoose.model('AHU11m', AHU11mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU11m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU11m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU11m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU11m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU11m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
