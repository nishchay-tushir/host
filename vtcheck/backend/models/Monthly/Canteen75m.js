...
const Canteen75mSchema = new mongoose.Schema({
...
const Canteen75m = mongoose.model('Canteen75m', Canteen75mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen75m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen75m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen75m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen75m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen75m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen75m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
