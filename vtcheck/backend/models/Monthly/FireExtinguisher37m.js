...
const FireExtinguisher37mSchema = new mongoose.Schema({
...
const FireExtinguisher37m = mongoose.model('FireExtinguisher37m', FireExtinguisher37mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher37m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher37m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher37m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher37m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher37m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher37m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
