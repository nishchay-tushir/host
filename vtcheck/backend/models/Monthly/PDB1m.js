...
const PDB1mSchema = new mongoose.Schema({
...
const PDB1m = mongoose.model('PDB1m', PDB1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await PDB1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await PDB1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await PDB1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await PDB1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await PDB1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
