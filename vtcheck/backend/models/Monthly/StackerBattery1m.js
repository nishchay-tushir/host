...
const StackerBattery1mSchema = new mongoose.Schema({
...
const StackerBattery1m = mongoose.model('StackerBattery1m', StackerBattery1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await StackerBattery1m.findOne({
        submissionDate: {
      ...
      const newRecord = new StackerBattery1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StackerBattery1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await StackerBattery1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await StackerBattery1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await StackerBattery1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
