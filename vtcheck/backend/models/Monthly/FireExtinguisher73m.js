...
const FireExtinguisher73mSchema = new mongoose.Schema({
...
const FireExtinguisher73m = mongoose.model('FireExtinguisher73m', FireExtinguisher73mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher73m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher73m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher73m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher73m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher73m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher73m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
