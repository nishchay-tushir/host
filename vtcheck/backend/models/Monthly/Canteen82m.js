...
const Canteen82mSchema = new mongoose.Schema({
...
const Canteen82m = mongoose.model('Canteen82m', Canteen82mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen82m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen82m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen82m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen82m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen82m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen82m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
