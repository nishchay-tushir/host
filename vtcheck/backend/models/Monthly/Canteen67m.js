...
const Canteen67mSchema = new mongoose.Schema({
...
const Canteen67m = mongoose.model('Canteen67m', Canteen67mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen67m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen67m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen67m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen67m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen67m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen67m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
