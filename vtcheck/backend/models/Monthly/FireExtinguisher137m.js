...
const FireExtinguisher137mSchema = new mongoose.Schema({
...
const FireExtinguisher137m = mongoose.model('FireExtinguisher137m', FireExtinguisher137mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher137m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher137m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher137m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher137m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher137m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher137m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
