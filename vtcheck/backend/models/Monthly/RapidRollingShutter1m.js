...
const RapidRollingShutter1mSchema = new mongoose.Schema({
...
const RapidRollingShutter1m = mongoose.model('RapidRollingShutter1m', RapidRollingShutter1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidRollingShutter1m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidRollingShutter1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidRollingShutter1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidRollingShutter1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidRollingShutter1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidRollingShutter1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
