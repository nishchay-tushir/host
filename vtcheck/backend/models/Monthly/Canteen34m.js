...
const Canteen34mSchema = new mongoose.Schema({
...
const Canteen34m = mongoose.model('Canteen34m', Canteen34mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen34m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen34m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen34m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen34m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen34m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen34m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
