...
const AHU4mSchema = new mongoose.Schema({
...
const AHU4m = mongoose.model('AHU4m', AHU4mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU4m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU4m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU4m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU4m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU4m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
