...
const Canteen69mSchema = new mongoose.Schema({
...
const Canteen69m = mongoose.model('Canteen69m', Canteen69mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen69m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen69m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen69m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen69m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen69m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen69m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
