...
const Canteen58mSchema = new mongoose.Schema({
...
const Canteen58m = mongoose.model('Canteen58m', Canteen58mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen58m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen58m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen58m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen58m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen58m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen58m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
