...
const Panel6mSchema = new mongoose.Schema({
...
const Panel6m = mongoose.model('Panel6m', Panel6mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Panel6m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel6m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Panel6m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await Panel6m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await Panel6m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
