...
const FireExtinguisher115mSchema = new mongoose.Schema({
...
const FireExtinguisher115m = mongoose.model('FireExtinguisher115m', FireExtinguisher115mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher115m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher115m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher115m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher115m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher115m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher115m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
