...
const FireExtinguisher42mSchema = new mongoose.Schema({
...
const FireExtinguisher42m = mongoose.model('FireExtinguisher42m', FireExtinguisher42mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher42m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher42m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher42m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher42m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher42m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher42m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
