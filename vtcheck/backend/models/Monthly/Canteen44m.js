...
const Canteen44mSchema = new mongoose.Schema({
...
const Canteen44m = mongoose.model('Canteen44m', Canteen44mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen44m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen44m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen44m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen44m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen44m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen44m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
