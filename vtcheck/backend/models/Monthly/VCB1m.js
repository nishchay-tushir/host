...
const VCB1mSchema = new mongoose.Schema({
...
const VCB1m = mongoose.model('VCB1m', VCB1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await VCB1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await VCB1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await VCB1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await VCB1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await VCB1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
