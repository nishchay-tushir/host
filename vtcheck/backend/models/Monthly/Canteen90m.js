...
const Canteen90mSchema = new mongoose.Schema({
...
const Canteen90m = mongoose.model('Canteen90m', Canteen90mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen90m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen90m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen90m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen90m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen90m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen90m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
