...
const FireExtinguisher57mSchema = new mongoose.Schema({
...
const FireExtinguisher57m = mongoose.model('FireExtinguisher57m', FireExtinguisher57mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher57m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher57m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher57m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher57m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher57m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher57m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
