...
const DG10101mSchema = new mongoose.Schema({
...
const DG10101M = mongoose.model('DG10101M', DG10101mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await DG10101M.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await DG10101M.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await DG10101M.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await DG10101M.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await DG10101M.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
