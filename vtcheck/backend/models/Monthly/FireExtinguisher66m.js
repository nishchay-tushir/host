...
const FireExtinguisher66mSchema = new mongoose.Schema({
...
const FireExtinguisher66m = mongoose.model('FireExtinguisher66m', FireExtinguisher66mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher66m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher66m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher66m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher66m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher66m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher66m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
