...
const RapidSlidingGate3mSchema = new mongoose.Schema({
...
const RapidSlidingGate3m = mongoose.model('RapidSlidingGate3m', RapidSlidingGate3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidSlidingGate3m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidSlidingGate3m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidSlidingGate3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidSlidingGate3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidSlidingGate3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidSlidingGate3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
