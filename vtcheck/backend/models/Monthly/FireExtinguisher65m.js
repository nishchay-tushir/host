...
const FireExtinguisher65mSchema = new mongoose.Schema({
...
const FireExtinguisher65m = mongoose.model('FireExtinguisher65m', FireExtinguisher65mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher65m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher65m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher65m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher65m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher65m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher65m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
