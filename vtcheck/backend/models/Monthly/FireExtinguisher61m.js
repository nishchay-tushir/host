...
const FireExtinguisher61mSchema = new mongoose.Schema({
...
const FireExtinguisher61m = mongoose.model('FireExtinguisher61m', FireExtinguisher61mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher61m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher61m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher61m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher61m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher61m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher61m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
