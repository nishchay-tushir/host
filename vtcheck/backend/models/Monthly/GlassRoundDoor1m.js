...
const GlassRoundDoor1mSchema = new mongoose.Schema({
...
const GlassRoundDoor1m = mongoose.model('GlassRoundDoor1m', GlassRoundDoor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await GlassRoundDoor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new GlassRoundDoor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassRoundDoor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await GlassRoundDoor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await GlassRoundDoor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await GlassRoundDoor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
