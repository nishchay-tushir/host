...
const Panel2mSchema = new mongoose.Schema({
...
const Panel2m = mongoose.model('Panel2m', Panel2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Panel2m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Panel2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await Panel2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await Panel2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
