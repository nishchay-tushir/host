...
const RapidRollingShutter5mSchema = new mongoose.Schema({
...
const RapidRollingShutter5m = mongoose.model('RapidRollingShutter5m', RapidRollingShutter5mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidRollingShutter5m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidRollingShutter5m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidRollingShutter5m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidRollingShutter5m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidRollingShutter5m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidRollingShutter5m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
