...
const AirChiller1mSchema = new mongoose.Schema({
...
const AirChiller1m = mongoose.model('AirChiller1m', AirChiller1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AirChiller1m.findOne({
        submissionDate: {
      ...
      const newRecord = new AirChiller1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirChiller1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AirChiller1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirChiller1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirChiller1m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
