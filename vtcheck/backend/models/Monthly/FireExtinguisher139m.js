...
const FireExtinguisher139mSchema = new mongoose.Schema({
...
const FireExtinguisher139m = mongoose.model('FireExtinguisher139m', FireExtinguisher139mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher139m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher139m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher139m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher139m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher139m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher139m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
