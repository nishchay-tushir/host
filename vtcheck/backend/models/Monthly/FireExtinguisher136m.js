...
const FireExtinguisher136mSchema = new mongoose.Schema({
...
const FireExtinguisher136m = mongoose.model('FireExtinguisher136m', FireExtinguisher136mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher136m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher136m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher136m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher136m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher136m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher136m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
