...
const HooterStrobe1mSchema = new mongoose.Schema({
...
const HooterStrobe1m = mongoose.model('HooterStrobe1m', HooterStrobe1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await HooterStrobe1m.findOne({
        submissionDate: {
      ...
      const newRecord = new HooterStrobe1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HooterStrobe1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await HooterStrobe1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await HooterStrobe1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await HooterStrobe1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
