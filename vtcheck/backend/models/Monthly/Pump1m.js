...
const Pump1mSchema = new mongoose.Schema({
...
const Pump1m = mongoose.model('Pump1m', Pump1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Pump1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Pump1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Pump1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pump1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pump1m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
