...
const FireExtinguisher40mSchema = new mongoose.Schema({
...
const FireExtinguisher40m = mongoose.model('FireExtinguisher40m', FireExtinguisher40mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher40m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher40m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher40m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher40m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher40m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher40m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
