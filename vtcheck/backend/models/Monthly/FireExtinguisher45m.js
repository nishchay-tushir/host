...
const FireExtinguisher45mSchema = new mongoose.Schema({
...
const FireExtinguisher45m = mongoose.model('FireExtinguisher45m', FireExtinguisher45mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher45m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher45m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher45m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher45m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher45m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher45m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
