...
const FireWaterLine5mSchema = new mongoose.Schema({
...
const FireWaterLine5m = mongoose.model('FireWaterLine5m', FireWaterLine5mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireWaterLine5m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireWaterLine5m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireWaterLine5m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireWaterLine5m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireWaterLine5m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireWaterLine5m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
