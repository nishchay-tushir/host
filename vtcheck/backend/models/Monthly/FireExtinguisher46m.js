...
const FireExtinguisher46mSchema = new mongoose.Schema({
...
const FireExtinguisher46m = mongoose.model('FireExtinguisher46m', FireExtinguisher46mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher46m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher46m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher46m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher46m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher46m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher46m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
