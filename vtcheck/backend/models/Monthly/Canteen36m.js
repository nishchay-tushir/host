...
const Canteen36mSchema = new mongoose.Schema({
...
const Canteen36m = mongoose.model('Canteen36m', Canteen36mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen36m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen36m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen36m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen36m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen36m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen36m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
