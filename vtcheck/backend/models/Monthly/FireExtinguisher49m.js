...
const FireExtinguisher49mSchema = new mongoose.Schema({
...
const FireExtinguisher49m = mongoose.model('FireExtinguisher49m', FireExtinguisher49mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher49m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher49m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher49m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher49m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher49m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher49m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
