...
const FireExtinguisher166mSchema = new mongoose.Schema({
...
const FireExtinguisher166m = mongoose.model('FireExtinguisher166m', FireExtinguisher166mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher166m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher166m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher166m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher166m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher166m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher166m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
