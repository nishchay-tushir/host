...
const FireExtinguisher104mSchema = new mongoose.Schema({
...
const FireExtinguisher104m = mongoose.model('FireExtinguisher104m', FireExtinguisher104mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher104m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher104m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher104m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher104m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher104m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher104m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
