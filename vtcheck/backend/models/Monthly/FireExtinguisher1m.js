...
const FireExtinguisher1mSchema = new mongoose.Schema({
...
const FireExtinguisher1m = mongoose.model('FireExtinguisher1m', FireExtinguisher1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher1m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
