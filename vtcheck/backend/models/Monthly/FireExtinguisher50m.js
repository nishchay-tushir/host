...
const FireExtinguisher50mSchema = new mongoose.Schema({
...
const FireExtinguisher50m = mongoose.model('FireExtinguisher50m', FireExtinguisher50mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher50m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher50m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher50m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher50m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher50m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher50m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
