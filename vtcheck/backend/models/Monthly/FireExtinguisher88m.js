...
const FireExtinguisher88mSchema = new mongoose.Schema({
...
const FireExtinguisher88m = mongoose.model('FireExtinguisher88m', FireExtinguisher88mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher88m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher88m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher88m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher88m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher88m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher88m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
