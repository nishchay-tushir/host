...
const FireExtinguisher142mSchema = new mongoose.Schema({
...
const FireExtinguisher142m = mongoose.model('FireExtinguisher142m', FireExtinguisher142mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher142m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher142m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher142m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher142m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher142m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher142m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
