...
const Canteen65mSchema = new mongoose.Schema({
...
const Canteen65m = mongoose.model('Canteen65m', Canteen65mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen65m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen65m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen65m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen65m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen65m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen65m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
