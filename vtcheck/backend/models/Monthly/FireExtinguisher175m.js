...
const FireExtinguisher175mSchema = new mongoose.Schema({
...
const FireExtinguisher175m = mongoose.model('FireExtinguisher175m', FireExtinguisher175mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher175m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher175m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher175m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher175m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher175m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher175m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
