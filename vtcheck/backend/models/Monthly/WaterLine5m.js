...
const WaterLine5mSchema = new mongoose.Schema({
...
const WaterLine5m = mongoose.model('WaterLine5m', WaterLine5mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WaterLine5m.findOne({
        submissionDate: {
      ...
      const newRecord = new WaterLine5m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterLine5m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WaterLine5m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WaterLine5m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WaterLine5m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
