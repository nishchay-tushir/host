...
const FireExtinguisher165mSchema = new mongoose.Schema({
...
const FireExtinguisher165m = mongoose.model('FireExtinguisher165m', FireExtinguisher165mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher165m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher165m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher165m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher165m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher165m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher165m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
