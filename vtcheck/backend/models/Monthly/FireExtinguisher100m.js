...
const FireExtinguisher100mSchema = new mongoose.Schema({
...
const FireExtinguisher100m = mongoose.model('FireExtinguisher100m', FireExtinguisher100mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher100m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher100m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher100m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher100m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher100m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher100m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
