...
const FireExtinguisher162mSchema = new mongoose.Schema({
...
const FireExtinguisher162m = mongoose.model('FireExtinguisher162m', FireExtinguisher162mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher162m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher162m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher162m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher162m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher162m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher162m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
