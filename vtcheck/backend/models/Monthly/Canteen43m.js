...
const Canteen43mSchema = new mongoose.Schema({
...
const Canteen43m = mongoose.model('Canteen43m', Canteen43mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen43m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen43m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen43m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen43m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen43m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen43m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
