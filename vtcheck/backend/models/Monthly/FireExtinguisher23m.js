...
const FireExtinguisher23mSchema = new mongoose.Schema({
...
const FireExtinguisher23m = mongoose.model('FireExtinguisher23m', FireExtinguisher23mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher23m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher23m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher23m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher23m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher23m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher23m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
