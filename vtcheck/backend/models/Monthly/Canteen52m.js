...
const Canteen52mSchema = new mongoose.Schema({
...
const Canteen52m = mongoose.model('Canteen52m', Canteen52mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen52m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen52m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen52m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen52m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen52m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen52m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
