...
const FireExtinguisher127mSchema = new mongoose.Schema({
...
const FireExtinguisher127m = mongoose.model('FireExtinguisher127m', FireExtinguisher127mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher127m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher127m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher127m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher127m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher127m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher127m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
