...
const FireExtinguisher150mSchema = new mongoose.Schema({
...
const FireExtinguisher150m = mongoose.model('FireExtinguisher150m', FireExtinguisher150mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher150m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher150m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher150m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher150m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher150m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher150m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
