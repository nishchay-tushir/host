...
const Transformer1mSchema = new mongoose.Schema({
...
const Transformer1m = mongoose.model('Transformer1m', Transformer1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Transformer1m.findOne({
        submissionDate: {
      ...
      const newRecord = new Transformer1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Transformer1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Transformer1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await Transformer1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await Transformer1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
