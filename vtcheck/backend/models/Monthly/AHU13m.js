...
const AHU13mSchema = new mongoose.Schema({
...
const AHU13m = mongoose.model('AHU13m', AHU13mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU13m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU13m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU13m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU13m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU13m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
