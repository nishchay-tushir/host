...
const FireExtinguisher94mSchema = new mongoose.Schema({
...
const FireExtinguisher94m = mongoose.model('FireExtinguisher94m', FireExtinguisher94mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher94m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher94m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher94m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher94m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher94m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher94m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
