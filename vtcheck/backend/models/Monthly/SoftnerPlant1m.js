...
const SoftnerPlant1mSchema = new mongoose.Schema({
...
const SoftnerPlant1m = mongoose.model('SoftnerPlant1m', SoftnerPlant1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SoftnerPlant1m.findOne({
        submissionDate: {
      ...
      const newRecord = new SoftnerPlant1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SoftnerPlant1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SoftnerPlant1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SoftnerPlant1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SoftnerPlant1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
