...
const MAU3mSchema = new mongoose.Schema({
...
const MAU3m = mongoose.model('MAU3m', MAU3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await MAU3m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await MAU3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await MAU3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await MAU3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await MAU3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
