...
const FireExtinguisher15mSchema = new mongoose.Schema({
...
const FireExtinguisher15m = mongoose.model('FireExtinguisher15m', FireExtinguisher15mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher15m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher15m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher15m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher15m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher15m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher15m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
