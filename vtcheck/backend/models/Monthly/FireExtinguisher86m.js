...
const FireExtinguisher86mSchema = new mongoose.Schema({
...
const FireExtinguisher86m = mongoose.model('FireExtinguisher86m', FireExtinguisher86mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher86m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher86m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher86m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher86m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher86m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher86m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
