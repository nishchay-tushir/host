...
const FireExtinguisher114mSchema = new mongoose.Schema({
...
const FireExtinguisher114m = mongoose.model('FireExtinguisher114m', FireExtinguisher114mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher114m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher114m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher114m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher114m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher114m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher114m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
