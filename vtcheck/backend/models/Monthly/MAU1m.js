...
const MAU1mSchema = new mongoose.Schema({
...
const MAU1m = mongoose.model('MAU1m', MAU1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await MAU1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await MAU1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await MAU1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await MAU1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await MAU1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
