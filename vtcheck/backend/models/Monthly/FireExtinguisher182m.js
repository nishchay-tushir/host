...
const FireExtinguisher182mSchema = new mongoose.Schema({
...
const FireExtinguisher182m = mongoose.model('FireExtinguisher182m', FireExtinguisher182mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher182m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher182m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher182m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher182m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher182m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher182m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
