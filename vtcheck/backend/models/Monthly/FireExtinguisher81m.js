...
const FireExtinguisher81mSchema = new mongoose.Schema({
...
const FireExtinguisher81m = mongoose.model('FireExtinguisher81m', FireExtinguisher81mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher81m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher81m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher81m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher81m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher81m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher81m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
