...
const Canteen35mSchema = new mongoose.Schema({
...
const Canteen35m = mongoose.model('Canteen35m', Canteen35mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen35m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen35m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen35m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen35m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen35m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen35m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
