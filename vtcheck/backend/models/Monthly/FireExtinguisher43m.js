...
const FireExtinguisher43mSchema = new mongoose.Schema({
...
const FireExtinguisher43m = mongoose.model('FireExtinguisher43m', FireExtinguisher43mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher43m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher43m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher43m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher43m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher43m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher43m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
