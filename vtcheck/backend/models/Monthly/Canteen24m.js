...
const Canteen24mSchema = new mongoose.Schema({
...
const Canteen24m = mongoose.model('Canteen24m', Canteen24mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen24m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen24m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen24m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen24m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen24m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen24m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
