...
const FireExtinguisher47mSchema = new mongoose.Schema({
...
const FireExtinguisher47m = mongoose.model('FireExtinguisher47m', FireExtinguisher47mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher47m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher47m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher47m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher47m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher47m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher47m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
