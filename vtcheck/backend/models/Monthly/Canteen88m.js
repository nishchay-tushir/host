...
const Canteen88mSchema = new mongoose.Schema({
...
const Canteen88m = mongoose.model('Canteen88m', Canteen88mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen88m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen88m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen88m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen88m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen88m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen88m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
