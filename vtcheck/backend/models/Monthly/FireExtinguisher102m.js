...
const FireExtinguisher102mSchema = new mongoose.Schema({
...
const FireExtinguisher102m = mongoose.model('FireExtinguisher102m', FireExtinguisher102mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher102m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher102m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher102m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher102m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher102m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher102m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
