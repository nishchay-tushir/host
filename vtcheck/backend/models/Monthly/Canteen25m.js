...
const Canteen25mSchema = new mongoose.Schema({
...
const Canteen25m = mongoose.model('Canteen25m', Canteen25mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen25m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen25m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen25m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen25m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen25m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen25m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
