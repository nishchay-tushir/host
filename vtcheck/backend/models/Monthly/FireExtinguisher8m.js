...
const FireExtinguisher8mSchema = new mongoose.Schema({
...
const FireExtinguisher8m = mongoose.model('FireExtinguisher8m', FireExtinguisher8mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher8m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher8m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher8m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher8m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher8m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher8m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
