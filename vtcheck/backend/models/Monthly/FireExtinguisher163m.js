...
const FireExtinguisher163mSchema = new mongoose.Schema({
...
const FireExtinguisher163m = mongoose.model('FireExtinguisher163m', FireExtinguisher163mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher163m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher163m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher163m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher163m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher163m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher163m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
