...
const Canteen18mSchema = new mongoose.Schema({
...
const Canteen18m = mongoose.model('Canteen18m', Canteen18mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen18m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen18m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen18m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen18m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen18m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen18m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
