...
const Canteen32mSchema = new mongoose.Schema({
...
const Canteen32m = mongoose.model('Canteen32m', Canteen32mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen32m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen32m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen32m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen32m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen32m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen32m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
