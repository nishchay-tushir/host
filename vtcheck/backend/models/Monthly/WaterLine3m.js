...
const WaterLine3mSchema = new mongoose.Schema({
...
const WaterLine3m = mongoose.model('WaterLine3m', WaterLine3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WaterLine3m.findOne({
        submissionDate: {
      ...
      const newRecord = new WaterLine3m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterLine3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WaterLine3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WaterLine3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WaterLine3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
