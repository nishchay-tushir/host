...
const Canteen85mSchema = new mongoose.Schema({
...
const Canteen85m = mongoose.model('Canteen85m', Canteen85mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen85m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen85m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen85m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen85m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen85m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen85m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
