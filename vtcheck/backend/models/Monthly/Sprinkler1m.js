...
const Sprinkler1mSchema = new mongoose.Schema({
...
const Sprinkler1m = mongoose.model('Sprinkler1m', Sprinkler1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Sprinkler1m.findOne({
        submissionDate: {
      ...
      const newRecord = new Sprinkler1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Sprinkler1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Sprinkler1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await Sprinkler1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await Sprinkler1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
