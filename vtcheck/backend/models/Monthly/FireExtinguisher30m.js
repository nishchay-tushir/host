...
const FireExtinguisher30mSchema = new mongoose.Schema({
...
const FireExtinguisher30m = mongoose.model('FireExtinguisher30m', FireExtinguisher30mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher30m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher30m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher30m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher30m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher30m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher30m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
