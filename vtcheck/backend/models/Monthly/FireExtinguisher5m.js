...
const FireExtinguisher5mSchema = new mongoose.Schema({
...
const FireExtinguisher5m = mongoose.model('FireExtinguisher5m', FireExtinguisher5mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher5m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher5m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher5m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher5m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher5m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher5m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
