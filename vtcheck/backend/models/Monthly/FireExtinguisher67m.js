...
const FireExtinguisher67mSchema = new mongoose.Schema({
...
const FireExtinguisher67m = mongoose.model('FireExtinguisher67m', FireExtinguisher67mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher67m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher67m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher67m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher67m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher67m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher67m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
