...
const FireExtinguisher153mSchema = new mongoose.Schema({
...
const FireExtinguisher153m = mongoose.model('FireExtinguisher153m', FireExtinguisher153mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher153m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher153m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher153m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher153m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher153m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher153m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
