...
const HtLtPanel1mSchema = new mongoose.Schema({
...
const HtLtPanel1m = mongoose.model('HtLtPanel1m', HtLtPanel1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await HtLtPanel1m.findOne({
        submissionDate: {
      ...
      const newRecord = new HtLtPanel1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HtLtPanel1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await HtLtPanel1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await HtLtPanel1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await HtLtPanel1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
