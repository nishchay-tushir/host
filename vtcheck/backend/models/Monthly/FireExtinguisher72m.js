...
const FireExtinguisher72mSchema = new mongoose.Schema({
...
const FireExtinguisher72m = mongoose.model('FireExtinguisher72m', FireExtinguisher72mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher72m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher72m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher72m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher72m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher72m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher72m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
