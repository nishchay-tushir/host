...
const EmergencyDoor1mSchema = new mongoose.Schema({
...
const EmergencyDoor1m = mongoose.model('EmergencyDoor1m', EmergencyDoor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await EmergencyDoor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new EmergencyDoor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await EmergencyDoor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await EmergencyDoor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await EmergencyDoor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await EmergencyDoor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
