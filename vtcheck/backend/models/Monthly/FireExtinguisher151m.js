...
const FireExtinguisher151mSchema = new mongoose.Schema({
...
const FireExtinguisher151m = mongoose.model('FireExtinguisher151m', FireExtinguisher151mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher151m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher151m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher151m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher151m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher151m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher151m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
