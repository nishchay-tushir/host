...
const FireHydrant1mSchema = new mongoose.Schema({
...
const FireHydrant1m = mongoose.model('FireHydrant1m', FireHydrant1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireHydrant1m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireHydrant1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireHydrant1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireHydrant1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireHydrant1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireHydrant1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
