...
const AirCompressor1MSchema = new mongoose.Schema({
...
const AirCompressor1M = mongoose.model('AirCompressor1M', AirCompressor1MSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AirCompressor1M.findOne({
        submissionDate: {
      ...
      const newRecord = new AirCompressor1M({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirCompressor1M.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AirCompressor1M.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirCompressor1M.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirCompressor1M.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
