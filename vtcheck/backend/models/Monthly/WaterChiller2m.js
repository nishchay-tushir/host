...
const WaterChiller2mSchema = new mongoose.Schema({
...
const WaterChiller2m = mongoose.model('WaterChiller2m', WaterChiller2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WaterChiller2m.findOne({
        submissionDate: {
      ...
      const newRecord = new WaterChiller2m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WaterChiller2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WaterChiller2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WaterChiller2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
