...
const FireExtinguisher107mSchema = new mongoose.Schema({
...
const FireExtinguisher107m = mongoose.model('FireExtinguisher107m', FireExtinguisher107mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher107m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher107m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher107m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher107m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher107m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher107m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
