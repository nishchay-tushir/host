...
const FireExtinguisher82mSchema = new mongoose.Schema({
...
const FireExtinguisher82m = mongoose.model('FireExtinguisher82m', FireExtinguisher82mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher82m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher82m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher82m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher82m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher82m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher82m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
