...
const AHU10mSchema = new mongoose.Schema({
...
const AHU10m = mongoose.model('AHU10m', AHU10mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU10m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU10m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU10m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU10m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU10m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
