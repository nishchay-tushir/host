...
const StpPumpMotor1mSchema = new mongoose.Schema({
...
const StpPumpMotor1m = mongoose.model('StpPumpMotor1m', StpPumpMotor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await StpPumpMotor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new StpPumpMotor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await StpPumpMotor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await StpPumpMotor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await StpPumpMotor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await StpPumpMotor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
