...
const DG10102mSchema = new mongoose.Schema({
...
const DG10102M = mongoose.model('DG10102M', DG10102mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await DG10102M.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await DG10102M.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await DG10102M.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await DG10102M.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await DG10102M.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
