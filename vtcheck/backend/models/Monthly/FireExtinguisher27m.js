...
const FireExtinguisher27mSchema = new mongoose.Schema({
...
const FireExtinguisher27m = mongoose.model('FireExtinguisher27m', FireExtinguisher27mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher27m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher27m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher27m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher27m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher27m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher27m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
