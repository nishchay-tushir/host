...
const Panel4mSchema = new mongoose.Schema({
...
const Panel4m = mongoose.model('Panel4m', Panel4mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Panel4m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel4m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Panel4m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await Panel4m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await Panel4m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
