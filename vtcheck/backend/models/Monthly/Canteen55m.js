...
const Canteen55mSchema = new mongoose.Schema({
...
const Canteen55m = mongoose.model('Canteen55m', Canteen55mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen55m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen55m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen55m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen55m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen55m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen55m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
