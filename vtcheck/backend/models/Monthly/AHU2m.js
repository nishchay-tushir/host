...
const AHU2mSchema = new mongoose.Schema({
...
const AHU2m = mongoose.model('AHU2m', AHU2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU2m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
