...
const FireExtinguisher22mSchema = new mongoose.Schema({
...
const FireExtinguisher22m = mongoose.model('FireExtinguisher22m', FireExtinguisher22mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher22m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher22m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher22m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher22m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher22m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher22m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
