...
const FireExtinguisher44mSchema = new mongoose.Schema({
...
const FireExtinguisher44m = mongoose.model('FireExtinguisher44m', FireExtinguisher44mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher44m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher44m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher44m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher44m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher44m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher44m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
