...
const SplitAC1mSchema = new mongoose.Schema({
...
const SplitAC1m = mongoose.model('SplitAC1m', SplitAC1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SplitAC1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAC1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SplitAC1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SplitAC1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SplitAC1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
