...
const AHU8mSchema = new mongoose.Schema({
...
const AHU8m = mongoose.model('AHU8m', AHU8mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU8m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU8m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU8m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU8m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU8m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
