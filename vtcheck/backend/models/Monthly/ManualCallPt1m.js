...
const ManualCallPt1mSchema = new mongoose.Schema({
...
const ManualCallPt1m = mongoose.model('ManualCallPt1m', ManualCallPt1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await ManualCallPt1m.findOne({
        submissionDate: {
      ...
      const newRecord = new ManualCallPt1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await ManualCallPt1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await ManualCallPt1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await ManualCallPt1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await ManualCallPt1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
