...
const FRPdoor1mSchema = new mongoose.Schema({
...
const FRPdoor1m = mongoose.model('FRPdoor1m', FRPdoor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FRPdoor1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await FRPdoor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FRPdoor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FRPdoor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FRPdoor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
