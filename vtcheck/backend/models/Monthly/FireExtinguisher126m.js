...
const FireExtinguisher126mSchema = new mongoose.Schema({
...
const FireExtinguisher126m = mongoose.model('FireExtinguisher126m', FireExtinguisher126mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher126m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher126m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher126m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher126m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher126m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher126m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
