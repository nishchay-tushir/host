...
const WaterLine4mSchema = new mongoose.Schema({
...
const WaterLine4m = mongoose.model('WaterLine4m', WaterLine4mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WaterLine4m.findOne({
        submissionDate: {
      ...
      const newRecord = new WaterLine4m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterLine4m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WaterLine4m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WaterLine4m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WaterLine4m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
