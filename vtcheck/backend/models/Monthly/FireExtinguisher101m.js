...
const FireExtinguisher101mSchema = new mongoose.Schema({
...
const FireExtinguisher101m = mongoose.model('FireExtinguisher101m', FireExtinguisher101mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher101m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher101m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher101m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher101m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher101m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher101m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
