...
const Canteen50mSchema = new mongoose.Schema({
...
const Canteen50m = mongoose.model('Canteen50m', Canteen50mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen50m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen50m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen50m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen50m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen50m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen50m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
