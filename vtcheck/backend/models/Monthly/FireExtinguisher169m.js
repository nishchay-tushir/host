...
const FireExtinguisher169mSchema = new mongoose.Schema({
...
const FireExtinguisher169m = mongoose.model('FireExtinguisher169m', FireExtinguisher169mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher169m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher169m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher169m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher169m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher169m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher169m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
