...
const Canteen83mSchema = new mongoose.Schema({
...
const Canteen83m = mongoose.model('Canteen83m', Canteen83mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen83m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen83m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen83m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen83m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen83m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen83m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
