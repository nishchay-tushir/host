...
const Canteen14mSchema = new mongoose.Schema({
...
const Canteen14m = mongoose.model('Canteen14m', Canteen14mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen14m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen14m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen14m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen14m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen14m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen14m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
