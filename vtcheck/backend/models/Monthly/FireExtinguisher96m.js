...
const FireExtinguisher96mSchema = new mongoose.Schema({
...
const FireExtinguisher96m = mongoose.model('FireExtinguisher96m', FireExtinguisher96mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher96m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher96m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher96m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher96m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher96m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher96m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
