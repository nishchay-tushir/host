...
const FireExtinguisher29mSchema = new mongoose.Schema({
...
const FireExtinguisher29m = mongoose.model('FireExtinguisher29m', FireExtinguisher29mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher29m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher29m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher29m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher29m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher29m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher29m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
