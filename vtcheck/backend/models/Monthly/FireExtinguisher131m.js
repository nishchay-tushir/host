...
const FireExtinguisher131mSchema = new mongoose.Schema({
...
const FireExtinguisher131m = mongoose.model('FireExtinguisher131m', FireExtinguisher131mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher131m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher131m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher131m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher131m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher131m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher131m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
