...
const GardenPump1mSchema = new mongoose.Schema({
...
const GardenPump1m = mongoose.model('GardenPump1m', GardenPump1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await GardenPump1m.findOne({
        submissionDate: {
      ...
      const newRecord = new GardenPump1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GardenPump1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await GardenPump1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await GardenPump1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await GardenPump1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
