...
const FireExtinguisher155mSchema = new mongoose.Schema({
...
const FireExtinguisher155m = mongoose.model('FireExtinguisher155m', FireExtinguisher155mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher155m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher155m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher155m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher155m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher155m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher155m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
