...
const FireExtinguisher7mSchema = new mongoose.Schema({
...
const FireExtinguisher7m = mongoose.model('FireExtinguisher7m', FireExtinguisher7mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher7m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher7m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher7m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher7m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher7m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher7m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
