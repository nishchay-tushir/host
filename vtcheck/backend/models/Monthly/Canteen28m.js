...
const Canteen28mSchema = new mongoose.Schema({
...
const Canteen28m = mongoose.model('Canteen28m', Canteen28mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen28m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen28m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen28m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen28m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen28m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen28m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
