...
const Canteen77mSchema = new mongoose.Schema({
...
const Canteen77m = mongoose.model('Canteen77m', Canteen77mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen77m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen77m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen77m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen77m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen77m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen77m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
