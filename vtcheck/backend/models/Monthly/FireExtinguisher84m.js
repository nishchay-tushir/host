...
const FireExtinguisher84mSchema = new mongoose.Schema({
...
const FireExtinguisher84m = mongoose.model('FireExtinguisher84m', FireExtinguisher84mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher84m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher84m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher84m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher84m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher84m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher84m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
