...
const AHU3mSchema = new mongoose.Schema({
...
const AHU3m = mongoose.model('AHU3m', AHU3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU3m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
