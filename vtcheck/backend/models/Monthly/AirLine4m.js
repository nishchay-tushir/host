...
const AirLine4mSchema = new mongoose.Schema({
...
const AirLine4m = mongoose.model('AirLine4m', AirLine4mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AirLine4m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AirLine4m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AirLine4m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AirLine4m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AirLine4m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
