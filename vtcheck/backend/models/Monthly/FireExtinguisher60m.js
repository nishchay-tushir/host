...
const FireExtinguisher60mSchema = new mongoose.Schema({
...
const FireExtinguisher60m = mongoose.model('FireExtinguisher60m', FireExtinguisher60mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher60m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher60m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher60m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher60m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher60m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher60m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
