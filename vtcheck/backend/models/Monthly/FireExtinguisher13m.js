...
const FireExtinguisher13mSchema = new mongoose.Schema({
...
const FireExtinguisher13m = mongoose.model('FireExtinguisher13m', FireExtinguisher13mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher13m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher13m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher13m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher13m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher13m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher13m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
