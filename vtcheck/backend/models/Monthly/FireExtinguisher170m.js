...
const FireExtinguisher170mSchema = new mongoose.Schema({
...
const FireExtinguisher170m = mongoose.model('FireExtinguisher170m', FireExtinguisher170mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher170m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher170m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher170m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher170m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher170m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher170m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
