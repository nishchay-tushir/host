...
const Canteen20mSchema = new mongoose.Schema({
...
const Canteen20m = mongoose.model('Canteen20m', Canteen20mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen20m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen20m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen20m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen20m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen20m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen20m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
