...
const FireExtinguisher11mSchema = new mongoose.Schema({
...
const FireExtinguisher11m = mongoose.model('FireExtinguisher11m', FireExtinguisher11mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher11m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher11m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher11m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher11m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher11m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher11m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
