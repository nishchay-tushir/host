...
const RapidRollingShutter9mSchema = new mongoose.Schema({
...
const RapidRollingShutter9m = mongoose.model('RapidRollingShutter9m', RapidRollingShutter9mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidRollingShutter9m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidRollingShutter9m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidRollingShutter9m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidRollingShutter9m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidRollingShutter9m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidRollingShutter9m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
