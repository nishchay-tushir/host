...
const FireExtinguisher161mSchema = new mongoose.Schema({
...
const FireExtinguisher161m = mongoose.model('FireExtinguisher161m', FireExtinguisher161mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher161m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher161m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher161m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher161m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher161m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher161m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
