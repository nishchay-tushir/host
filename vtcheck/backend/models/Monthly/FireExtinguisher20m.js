...
const FireExtinguisher20mSchema = new mongoose.Schema({
...
const FireExtinguisher20m = mongoose.model('FireExtinguisher20m', FireExtinguisher20mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher20m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher20m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher20m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher20m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher20m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher20m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
