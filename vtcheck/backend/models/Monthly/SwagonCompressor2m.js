...
const SwagonCompressor2mSchema = new mongoose.Schema({
...
const SwagonCompressor2m = mongoose.model('SwagonCompressor2m', SwagonCompressor2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SwagonCompressor2m.findOne({
        submissionDate: {
      ...
      const newRecord = new SwagonCompressor2m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SwagonCompressor2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SwagonCompressor2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SwagonCompressor2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SwagonCompressor2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
