...
const FireExtinguisher10mSchema = new mongoose.Schema({
...
const FireExtinguisher10m = mongoose.model('FireExtinguisher10m', FireExtinguisher10mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher10m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher10m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher10m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher10m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher10m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher10m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
