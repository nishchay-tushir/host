...
const AHU7mSchema = new mongoose.Schema({
...
const AHU7m = mongoose.model('AHU7m', AHU7mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU7m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU7m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU7m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU7m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU7m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
