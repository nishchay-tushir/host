...
const Canteen57mSchema = new mongoose.Schema({
...
const Canteen57m = mongoose.model('Canteen57m', Canteen57mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen57m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen57m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen57m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen57m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen57m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen57m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
