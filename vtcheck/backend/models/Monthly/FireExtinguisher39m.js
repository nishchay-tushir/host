...
const FireExtinguisher39mSchema = new mongoose.Schema({
...
const FireExtinguisher39m = mongoose.model('FireExtinguisher39m', FireExtinguisher39mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher39m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher39m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher39m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher39m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher39m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher39m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
