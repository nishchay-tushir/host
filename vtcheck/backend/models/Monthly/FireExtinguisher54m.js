...
const FireExtinguisher54mSchema = new mongoose.Schema({
...
const FireExtinguisher54m = mongoose.model('FireExtinguisher54m', FireExtinguisher54mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher54m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher54m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher54m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher54m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher54m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher54m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
