...
const FireExtinguisher152mSchema = new mongoose.Schema({
...
const FireExtinguisher152m = mongoose.model('FireExtinguisher152m', FireExtinguisher152mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher152m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher152m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher152m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher152m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher152m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher152m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
