...
const FireExtinguisher121mSchema = new mongoose.Schema({
...
const FireExtinguisher121m = mongoose.model('FireExtinguisher121m', FireExtinguisher121mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher121m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher121m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher121m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher121m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher121m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher121m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
