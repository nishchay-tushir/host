...
const AHU9mSchema = new mongoose.Schema({
...
const AHU9m = mongoose.model('AHU9m', AHU9mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU9m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU9m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU9m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU9m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU9m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
