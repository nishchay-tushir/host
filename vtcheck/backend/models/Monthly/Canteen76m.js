...
const Canteen76mSchema = new mongoose.Schema({
...
const Canteen76m = mongoose.model('Canteen76m', Canteen76mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen76m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen76m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen76m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen76m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen76m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen76m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
