...
const FireExtinguisher68mSchema = new mongoose.Schema({
...
const FireExtinguisher68m = mongoose.model('FireExtinguisher68m', FireExtinguisher68mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher68m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher68m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher68m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher68m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher68m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher68m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
