...
const AirLine2mSchema = new mongoose.Schema({
...
const AirLine2m = mongoose.model('AirLine2m', AirLine2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AirLine2m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AirLine2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AirLine2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AirLine2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AirLine2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
