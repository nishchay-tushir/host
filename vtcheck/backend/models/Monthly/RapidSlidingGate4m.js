...
const RapidSlidingGate4mSchema = new mongoose.Schema({
...
const RapidSlidingGate4m = mongoose.model('RapidSlidingGate4m', RapidSlidingGate4mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await RapidSlidingGate4m.findOne({
        submissionDate: {
      ...
      const newRecord = new RapidSlidingGate4m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await RapidSlidingGate4m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await RapidSlidingGate4m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await RapidSlidingGate4m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await RapidSlidingGate4m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
