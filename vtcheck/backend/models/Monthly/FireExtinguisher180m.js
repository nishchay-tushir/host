...
const FireExtinguisher180mSchema = new mongoose.Schema({
...
const FireExtinguisher180m = mongoose.model('FireExtinguisher180m', FireExtinguisher180mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher180m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher180m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher180m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher180m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher180m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher180m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
