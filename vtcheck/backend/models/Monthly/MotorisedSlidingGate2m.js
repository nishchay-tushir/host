...
const MotorisedSlidingGate2mSchema = new mongoose.Schema({
...
const MotorisedSlidingGate2m = mongoose.model('MotorisedSlidingGate2m', MotorisedSlidingGate2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await MotorisedSlidingGate2m.findOne({
        submissionDate: {
      ...
      const newRecord = new MotorisedSlidingGate2m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MotorisedSlidingGate2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await MotorisedSlidingGate2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await MotorisedSlidingGate2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await MotorisedSlidingGate2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
