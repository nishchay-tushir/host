...
const Canteen60mSchema = new mongoose.Schema({
...
const Canteen60m = mongoose.model('Canteen60m', Canteen60mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen60m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen60m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen60m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen60m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen60m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen60m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
