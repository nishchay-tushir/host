...
const HoseReel1mSchema = new mongoose.Schema({
...
const HoseReel1m = mongoose.model('HoseReel1m', HoseReel1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await HoseReel1m.findOne({
        submissionDate: {
      ...
      const newRecord = new HoseReel1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await HoseReel1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await HoseReel1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await HoseReel1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await HoseReel1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
