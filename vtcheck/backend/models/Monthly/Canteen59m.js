...
const Canteen59mSchema = new mongoose.Schema({
...
const Canteen59m = mongoose.model('Canteen59m', Canteen59mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen59m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen59m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen59m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen59m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen59m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen59m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
