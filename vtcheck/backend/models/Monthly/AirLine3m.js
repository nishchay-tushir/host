...
const AirLine3mSchema = new mongoose.Schema({
...
const AirLine3m = mongoose.model('AirLine3m', AirLine3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AirLine3m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AirLine3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AirLine3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AirLine3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AirLine3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
