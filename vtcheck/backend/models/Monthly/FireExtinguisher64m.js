...
const FireExtinguisher64mSchema = new mongoose.Schema({
...
const FireExtinguisher64m = mongoose.model('FireExtinguisher64m', FireExtinguisher64mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher64m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher64m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher64m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher64m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher64m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher64m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
