...
const EmergencyDoor2mSchema = new mongoose.Schema({
...
const EmergencyDoor2m = mongoose.model('EmergencyDoor2m', EmergencyDoor2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await EmergencyDoor2m.findOne({
        submissionDate: {
      ...
      const newRecord = new EmergencyDoor2m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await EmergencyDoor2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await EmergencyDoor2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await EmergencyDoor2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await EmergencyDoor2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
