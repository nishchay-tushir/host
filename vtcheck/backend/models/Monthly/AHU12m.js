...
const AHU12mSchema = new mongoose.Schema({
...
const AHU12m = mongoose.model('AHU12m', AHU12mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AHU12m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await AHU12m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AHU12m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AHU12m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AHU12m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
