...
const AccWaterMakeup1mSchema = new mongoose.Schema({
...
const AccWaterMakeup1m = mongoose.model('AccWaterMakeup1m', AccWaterMakeup1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await AccWaterMakeup1m.findOne({
        submissionDate: {
      ...
      const newRecord = new AccWaterMakeup1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AccWaterMakeup1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await AccWaterMakeup1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await AccWaterMakeup1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await AccWaterMakeup1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
