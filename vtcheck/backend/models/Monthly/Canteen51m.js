...
const Canteen51mSchema = new mongoose.Schema({
...
const Canteen51m = mongoose.model('Canteen51m', Canteen51mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen51m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen51m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen51m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen51m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen51m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen51m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
