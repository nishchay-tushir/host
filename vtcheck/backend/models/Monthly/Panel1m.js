...
const Panel1mSchema = new mongoose.Schema({
...
const Panel1m = mongoose.model('Panel1m', Panel1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Panel1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Panel1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Panel1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await Panel1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await Panel1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
