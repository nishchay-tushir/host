...
const FireExtinguisher108mSchema = new mongoose.Schema({
...
const FireExtinguisher108m = mongoose.model('FireExtinguisher108m', FireExtinguisher108mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher108m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher108m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher108m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher108m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher108m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher108m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
