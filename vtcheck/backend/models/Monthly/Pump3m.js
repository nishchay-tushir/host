...
const Pump3mSchema = new mongoose.Schema({
...
const Pump3m = mongoose.model('Pump3m', Pump3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Pump3m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Pump3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Pump3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Pump3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Pump3m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
