...
const GlassSingleLeafDoor1mSchema = new mongoose.Schema({
...
const GlassSingleLeafDoor1m = mongoose.model('GlassSingleLeafDoor1m', GlassSingleLeafDoor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await GlassSingleLeafDoor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new GlassSingleLeafDoor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await GlassSingleLeafDoor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await GlassSingleLeafDoor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await GlassSingleLeafDoor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await GlassSingleLeafDoor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
