...
const FireExtinguisher24mSchema = new mongoose.Schema({
...
const FireExtinguisher24m = mongoose.model('FireExtinguisher24m', FireExtinguisher24mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher24m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher24m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher24m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher24m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher24m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher24m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
