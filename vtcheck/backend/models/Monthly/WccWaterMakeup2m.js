...
const WccWaterMakeup2mSchema = new mongoose.Schema({
...
const WccWaterMakeup2m = mongoose.model('WccWaterMakeup2m', WccWaterMakeup2mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WccWaterMakeup2m.findOne({
        submissionDate: {
      ...
      const newRecord = new WccWaterMakeup2m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WccWaterMakeup2m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WccWaterMakeup2m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WccWaterMakeup2m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WccWaterMakeup2m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
