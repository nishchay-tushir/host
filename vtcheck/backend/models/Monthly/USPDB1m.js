...
const USPDB1mSchema = new mongoose.Schema({
...
const USPDB1m = mongoose.model('USPDB1m', USPDB1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await USPDB1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await USPDB1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await USPDB1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await USPDB1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await USPDB1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
