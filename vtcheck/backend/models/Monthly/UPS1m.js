...
const UPS1mSchema = new mongoose.Schema({
...
const UPS1m = mongoose.model('UPS1m', UPS1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await UPS1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await UPS1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await UPS1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await UPS1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await UPS1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
