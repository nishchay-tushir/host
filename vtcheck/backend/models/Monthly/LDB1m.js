...
const LDB1mSchema = new mongoose.Schema({
...
const LDB1m = mongoose.model('LDB1m', LDB1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await LDB1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await LDB1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await LDB1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await LDB1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await LDB1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
