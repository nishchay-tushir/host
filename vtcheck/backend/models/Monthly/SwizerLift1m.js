...
const SwizerLift1mSchema = new mongoose.Schema({
...
const SwizerLift1m = mongoose.model('SwizerLift1m', SwizerLift1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SwizerLift1m.findOne({
        submissionDate: {
      ...
      const newRecord = new SwizerLift1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SwizerLift1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SwizerLift1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await SwizerLift1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await SwizerLift1m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
