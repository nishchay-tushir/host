...
const Canteen70mSchema = new mongoose.Schema({
...
const Canteen70m = mongoose.model('Canteen70m', Canteen70mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen70m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen70m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen70m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen70m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen70m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen70m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
