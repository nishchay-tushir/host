...
const FireExtinguisher147mSchema = new mongoose.Schema({
...
const FireExtinguisher147m = mongoose.model('FireExtinguisher147m', FireExtinguisher147mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher147m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher147m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher147m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher147m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher147m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher147m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
