...
const Stacker1mSchema = new mongoose.Schema({
...
const Stacker1m = mongoose.model('Stacker1m', Stacker1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Stacker1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await Stacker1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Stacker1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await Stacker1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await Stacker1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
