...
const FireExtinguisher76mSchema = new mongoose.Schema({
...
const FireExtinguisher76m = mongoose.model('FireExtinguisher76m', FireExtinguisher76mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher76m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher76m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher76m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher76m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher76m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher76m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
