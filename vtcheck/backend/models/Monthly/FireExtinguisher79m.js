...
const FireExtinguisher79mSchema = new mongoose.Schema({
...
const FireExtinguisher79m = mongoose.model('FireExtinguisher79m', FireExtinguisher79mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher79m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher79m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher79m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher79m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher79m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher79m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
