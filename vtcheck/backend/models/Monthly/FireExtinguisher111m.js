...
const FireExtinguisher111mSchema = new mongoose.Schema({
...
const FireExtinguisher111m = mongoose.model('FireExtinguisher111m', FireExtinguisher111mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher111m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher111m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher111m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher111m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher111m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher111m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
