...
const Canteen84mSchema = new mongoose.Schema({
...
const Canteen84m = mongoose.model('Canteen84m', Canteen84mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen84m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen84m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen84m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen84m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen84m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen84m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
