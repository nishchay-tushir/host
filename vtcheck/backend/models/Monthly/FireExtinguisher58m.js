...
const FireExtinguisher58mSchema = new mongoose.Schema({
...
const FireExtinguisher58m = mongoose.model('FireExtinguisher58m', FireExtinguisher58mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher58m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher58m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher58m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher58m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher58m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher58m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
