...
const FireExtinguisher167mSchema = new mongoose.Schema({
...
const FireExtinguisher167m = mongoose.model('FireExtinguisher167m', FireExtinguisher167mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher167m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher167m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher167m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher167m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher167m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher167m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
