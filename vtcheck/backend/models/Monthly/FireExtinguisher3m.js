...
const FireExtinguisher3mSchema = new mongoose.Schema({
...
const FireExtinguisher3m = mongoose.model('FireExtinguisher3m', FireExtinguisher3mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher3m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher3m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher3m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher3m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher3m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher3m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
