...
const MotorisedSlidingGate1mSchema = new mongoose.Schema({
...
const MotorisedSlidingGate1m = mongoose.model('MotorisedSlidingGate1m', MotorisedSlidingGate1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await MotorisedSlidingGate1m.findOne({
        submissionDate: {
      ...
      const newRecord = new MotorisedSlidingGate1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MotorisedSlidingGate1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await MotorisedSlidingGate1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await MotorisedSlidingGate1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await MotorisedSlidingGate1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
