...
const WccWaterMakeup1mSchema = new mongoose.Schema({
...
const WccWaterMakeup1m = mongoose.model('WccWaterMakeup1m', WccWaterMakeup1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await WccWaterMakeup1m.findOne({
        submissionDate: {
      ...
      const newRecord = new WccWaterMakeup1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WccWaterMakeup1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await WccWaterMakeup1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await WccWaterMakeup1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await WccWaterMakeup1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
