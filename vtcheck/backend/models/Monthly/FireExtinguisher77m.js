...
const FireExtinguisher77mSchema = new mongoose.Schema({
...
const FireExtinguisher77m = mongoose.model('FireExtinguisher77m', FireExtinguisher77mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher77m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher77m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher77m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher77m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher77m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher77m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
