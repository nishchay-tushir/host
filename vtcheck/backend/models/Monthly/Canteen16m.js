...
const Canteen16mSchema = new mongoose.Schema({
...
const Canteen16m = mongoose.model('Canteen16m', Canteen16mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen16m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen16m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen16m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen16m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen16m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen16m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
