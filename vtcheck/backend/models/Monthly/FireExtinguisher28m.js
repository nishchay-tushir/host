...
const FireExtinguisher28mSchema = new mongoose.Schema({
...
const FireExtinguisher28m = mongoose.model('FireExtinguisher28m', FireExtinguisher28mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher28m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher28m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher28m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher28m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher28m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher28m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
