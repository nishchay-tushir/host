...
const Canteen80mSchema = new mongoose.Schema({
...
const Canteen80m = mongoose.model('Canteen80m', Canteen80mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen80m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen80m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen80m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen80m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen80m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen80m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
