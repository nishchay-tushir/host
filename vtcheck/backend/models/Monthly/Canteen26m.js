...
const Canteen26mSchema = new mongoose.Schema({
...
const Canteen26m = mongoose.model('Canteen26m', Canteen26mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen26m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen26m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen26m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen26m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen26m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen26m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
