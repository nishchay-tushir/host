...
const FireExtinguisher128mSchema = new mongoose.Schema({
...
const FireExtinguisher128m = mongoose.model('FireExtinguisher128m', FireExtinguisher128mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher128m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher128m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher128m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher128m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher128m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher128m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
