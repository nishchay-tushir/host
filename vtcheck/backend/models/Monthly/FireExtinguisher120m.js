...
const FireExtinguisher120mSchema = new mongoose.Schema({
...
const FireExtinguisher120m = mongoose.model('FireExtinguisher120m', FireExtinguisher120mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher120m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher120m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher120m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher120m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher120m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher120m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
