...
const SplitAC6mSchema = new mongoose.Schema({
...
const SplitAC6m = mongoose.model('SplitAC6m', SplitAC6mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SplitAC6m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await SplitAC6m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SplitAC6m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SplitAC6m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SplitAC6m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
