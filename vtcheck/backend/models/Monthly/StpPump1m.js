...
const StpPump1mSchema = new mongoose.Schema({
...
const StpPump1m = mongoose.model('StpPump1m', StpPump1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await StpPump1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await StpPump1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await StpPump1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await StpPump1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await StpPump1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
