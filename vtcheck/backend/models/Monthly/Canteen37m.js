...
const Canteen37mSchema = new mongoose.Schema({
...
const Canteen37m = mongoose.model('Canteen37m', Canteen37mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen37m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen37m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen37m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen37m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen37m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen37m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
