...
const SmokeDetector1mSchema = new mongoose.Schema({
...
const SmokeDetector1m = mongoose.model('SmokeDetector1m', SmokeDetector1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await SmokeDetector1m.findOne({
        submissionDate: {
      ...
      const newRecord = new SmokeDetector1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await SmokeDetector1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await SmokeDetector1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await SmokeDetector1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await SmokeDetector1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
