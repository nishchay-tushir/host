...
const Canteen33mSchema = new mongoose.Schema({
...
const Canteen33m = mongoose.model('Canteen33m', Canteen33mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen33m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen33m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen33m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen33m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen33m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen33m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
