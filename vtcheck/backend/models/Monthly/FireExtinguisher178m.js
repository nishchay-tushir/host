...
const FireExtinguisher178mSchema = new mongoose.Schema({
...
const FireExtinguisher178m = mongoose.model('FireExtinguisher178m', FireExtinguisher178mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher178m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher178m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher178m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher178m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher178m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher178m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
