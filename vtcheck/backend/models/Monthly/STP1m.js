...
const STP1mSchema = new mongoose.Schema({
...
const STP1m = mongoose.model('STP1m', STP1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await STP1m.findOne({
        submissionDate: {
...
router.get('/', async (req, res) => {
  try {
    const records = await STP1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await STP1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await STP1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await STP1m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
