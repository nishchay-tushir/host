...
const Canteen53mSchema = new mongoose.Schema({
...
const Canteen53m = mongoose.model('Canteen53m', Canteen53mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await Canteen53m.findOne({
        submissionDate: {
      ...
      const newRecord = new Canteen53m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await Canteen53m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await Canteen53m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await Canteen53m.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await Canteen53m.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
