...
const MetalSingleLeafDoor1mSchema = new mongoose.Schema({
...
const MetalSingleLeafDoor1m = mongoose.model('MetalSingleLeafDoor1m', MetalSingleLeafDoor1mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await MetalSingleLeafDoor1m.findOne({
        submissionDate: {
      ...
      const newRecord = new MetalSingleLeafDoor1m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await MetalSingleLeafDoor1m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await MetalSingleLeafDoor1m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await MetalSingleLeafDoor1m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await MetalSingleLeafDoor1m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
