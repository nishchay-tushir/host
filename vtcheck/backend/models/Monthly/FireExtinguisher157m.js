...
const FireExtinguisher157mSchema = new mongoose.Schema({
...
const FireExtinguisher157m = mongoose.model('FireExtinguisher157m', FireExtinguisher157mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher157m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher157m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher157m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher157m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher157m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher157m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
