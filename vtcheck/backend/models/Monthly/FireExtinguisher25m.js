...
const FireExtinguisher25mSchema = new mongoose.Schema({
...
const FireExtinguisher25m = mongoose.model('FireExtinguisher25m', FireExtinguisher25mSchema);
...
router.post('/', async (req, res) => {
    try {
      ...
      const existingRecord = await FireExtinguisher25m.findOne({
        submissionDate: {
      ...
      const newRecord = new FireExtinguisher25m({
        ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await FireExtinguisher25m.find();
...
router.get('/status', async (req, res) => {
    try {
      ...
      const record = await FireExtinguisher25m.findOne({
        submissionDate: {
          $gte: new Date(currentYear, currentMonth, 1), // Start of the month
          $lt: new Date(currentYear, currentMonth + 1, 1) // Start of next month
        }
...
router.put('/:id', async (req, res) => {
    try {
      const updatedRecord = await FireExtinguisher25m.findByIdAndUpdate(req.params.id, req.body, { new: true });
      if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
    try {
      const deletedRecord = await FireExtinguisher25m.findByIdAndDelete(req.params.id);
      if (deletedRecord) {
