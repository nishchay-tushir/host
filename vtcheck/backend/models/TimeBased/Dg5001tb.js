...
const DG5001tbSchema = new mongoose.Schema({
...
const DG5001tb = mongoose.model('DG5001tb', DG5001tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DG5001tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await DG5001tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG5001tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG5001tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
