...
const AirCompressor1tbSchema = new mongoose.Schema({
...
const AirCompressor1tb = mongoose.model('AirCompressor1tb', AirCompressor1tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await AirCompressor1tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new AirCompressor1tb({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirCompressor1tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirCompressor1tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirCompressor1tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
