...
const DG10102tbSchema = new mongoose.Schema({
...
const DG10102tb = mongoose.model('DG10102tb', DG10102tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DG10102tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await DG10102tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG10102tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG10102tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
