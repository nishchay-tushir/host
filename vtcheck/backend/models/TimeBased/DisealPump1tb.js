...
const DisealPump1tbSchema = new mongoose.Schema({
...
const DisealPump1tb = mongoose.model('DisealPump1tb', DisealPump1tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DisealPump1tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new DisealPump1tb({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await DisealPump1tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DisealPump1tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DisealPump1tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
