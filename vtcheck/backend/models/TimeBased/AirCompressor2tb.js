...
const AirCompressor2tbSchema = new mongoose.Schema({
...
const AirCompressor2tb = mongoose.model('AirCompressor2tb', AirCompressor2tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await AirCompressor2tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new AirCompressor2tb({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirCompressor2tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirCompressor2tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirCompressor2tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
