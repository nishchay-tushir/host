...
const AirDryer2tbSchema = new mongoose.Schema({
...
const AirDryer2tb = mongoose.model('AirDryer2tb', AirDryer2tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await AirDryer2tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new AirDryer2tb({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirDryer2tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirDryer2tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirDryer2tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
