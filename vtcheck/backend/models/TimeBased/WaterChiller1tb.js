...
const WaterChiller1tbSchema = new mongoose.Schema({
...
const WaterChiller1tb = mongoose.model('WaterChiller1tb', WaterChiller1tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WaterChiller1tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WaterChiller1tb({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller1tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WaterChiller1tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WaterChiller1tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
