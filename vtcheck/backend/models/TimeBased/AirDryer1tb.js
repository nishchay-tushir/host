...
const AirDryer1tbSchema = new mongoose.Schema({
...
const AirDryer1tb = mongoose.model('AirDryer1tb', AirDryer1tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await AirDryer1tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new AirDryer1tb({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await AirDryer1tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await AirDryer1tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await AirDryer1tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
