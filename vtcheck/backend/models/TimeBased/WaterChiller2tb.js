...
const WaterChiller2tbSchema = new mongoose.Schema({
...
const WaterChiller2tb = mongoose.model('WaterChiller2tb', WaterChiller2tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await WaterChiller2tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
      ...
      const newRecord = new WaterChiller2tb({
          ...req.body,
...
router.get('/', async (req, res) => {
  try {
    const records = await WaterChiller2tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await WaterChiller2tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await WaterChiller2tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
