...
const DG10103tbSchema = new mongoose.Schema({
...
const DG10103tb = mongoose.model('DG10103tb', DG10103tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DG10103tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await DG10103tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG10103tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG10103tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
