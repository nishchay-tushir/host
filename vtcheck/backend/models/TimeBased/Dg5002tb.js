...
const DG5002tbSchema = new mongoose.Schema({
...
const DG5002tb = mongoose.model('DG5002tb', DG5002tbSchema);
...
router.post('/', async (req, res) => {
  try {
      ...
      const existingRecord = await DG5002tb.findOne({
          submissionDate: {
              $gte: startOfQuarter,
...
router.get('/', async (req, res) => {
  try {
    const records = await DG5002tb.find();
...
router.put('/:id', async (req, res) => {
  try {
    const updatedRecord = await DG5002tb.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (updatedRecord) {
...
router.delete('/:id', async (req, res) => {
  try {
    const deletedRecord = await DG5002tb.findByIdAndDelete(req.params.id);
    if (deletedRecord) {
